...
public class ExpressionEvaluatorServiceImpl implements ExpressionEvaluatorService {
    ...
    static{
        ...
        singletonManager.addCache(new Cache("ExpressionEvaluatorServiceCache", 1500, false, false, 120, 0));
    }
    ...
    public String evaluateExpressionTemplate(StandardEvaluationContext context,
        ...
        try {
            ...
            if (StringUtils.contains(expressionTemplate, UifConstants.EL_PLACEHOLDER_PREFIX)) {
                expression = getTemplateExpression(expressionTemplate);
            } else {
                expression = getExpression(expressionTemplate);
            }
    ...
    public Object evaluateExpression(StandardEvaluationContext context,
        ...
        Object result = null;
        try {
            Expression expression = getExpression(expressionStr);
    ...
    private Expression getTemplateExpression(String expressionTemplate) {
        Element cachedResult = expressionCache.get(expressionTemplate);
        Object result;
        if(cachedResult==null){
            result = parser.parseExpression(expressionTemplate, new TemplateParserContext(
                    UifConstants.EL_PLACEHOLDER_PREFIX, UifConstants.EL_PLACEHOLDER_SUFFIX));;
            expressionCache.put(new Element(expressionTemplate, result));
        }else{
