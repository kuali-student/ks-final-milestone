...
//                StandardResponse stdResp = simpleDocService.requestAdHocAckToPrincipal(docId, username, recipientPrincipalId, ackAnnotation);
	...
	private class LuRuleInfoPersistanceBean {
	    ...
	    public String saveRules(String cluId, List<RuleInfo> rules) throws Exception {
	    	for (RuleInfo ruleInfo : rules) {
	            ...
	            StatusInfo status = service.addLuStatementToClu(cluId, rooStmtId);
	    ...
	    private void deleteRule(String cluId, StatementVO statement, boolean topLevel) throws Exception {
	        ...
	        if (topLevel) {
	            ...
	            StatusInfo status = service.removeLuStatementFromClu(cluId, topStmt.getId());
	            if (!status.getSuccess()) {
	        ...
	        if ((stmtVOs != null) && (stmtVOs.size() > 0)) {        	          
	            ...
	            for (StatementVO stmtVO : stmtVOs) {            	          	
	                ...
	                StatusInfo status = service.deleteLuStatement(stmt.getId());
	                if (!status.getSuccess()) {
	            ...
	            for (ReqComponentVO reqCompVO : reqCompVOs) {             
	            	...
	            	//TODO implement
	                List<LuStatementInfo> refStmts = service.getStatementsUsingComponent(reqComp.getId()); 
	            	...
	            	StatusInfo status = service.deleteReqComponent(reqComp.getId());
	            	if (!status.getSuccess()) {            	
