...
import org.kuali.student.lum.lu.assembly.data.client.refactorme.base.ProposalDocRelationInfoHelper.Properties;


public class ProposalDocRelationInfoMetadata
{
	...
	{
		int recurseLevel = increment (recursions, "ProposalDocRelationInfoMetadata");
		
		Metadata childMeta;
		Metadata listMeta;
		
		// metadata for proposalId
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.PROPOSAL_ID.getKey (), childMeta);
		...
		if (this.matches (type, state, "(default)", "(default)"))
		{
			...
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("related.proposal"));
		}
		
		// metadata for documentId
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.DOCUMENT_ID.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.STRING);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("required"));
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("single"));
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("related.document"));
		}
