...
import org.kuali.student.lum.lu.assembly.data.client.refactorme.base.LoLoRelationInfoHelper.Properties;


public class LoLoRelationInfoMetadata
{
	...
	{
		int recurseLevel = increment (recursions, "LoLoRelationInfoMetadata");
		...
		// metadata for relatedLoId
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.RELATED_LO_ID.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.STRING);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
