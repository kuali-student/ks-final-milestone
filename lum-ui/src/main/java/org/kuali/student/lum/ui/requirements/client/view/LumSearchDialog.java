 ...
 * @author stse & Zdenek
 */
public class LumSearchDialog <T extends ILumModelObject> extends Composite implements ModelWidget<T> {
    ...
    KSTextArea requirementCompositionText = new KSTextArea();
    ...
    ListBox cluList = new ListBox();
    TextArea exampleArea = new TextArea();
    KSButton btnCancelRequirement = new KSButton("Cancel");
    KSButton btnAddComposedRequirement = new KSButton("Add Courses");
    
    boolean loaded = false;

    public LumSearchDialog() {
      super.initWidget(mainDialogPanel);
    }

    public void update(T modelObject) {
        this.modelObject = modelObject;
        boolean show = ((LumModelObject)modelObject).isShowSearchDialog();
        System.out.println("lumseachdialog: flag:" + show);
        if (show) {// && (!reqEditDialog.isShowing())) {
            System.out.println("Showing dialog...");
           // reqEditDialog.center();
            reqEditDialog.show();
            System.out.println("here: " + reqEditDialog.isShowing());
        } else { //if (!show && reqEditDialog.isShowing()) {
    ...
    public void layoutWidgets() {
        ...
        RequirementsService.Util.getInstance().getAllClus(new AsyncCallback<List<Result>>() {
            ...
            public void onSuccess(List<Result> clus) {
                System.out.println("Got back from service call...");
                for (Result clu : clus) {
                    System.out.println("CLU: " + clu.getResultCells().get(0).getKey());
                    cluList.addItem(clu.getResultCells().get(0).getValue(), clu.getResultCells().get(0).getKey());
                }   
            }
        });           
         
        // Setup Requirement Type selection area
        cluList.setVisibleItemCount(10);
        VerticalPanel multiBoxPanel = new VerticalPanel();        
        KSLabel coursesLbl = new KSLabel("Courses");
        coursesLbl.getElement().getStyle().setProperty("fontWeight", "bold");
        multiBoxPanel.add(coursesLbl);
        multiBoxPanel.add(cluList);        
        ...
        examplePanel.add(new Label("Course Details"));
        ...
        KSLabel selCoursesLbl = new KSLabel("Selected Courses");
        selCoursesLbl.getElement().getStyle().setProperty("fontWeight", "bold");        
        compositionText.add(selCoursesLbl);
        ...
        dialogContents.setSize("800px", "300px");   
        ...
        reqEditDialog.setHeader("Add Courses or Course Set");
        reqEditDialog.setModal(true);
        //reqEditDialog.center();    TODO -> throwing NULL pointer exception?                 
    ...
    public void setUpListeners() {    
        ...
        btnCancelRequirement.addClickHandler(new ClickHandler() {
            public void onClick(ClickEvent sender) {
                ...
                if (lumUIListeners != null && !lumUIListeners.isEmpty()) {
                    for (LumUIEventListener listener : lumUIListeners) {
                        ...
                        listener.hideSearchDialog();
                    }
                }
            }
        });  
        cluList.addClickHandler(new ClickHandler() {
            public void onClick(ClickEvent sender) {
                System.out.println("requirementTypes clicked");
                if (lumUIListeners != null && !lumUIListeners.isEmpty()) {
                    for (LumUIEventListener listener : lumUIListeners) {
                        String cluId = ((ListBox)sender.getSource()).getItemText(((ListBox)sender.getSource()).getSelectedIndex());
                        String cluIds = (requirementCompositionText.getText().isEmpty() ? cluId : requirementCompositionText.getText() + ", " + cluId);                                                                
                        requirementCompositionText.setText(cluIds);                        
                    }
                }
            }
        }); 
        btnAddComposedRequirement.addClickHandler(new ClickHandler() {
            public void onClick(ClickEvent sender) {
                System.out.println("btnAddComposedRequirement clicked");
                if (lumUIListeners != null && !lumUIListeners.isEmpty()) {
                    for (LumUIEventListener listener : lumUIListeners) {
                        //TODO check if user is not in the middle of editing i.e. ask if 'Abort changes?'
                        listener.updateSelectedCourseList(requirementCompositionText.getText());
                    }
                }
            }
        }); 
