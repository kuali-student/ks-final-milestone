...
import org.kuali.student.lum.lu.assembly.data.client.refactorme.base.DateRangeInfoHelper.Properties;


public class DateRangeInfoMetadata
{
	...
	public void loadChildMetadata (Metadata mainMeta, String type, String state,  RecursionCounter recursions)
	{
		...
		// metadata for startDate
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.START_DATE.getKey (), childMeta);
		...
		// metadata for endDate
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.END_DATE.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.DATE);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
		...
		if (this.matches (type, state, "(default)", "(default)"))
		{
			...
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("daterange.types"));
		}
