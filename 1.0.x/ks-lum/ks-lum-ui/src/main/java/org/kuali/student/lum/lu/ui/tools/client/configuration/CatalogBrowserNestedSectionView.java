...
public class CatalogBrowserNestedSectionView extends SectionView {
	...
	public CatalogBrowserNestedSectionView(Enum<?> viewEnum, String name,
			String modelId) {
		super(viewEnum, name);
	...
	public void beforeShow(final Callback<Boolean> onReadyCallback) {
		super.beforeShow(new Callback<Boolean>() {
			...
			public void exec(Boolean result) {
				if (!loaded) {
					layout.clear();
//					layout.add(CatalogBrowserNestedSectionView.this.sectionTitle);
//					layout.add(CatalogBrowserNestedSectionView.this.instructions);
//					layout.add(CatalogBrowserNestedSectionView.this.message);
					for (Section ns : sections) {
						ns.redraw();
						CatalogBrowserNestedSectionView.this
								.addSectionToLayout((BaseSection) ns);
					}
					for (FieldDescriptor f : fields) {
						CatalogBrowserNestedSectionView.this.addFieldToLayout(f);
					}
				...
				getController().requestModel(modelId,
	...
	protected void addFieldToLayout(FieldDescriptor fieldDescriptor) {
		...
		FieldInfo info = new FieldInfo(fieldDescriptor.getFieldLabel(),
				validationPanel, field, fieldLayout);
		...
		/*
		 * FieldElement field = new FieldElement(fieldDescriptor); FlowPanel
		 * panel = new FlowPanel(); panel.add(field); ValidationMessagePanel
		 * validationPanel = new ValidationMessagePanel();
		 * panel.add(validationPanel);
		 * pathValidationPanelMap.put(fieldDescriptor.getFieldKey(),
		 * validationPanel); layout.add(panel);
	...
	public void redraw() {
		...
		/*
		 * layout.clear(); layout.add(this.sectionTitle);
		 * layout.add(this.instructions); layout.add(this.message); for(Section
		 * ns: sections){ ns.redraw(); } for(FieldDescriptor f: fields){
