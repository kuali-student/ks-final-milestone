...
import org.kuali.student.lum.lu.assembly.data.client.refactorme.orch.CreditCourseCourseSpecificLOsHelper.Properties;


public class CreditCourseCourseSpecificLOsMetadata
{
	...
	public void loadChildMetadata (Metadata mainMeta, String type, String state,  RecursionCounter recursions)
	{
		...
		// metadata for IncludedSingleUseLo
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.INCLUDED_SINGLE_USE_LO.getKey (), childMeta);
		...
		recursions.set (SingleUseLoMetadata.class.getName (), 3);
		new SingleUseLoMetadata ().loadChildMetadata (childMeta, type, state, recursions);
		...
		childMeta.setDataType (Data.DataType.STRING);
		childMeta.setWriteAccess (Metadata.WriteAccess.NEVER);
		childMeta.setDefaultValue (new Data.StringValue ("kuali.lu.lo.relation.type.includes"));
		if (this.matches (type, state, "(default)", "(default)"))
		{
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("single"));
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("hard.coded.lulorealtion.includes"));
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("hard.coded.lulorealtion.includes"));
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("read.only"));
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("required"));
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("single"));
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("kuali.type"));
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("lu.lo.relation.types"));
		}
		...
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.SEQUENCE.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.STRING);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
