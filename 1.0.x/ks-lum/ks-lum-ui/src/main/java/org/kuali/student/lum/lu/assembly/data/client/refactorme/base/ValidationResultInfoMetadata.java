...
import org.kuali.student.lum.lu.assembly.data.client.refactorme.base.ValidationResultInfoHelper.Properties;


public class ValidationResultInfoMetadata
{
	...
	public void loadChildMetadata (Metadata mainMeta, String type, String state,  RecursionCounter recursions)
	{
		...
		mainMeta.getProperties ().put (Properties.ELEMENT.getKey (), childMeta);
		...
		// metadata for errorLevel
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.ERROR_LEVEL.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.STRING);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
		...
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.MESSAGE.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.STRING);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
		...
		// metadata for task
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.TASK.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.STRING);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
