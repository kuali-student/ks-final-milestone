...
import org.kuali.student.lum.lu.assembly.data.client.refactorme.base.FieldSelectorInfoHelper.Properties;


public class FieldSelectorInfoMetadata
{
	...
	public void loadChildMetadata (Metadata mainMeta, String type, String state,  RecursionCounter recursions)
	{
		...
		// metadata for isSelector
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.IS_SELECTOR.getKey (), childMeta);
		...
		// metadata for isDynamic
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.IS_DYNAMIC.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.BOOLEAN);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
