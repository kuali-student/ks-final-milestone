...
INSERT INTO KREW_STYLE_T (ACTV_IND,NM,OBJ_ID,STYLE_ID,VER_NBR,XML)
  VALUES (1,'widgets','6166CBA1BC06644DE0404F8189D86C09',2004,1,EMPTY_CLOB())
/
-- Length: 59806
-- Chunks: 15
...
BEGIN
    ...
    WHERE 
 STYLE_ID = 2004    FOR UPDATE;        
    buffer := '								<xsl:comment>* custom message: <xsl:value-of select="$custommessage"/>
</xsl:comment>
...
BEGIN
    ...
    WHERE 
 STYLE_ID = 2004    FOR UPDATE;        
    buffer := 'sl:param name="hasUserValue"/>				
...
BEGIN
    ...
    WHERE 
 STYLE_ID = 2004    FOR UPDATE;        
    buffer := '<xsl:variable name="clickFunctionVal" select="boolean(normalize-space($clickfunction))"/>
					<xsl:choose>
				...
				<xsl:template name="htmlHead">
					...
					<title>
						<xsl:value-of select="$pageTitle"/> docume';
    DBMS_LOB.writeappend(data,LENGTH(buffer),buffer);
END;
/
DECLARE    data CLOB; buffer VARCHAR2(32000);
...
BEGIN
    ...
    WHERE 
 STYLE_ID = 2004    FOR UPDATE;        
    buffer := 'l:variable name="annotatable" select="/documentContent/documentState/annotatable = ''true''"/>
						...
						<table align="center" border="0" cellpadding="0" cellspacing="0" class="bord-r-t" width="80%">
							...
							<xsl:choose>
								<xsl:when test="//NoteForm/numberOfNotes &gt;0">
									<tr>
										<td align="center" class="thnormal2" colspan="4" scope="c';
    DBMS_LOB.writeappend(data,LENGTH(buffer),buffer);
END;
/
DECLARE    data CLOB; buffer VARCHAR2(32000);
