...
public class DataDictionaryIndex implements Runnable {
	...
	// keyed by a class object, and the value is a set of classes that may block
	// the class represented by the key from inactivation
	...
	public DataDictionaryIndex(ConfigurableListableBeanFactory ddBeans) {
	...
	private void buildDDIndicies() {
		...
		Map<String, DataObjectEntry> boBeans = ddBeans
				.getBeansOfType(DataObjectEntry.class);
		for (DataObjectEntry entry : boBeans.values()) {

			DataObjectEntry indexedEntry = objectEntries
					.get(entry.getJstlKey());
			...
			if ((indexedEntry != null)
				...
				throw new DataDictionaryException(
						new StringBuilder(
								...
								.append(" / existing=")
								.append(indexedEntry.getDataObjectClass())
								.toString());
			}
			...
			objectEntries
					.put(entry.getDataObjectClass().getSimpleName(), entry);
			...
			if (entry instanceof BusinessObjectEntry) {
				...
				businessObjectEntries.put(boEntry.getBusinessObjectClass()
						.getName(), boEntry);
				businessObjectEntries.put(boEntry.getBusinessObjectClass()
						.getSimpleName(), boEntry);

				// If a "base" class is defined for the entry, index the entry
				// by that class as well.
				if (boEntry.getBaseBusinessObjectClass() != null) {
					businessObjectEntries.put(boEntry
							.getBaseBusinessObjectClass().getName(), boEntry);
					businessObjectEntries.put(boEntry
							.getBaseBusinessObjectClass().getSimpleName(),
							boEntry);
				}
			}
		...
		Map<String, DocumentEntry> docBeans = ddBeans
		...
		for (DocumentEntry entry : docBeans.values()) {
			...
			if ((entry instanceof TransactionalDocumentEntry)
					...
					&& !StringUtils.equals(
							documentEntries.get(entry.getFullClassName())
				...
				throw new DataDictionaryException(
						new StringBuilder(
								...
								.append(" / existing=")
								.append(((DocumentEntry) documentEntries
										.get(entry.getDocumentClass().getName()))
										.getDocumentTypeName()).toString());
			}

			if ((documentEntries.get(entry.getJstlKey()) != null)
			...
			if (entry.getBaseDocumentClass() != null) {
				documentEntries.put(entry.getBaseDocumentClass().getName(),
						entry);
			}
			entriesByJstlKey.put(entry.getJstlKey(), entry);
			...
			if (entry instanceof TransactionalDocumentEntry) {
				...
				documentEntries.put(tde.getDocumentClass().getSimpleName(),
						entry);
				if (tde.getBaseDocumentClass() != null) {
					documentEntries.put(tde.getBaseDocumentClass()
							.getSimpleName(), entry);
				}
			}
			...
			if (entry instanceof MaintenanceDocumentEntry) {
				...
				documentEntriesByBusinessObjectClass.put(
						mde.getDataObjectClass(), entry);
				documentEntriesByMaintainableClass.put(
						mde.getMaintainableClass(), entry);
				documentEntries.put(mde.getDataObjectClass().getSimpleName()
						+ "MaintenanceDocument", entry);
			}
		}
	}
	...
	private void buildDDInactivationBlockingIndices() {
		...
		Map<String, DataObjectEntry> doBeans = ddBeans
				.getBeansOfType(DataObjectEntry.class);
		for (DataObjectEntry entry : doBeans.values()) {
			List<InactivationBlockingDefinition> inactivationBlockingDefinitions = entry
					.getInactivationBlockingDefinitions();
			if (inactivationBlockingDefinitions != null
					&& !inactivationBlockingDefinitions.isEmpty()) {
	...
	private void registerInactivationBlockingDefinition(
			InactivationBlockingDefinition inactivationBlockingDefinition) {
		Set<InactivationBlockingMetadata> inactivationBlockingDefinitions = inactivationBlockersForClass
				.get(inactivationBlockingDefinition
						.getBlockedBusinessObjectClass());
		if (inactivationBlockingDefinitions == null) {
			inactivationBlockingDefinitions = new HashSet<InactivationBlockingMetadata>();
			inactivationBlockersForClass.put(inactivationBlockingDefinition
					.getBlockedBusinessObjectClass(),
					inactivationBlockingDefinitions);
		}
		boolean duplicateAdd = !inactivationBlockingDefinitions
				.add(inactivationBlockingDefinition);
		if (duplicateAdd) {
			throw new DataDictionaryException(
					"Detected duplicate InactivationBlockingDefinition for class "
	...
	/**
	 * Mapping of namespace codes to bean definition names that are associated
	 * with that namespace
	 * 
	 * @return Map<String, List<String>> where map key is namespace code, and
	 *         map value is list of bean names
	 */
	 ...
	 * 
	 * @param namespaceCode
	 *            - namespace code to associate beans with
	 * @param beanNames
	 *            - list of bean names that belong to the namespace
	 */
