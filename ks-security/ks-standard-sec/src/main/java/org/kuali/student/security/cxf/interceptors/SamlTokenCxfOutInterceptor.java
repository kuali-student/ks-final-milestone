...
public class SamlTokenCxfOutInterceptor extends AbstractWSS4JInterceptor {
	private static final Logger LOG = LogUtils.getL7dLogger(SamlTokenCxfOutInterceptor.class);

	private static final Logger TIME_LOG = LogUtils.getL7dLogger(SamlTokenCxfOutInterceptor.class, null, SamlTokenCxfOutInterceptor.class.getName() + "-Time");
	...
	/**
	 * Enable or disable mtom with WS-Security. By default MTOM is disabled as attachments would not get encrypted or be
	...
	final class SamlTokenCxfOutInterceptorInternal implements PhaseInterceptor<SoapMessage> {
		...
		public void handleMessage(SoapMessage mc) throws Fault {
			...
			try {
				 ...
				 */
				Vector<?> actions = new Vector<Object>();
				...
				/*
				 * For every action we need a username, so get this now. The username defined in the deployment
				...
				/*
				 * Now we perform some set-up for UsernameToken and Signature functions. No need to do it for encryption
				 * only. Check if username is available and then get a passowrd.
				 */
				if ((doAction & (WSConstants.SIGN | WSConstants.UT | WSConstants.UT_SIGN)) != 0 && (reqData.getUsername() == null || reqData.getUsername().equals(""))) {
					/*
					 * We need a username - if none throw an SoapFault. For encryption there is a specific parameter to
					 * get a username.
					 */
				...
				/*
				 * Now get the SOAP part from the request message and convert it into a Document. This forces CXF to
				 * serialize the SOAP request into FORM_STRING. This string is converted into a document. During the
				 * FORM_STRING serialization CXF performs multi-ref of complex data types (if requested), generates and
				 * inserts references for attachements and so on. The resulting Document MUST be the complete and final
				 * SOAP request as CXF would send it over the wire. Therefore this must shall be the last (or only)
				 * handler in a chain. Now we can perform our security operations on this request.
				 */
				...
				/**
				 * There is nothing to send...Usually happens when the provider needs to send a HTTP 202 message (with
				 * no content)
				 */
				if (mc == null) {
					return;
				...
				WSEncryptionPart encPBody = new WSEncryptionPart("Body", doc.getDocumentElement().getNamespaceURI(), "Content");
				reqData.getSignatureParts().add(encPBody);

				doSenderAction(doAction, doc, reqData, actions, Boolean.TRUE.equals(getProperty(mc, org.apache.cxf.message.Message.REQUESTOR_ROLE)));
		...
		public String handleMessageUserSAML(SoapMessage msg) throws Fault {
			...
			/*
			 * String user = getSamlProperties().get("user"); String pgt =
			 * getSamlProperties().get("proxyGrantingTicket"); String proxies = getSamlProperties().get("proxies");
			 * String issuer = getSamlProperties().get("samlIssuerForUser"); String nameQualifier =
			 * getSamlProperties().get("samlIssuerForUser");
			 * 
			 * SAMLAssertion assertion = new SAMLAssertion(); assertion.setIssuer(issuer);
			 * 
			 * try{ // prepare subject SAMLNameIdentifier nameId = new SAMLNameIdentifier(user, nameQualifier, "");
			 * String[] confirmationMethods = {SAMLSubject.CONF_SENDER_VOUCHES}; SAMLSubject subject = new
			 * SAMLSubject(); subject.setNameIdentifier(nameId);
			 * subject.setConfirmationMethods(Arrays.asList(confirmationMethods));
			 * 
			 * // prepare auth statement SAMLAuthenticationStatement authStmt = new SAMLAuthenticationStatement();
			 ...
			 * 
			 * // prepare attributes SAMLAttributeStatement attrStatement = new SAMLAttributeStatement(); SAMLAttribute
			 * attr1 = new SAMLAttribute(); attr1.setName("proxyGrantingTicket");
			 * attr1.setNamespace("Namesapce_of_Attribute1");
			 * 
			 * SAMLAttribute attr2 = new SAMLAttribute(); attr2.setName("proxies");
			 * attr2.setNamespace("Namesapce_of_Attribute2");
			 * 
			 * 
			 * attr1.addValue(pgt); attr1.addValue("additional value for proxy granting ticket");
			 * attr2.addValue(proxies); attr2.addValue("additional value for proxies");
			 * 
			 * attrStatement.addAttribute(attr1); attrStatement.addAttribute(attr2);
			 * 
			 * SAMLSubject subjectInAttr = (SAMLSubject)subject.clone(); attrStatement.setSubject(subjectInAttr);
			 * 
			 * // prepare Assertion assertion.addStatement(authStmt); assertion.addStatement(attrStatement);
			 * 
			 * assertionNode = assertion.toDOM();
			 * 
			 * } catch(SAMLException se){ Logger log = Logger.getLogger(SamlTokenCxfOutInterceptor.class.getName());
			 * throw new Fault("Error when adding SAML Attributes or Attribute Statement : ", log, se); }
			 * catch(CloneNotSupportedException cnse){ Logger log =
			 * Logger.getLogger(SamlTokenCxfOutInterceptor.class.getName()); throw new
			 * Fault("Error when cloning subject : ", log, cnse); }
			 */
