...
/**
 * Creates a List of EnumeratedValue DTOs, use to initialize database
 * for testing last index value minus initial index value determines
 * size of List EnumeratedValues are created with current index value
 * to give each object field pseudo unique values.
 * 
 * @author garystruthers Kuali Student Team 
 * @Immutable all fields are private, all mutator methods are private
 * @Threadsafe immutable classes are threadsafe The List and its
 * EnumeratedValue objects returned by this class are NOT immutable or
 * threadsafe
 */
