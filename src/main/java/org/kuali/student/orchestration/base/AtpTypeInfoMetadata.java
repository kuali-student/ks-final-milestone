...
import org.kuali.student.orchestration.base.AtpTypeInfoHelper.Properties;
...
public class AtpTypeInfoMetadata
{
	...
	{
		int recurseLevel = increment (recursions, "AtpTypeInfoMetadata");
		...
		// metadata for durationType
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.DURATION_TYPE.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.STRING);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
		...
		// metadata for seasonalType
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.SEASONAL_TYPE.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.STRING);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
