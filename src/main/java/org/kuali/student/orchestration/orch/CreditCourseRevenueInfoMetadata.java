...
import org.kuali.student.orchestration.orch.CreditCourseRevenueInfoHelper.Properties;


public class CreditCourseRevenueInfoMetadata
{
	...
	public void loadChildMetadata (Metadata mainMeta, String type, String state,  RecursionCounter recursions)
	{
		...
		childMeta.setDefaultValue (new Data.StringValue ("Revenue"));
		if (this.matches (type, state, "(default)", "(default)"))
		{
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("single"));
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("required"));
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("hard.coded.feetype.revenue"));
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("required"));
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("single"));
		...
		// metadata for RevenueOrg
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.REVENUE_ORG.getKey (), childMeta);
		...
		new AffiliatedOrgInfoMetadata ().loadChildMetadata (listMeta, type, state, recursions);
		
		// metadata for TotalPercentage
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.TOTAL_PERCENTAGE.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.STRING);
		childMeta.setWriteAccess (Metadata.WriteAccess.NEVER);
		if (this.matches (type, state, "(default)", "(default)"))
		{
