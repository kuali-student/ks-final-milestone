...
public class ServiceWriter
{
 ...
 public ServiceWriter (DictionaryModel model, String directory,
                       Service service)
 {
  this.model = model;
 ...
 public void write ()
 {
  ...
  {
   System.out.println ("generating exception class: " + service.getKey () + "." +
    error.getType ());
 ...
 private Set<XmlType> getXmlTypesUsedByService (List<ServiceMethod> methods)
 {
  ...
  for (ServiceMethod method : methods)
  {
   if (method.getReturnValue () != null)
   {
    ...
    if (xmlType == null)
    {
     throw new DictionaryValidationException ("Method " +
      method.getService () + "." + method.getName () +
      "returns an unknown type, " + ret.getType ());
    }
                                                 ...
                                                 XmlType xmlType)
 {
  ...
  for (MessageStructure ms : finder.findMessageStructures (xmlType.getName ()))
  {
   ...
   {
    throw new DictionaryValidationException ("Parameter " +
     ms.getId () + "has an unknown type, " + ms.getType ());
   }
