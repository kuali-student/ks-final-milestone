...
public class PropertyChangeSupport {
    ...
    public synchronized void addPropertyChangeListener(String propertyName, PropertyChangeListener listener) {
        if ((listener != null) && (propertyName != null)) {
            ...
            if (listener instanceof PropertyChangeListenerProxy) {
                ...
                listeners.add(new PropertyChangeListenerProxy(proxy.getPropertyName(), (PropertyChangeListener) proxy.getListener()));
            } else {
    ...
    public synchronized PropertyChangeListener[] getPropertyChangeListeners(String propertyName) {
        ...
        if (listeners == null) {
            ...
            PropertyChangeListener[] changeListeners = new PropertyChangeListener[listeners.size()];
    ...
    public synchronized PropertyChangeListener[] getPropertyChangeListeners() {
        ArrayList<PropertyChangeListener> result = new ArrayList<PropertyChangeListener>(allPropertiesChangeListeners);

        for (Iterator<String> it = selectedPropertiesChangeListeners.keySet().iterator(); it.hasNext();) {
            String propertyName = it.next();
            List<PropertyChangeListener> selectedListeners = selectedPropertiesChangeListeners.get(propertyName);

            if (selectedListeners != null) {
                for (Iterator<PropertyChangeListener> it1 = selectedListeners.iterator(); it1.hasNext();) {
                    PropertyChangeListener listener = it1.next();
        ...
        Iterator<PropertyChangeListener> iter = result.iterator();
        for (int i = 0; i < changeListeners.length; i++) {
            changeListeners[i] = iter.next();
        }
        return changeListeners;
    ...
    private void doFirePropertyChange(PropertyChangeEvent event) {
         ...
         */
        List<PropertyChangeListener> allListeners = new ArrayList<PropertyChangeListener>();
        ...
        // Fire the listeners
        for (Iterator<PropertyChangeListener> iter = allListeners.iterator(); iter.hasNext();) {
