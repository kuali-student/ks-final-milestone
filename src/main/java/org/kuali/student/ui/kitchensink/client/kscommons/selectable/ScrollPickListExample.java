 ...
 *
 */
public class ScrollPickListExample extends AbstractTablePickList<Person> {
   
    protected KSLabel label;
   
    public ScrollPickListExample() {
        super();
        ...
        main.add(label);
        PagingScrollTableBuilder<Person> builder = new PagingScrollTableBuilder<Person>();
        pagingScrollTablePicked = builder.tablePixelSize(220, 200).
        columnDefinitions(createColumnDefinitions()).
        build(new GenericTableModel<Person>(new PersonDTOs().getEmptyPersons()));
        pagingScrollTable = builder.build(new GenericTableModel<Person>(new PersonDTOs().getPersons()));


        main.add(createTableAndSelectionPanel());
