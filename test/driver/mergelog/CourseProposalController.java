...
public class CourseProposalController extends MenuEditableSectionController implements RequiresAuthorization, WorkflowEnhancedController {
	...
	private void init(final Callback<Boolean> onReadyCallback) {
		if (initialized) {
			...
			cluProposalRpcServiceAsync.getMetadata(idType, viewContextId, new AsyncCallback<Metadata>() {
	...
	private void init(DataModelDefinition modelDefinition) {
		...
		workflowUtil = new WorkflowUtilities((WorkflowRpcServiceAsync) GWT.create(CreditCourseProposalRpcService.class), this, cfg.getProposalIdPath(), createOnWorkflowSubmitSuccessHandler());
		...
		try {
			cluProposalRpcServiceAsync.saveData(cluProposalModel.getRoot(), new AsyncCallback<DataSaveResult>() {
				...
				public void onSuccess(DataSaveResult result) {
					// FIXME [KSCOR-225] needs to check validation results and
					// display messages if validation failed
							...
							public void exec(YesNoCancelEnum result) {
								...
								case NO:
									...
									getCurrentModel(new ModelRequestCallback<DataModel>() {
										...
										@Override
										public void onRequestFail(Throwable cause) {
											// TODO Is this correct... do we
											// want to stop view change if we
											// can't restore the data? Possibly
											// traps the user
											// if we don't it messes up saves,
											// possibly warn the user that it
											// failed and continue?
											okToChange.exec(false);
											dialog.hide();
