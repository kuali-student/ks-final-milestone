...
public class CourseProposalController extends MenuEditableSectionController
		implements RequiresAuthorization, WorkflowEnhancedController {

	// RPC Services
	CreditCourseProposalRpcServiceAsync cluProposalRpcServiceAsync = GWT
	...
	private void initialize() {
		...
		super.registerModel(CourseConfigurer.CLU_PROPOSAL_MODEL,
				new ModelProvider<DataModel>() {

					@Override
					public void requestModel(
							final ModelRequestCallback<DataModel> callback) {
		...
		super.addApplicationEventHandler(ValidateRequestEvent.TYPE,
					...
					public void onValidateRequest(ValidateRequestEvent event) {
						requestModel(new ModelRequestCallback<DataModel>() {
							...
							public void onRequestFail(Throwable cause) {
								GWT.log("Unable to retrieve model for validation",
										cause);
							}
		...
		addApplicationEventHandler(SaveActionEvent.TYPE,
				new SaveActionHandler() {
					public void doSave(SaveActionEvent saveAction) {
						GWT.log("CluProposalController received save action request.",
								null);
						doSaveAction(saveAction);
					}
				});

		addApplicationEventHandler(SubmitProposalEvent.TYPE,
				new SubmitProposalHandler() {
					public void onSubmitProposal() {
						GWT.log("CluProposalController received submit proposal request.",
								null);
	...
	private void initModel(final ModelRequestCallback<DataModel> callback,
			Callback<Boolean> workCompleteCallback) {
		if (getViewContext().getIdType() == IdType.DOCUMENT_ID) {
			...
			getCluProposalFromProposalId(getViewContext().getId(), callback,
	...
	private void getCurrentModel(
			...
			Callback<Boolean> workCompleteCallback) {
		if (cluProposalModel.getRoot() != null
				&& cluProposalModel.getRoot().size() > 0) {
	...
	private void init(final Callback<Boolean> onReadyCallback) {
		if (initialized) {
			...
			// For a new Create Course Proposal or Modify Course we send nulls
			// so that permissions are not checked.
			...
			cluProposalRpcServiceAsync.getMetadata(idType, viewContextId,
					new AsyncCallback<Metadata>() {
						...
						public void onSuccess(Metadata result) {
							DataModelDefinition def = new DataModelDefinition(
									result);
		...
		if (modelType == ReferenceModel.class) {
			if (cluProposalModel != null) {
				...
				if (cluProposalModel.get(CourseConfigurer.PROPOSAL_ID_PATH) != null) {
					ref.setReferenceId((String) cluProposalModel
							.get(CourseConfigurer.PROPOSAL_ID_PATH));
				} else {
			...
			if (cluProposalModel != null
					&& cluProposalModel.get(CourseConfigurer.PROPOSAL_ID_PATH) != null) {
				proposalId = cluProposalModel
						.get(CourseConfigurer.PROPOSAL_ID_PATH);
			}
	...
	@SuppressWarnings("unchecked")
	private void getCluProposalFromWorkflowId(
			final ModelRequestCallback callback,
	...
	@SuppressWarnings("unchecked")
	private void getCluProposalFromProposalId(String id,
			final ModelRequestCallback callback,
			final Callback<Boolean> workCompleteCallback) {
		KSBlockingProgressIndicator.addTask(loadDataTask);
	...
	@SuppressWarnings("unchecked")
	private void getNewProposalWithCopyOfClu(
			final ModelRequestCallback callback,
			final Callback<Boolean> workCompleteCallback) {
		KSBlockingProgressIndicator.addTask(loadDataTask);
		cluProposalRpcServiceAsync.getNewProposalWithCopyOfClu(getViewContext()
				.getId(), new AsyncCallback<Data>() {
		...
		requestModel(new ModelRequestCallback<DataModel>() {
			...
			public void onModelReady(DataModel model) {
				...
				if (isStartViewShowing()) {
					// This call required so fields in start section, which also
					// appear in
					// other sections don't get overridden from updateModel call
					// above.
			...
			@Override
			public void onRequestFail(Throwable cause) {
				GWT.log("Unable to retrieve model for validation and save",
						cause);
			}
	...
	public boolean startSectionRequired() {
		String proposalId = cluProposalModel
				.get(CourseConfigurer.PROPOSAL_ID_PATH);

		// Defaulting the courseTitle to proposalTitle, this way course data
		// gets set and assembler doesn't
		// complain. This may not be the correct approach.
		String courseTitle = cluProposalModel
				.get(CourseConfigurer.COURSE_TITLE_PATH);
		if (courseTitle == null) {
			String proposalTitle = cluProposalModel
					.get(CourseConfigurer.PROPOSAL_TITLE_PATH);
			cluProposalModel.set(
					QueryPath.parse(CourseConfigurer.COURSE_TITLE_PATH),
					proposalTitle);
		}

		return proposalId == null
				&& !CourseProposalController.this.isStartViewShowing();
	}
	...
	public void saveProposalClu(final SaveActionEvent saveActionEvent) {
		...
		try {
			cluProposalRpcServiceAsync.saveData(cluProposalModel.getRoot(),
					new AsyncCallback<DataSaveResult>() {
						public void onFailure(Throwable caught) {
							saveFailedCallback.exec(caught);
						}

						public void onSuccess(DataSaveResult result) {
							// FIXME [KSCOR-225] needs to check validation
							// results and display messages if validation failed
							cluProposalModel.setRoot(result.getValue());
							View currentView = getCurrentView();
							if (currentView instanceof SectionView) {
								((SectionView) currentView)
										.updateView(cluProposalModel);
								((SectionView) currentView).resetDirtyFlags();
							}
							if (saveActionEvent.isAcknowledgeRequired()) {
								saveMessage.setText("Save Successful");
								buttonGroup.getButton(OkEnum.Ok).setEnabled(
										true);
							} else {
								saveWindow.hide();
								saveActionEvent.doActionComplete();
							}
							ViewContext context = CourseProposalController.this
									.getViewContext();
							context.setId((String) cluProposalModel
									.get("proposal/id"));
	...
	public void checkAuthorization(final PermissionType permissionType,
			final AuthorizationCallback authCallback) {
		Map<String, String> attributes = new HashMap<String, String>();
		// if (StringUtils.isNotBlank(getViewContext().getId())) {
		GWT.log("Attempting Auth Check.", null);
		if ((getViewContext().getId() != null)
				&& (!"".equals(getViewContext().getId()))) {
			attributes.put(getViewContext().getIdType().toString(),
					getViewContext().getId());
		}
		cluProposalRpcServiceAsync.isAuthorized(permissionType, attributes,
				new AsyncCallback<Boolean>() {

					@Override
					public void onFailure(Throwable caught) {
						authCallback
								.isNotAuthorized("Error checking authorization.");
						GWT.log("Error checking proposal authorization.",
								caught);
						Window.alert("Error Checking Proposal Authorization: "
								+ caught.getMessage());
					}

					@Override
					public void onSuccess(Boolean result) {
						GWT.log("Succeeded checking auth for permission type '"
								+ permissionType + "' with result: " + result,
								null);
						if (Boolean.TRUE.equals(result)) {
							authCallback.isAuthorized();
						} else {
							authCallback
									.isNotAuthorized("User is not authorized: "
											+ permissionType);
						}
					}
				});
	...
	public void beforeViewChange(final Callback<Boolean> okToChange) {
		...
		// will display their own messages to the user to give them a reason why
		// the view
		// change has been cancelled, otherwise continue to check for reasons
		// not to change
			...
			public void exec(Boolean result) {
				if (result) {
					if (getCurrentView() instanceof SectionView
							&& ((SectionView) getCurrentView()).isDirty()) {
						ButtonGroup<YesNoCancelEnum> buttonGroup = new YesNoCancelGroup();
						final ButtonMessageDialog<YesNoCancelEnum> dialog = new ButtonMessageDialog<YesNoCancelEnum>(
								"Warning",
								"You may have unsaved changes.  Save changes?",
								buttonGroup);
						buttonGroup
								.addCallback(new Callback<YesNoCancelEnum>() {
													...
													new ModelRequestCallback<DataModel>() {
														...
														public void onRequestFail(
															...
															okToChange
																	.exec(false);
															dialog.hide();
															GWT.log("Unable to retrieve model for data restore on view change with no save",
																	cause);
														}
													}, NO_OP_CALLBACK);
