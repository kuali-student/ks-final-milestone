...
public class KSLumKRMSDataSetupTest extends KSLumAbstractBoTest {
	...
	static final String KSContextRepeatableforCredit = "Repeatable for Credit";
	static final String CONTEXT2 = "Context2";
	static final String NAME = "name";
	static final String CONTEXT1_QUALIFIER = "KSLum Qualifer 1";
	static final String CONTEXT1_QUALIFIER_VALUE = "KSLum Qualifer 1 - Value";
	static final String CONTEXT2_QUALIFIER = "KSLum Qualifer 1";
	static final String CONTEXT2_QUALIFIER_VALUE = "KSLum Qualifer 2 - Value";
	...
	static final String AGENDA_MayBeRepeatedForAMaxOfnCredits = "May be repeated for a maximum of <n> credits";
	static final String AGENDA_MustNotHaveSuccessfullyCompletedCourse = "Must not have successfully completed <course>";
	static final String AGENDA_MustHaveSuccessfullyCompletedAMinOfnCreditsFromCoursesInTheOrg = "Must have successfully completed a minimum of <n> credits from courses in the <org>";
	...
	public void setup() {
		...
		actionBoService = KrmsRepositoryServiceLocator
				.getBean("actionBoService");
		functionBoService = KrmsRepositoryServiceLocator
				.getBean("functionRepositoryService");
		krmsAttributeDefinitionService = KrmsRepositoryServiceLocator
				.getKrmsAttributeDefinitionService();

		boolean doTermDefinitions = true;
		if (doTermDefinitions ) {
			// createAllLumTermDefinitions(KSNAMESPACE);
		} else {

			ContextDefinition contextRecommendedPreparation = contextRepository
					.getContextByNameAndNamespace(KSContextRepeatableforCredit,
							KSNAMESPACE);
			ContextDefinition contextRepeatableForCredit = contextRepository
					.getContextByNameAndNamespace(KSContextRepeatableforCredit,
							KSNAMESPACE);

			// only set this stuff up if we don't already have Context1 (we
			// don't clear out KRMS tables between test methods)
			if (contextRecommendedPreparation == null) {

				PerformanceLogger perfLog = new PerformanceLogger();
				perfLog.log("starting agenda creation");

				createAllLumAttributeDefinitions(KSNAMESPACE);
				createAllLumTermDefinitions(KSNAMESPACE);
				KrmsTypeDefinition krmsContextTypeDefinition = createContextType(KSNAMESPACE);

				contextRepeatableForCredit = createContext(KSNAMESPACE,
						"Repeatable for Credit", krmsContextTypeDefinition);

				contextRecommendedPreparation = createContext(KSNAMESPACE,
						"Recommended Preparation", krmsContextTypeDefinition);
				...
				// createRulesForAgenda(agendaName, contextDefinition,
				// eventName,
				// nameSpace, agendaDef);

				// contextDefintion1 = createContextDefinition(KSNAMESPACE,
				// KSContextRepeatableforCredit, Collections.singletonMap(
				// CONTEXT1_QUALIFIER, CONTEXT1_QUALIFIER_VALUE));

				// createAgendaDefinition(AGENDA1, contextDefintion1,
				// TSUNAMI_EVENT, KSNAMESPACE);

				// ContextDefinition contextDefinition2 =
				// createContextDefinition(NAMESPACE2, CONTEXT2,
				// Collections.singletonMap(
				// CONTEXT2_QUALIFIER, CONTEXT2_QUALIFIER_VALUE));
				//
				// // Create multiple agendas so that we can test selection
				// createAgendaDefinition(AGENDA2, contextDefinition2,
				// EARTHQUAKE_EVENT, NAMESPACE2);
				// createAgendaDefinition(AGENDA3, contextDefinition2,
				// EARTHQUAKE_EVENT, NAMESPACE2);
				// createAgendaDefinition(AGENDA4, contextDefinition2,
				// TSUNAMI_EVENT, NAMESPACE2);

				perfLog.log("finished agenda creation", true);
			}

			// KrmsTypeDefinition krmsGenericTypeDefinition =
			// createKrmsGenericTypeDefinition(KSNAMESPACE,
			// "testAgendaTypeService", "event name", "Event");
			//
			// createAgendaDefinition(AGENDA_MayBeRepeatedForAMaxOfnCredits,
			// contextRepeatableForCredit, TSUNAMI_EVENT, KSNAMESPACE,
			// krmsGenericTypeDefinition);
			// createAgendaDefinition(AGENDA_MustNotHaveSuccessfullyCompletedCourse,
			// contextRecommendedPreparation, TSUNAMI_EVENT, KSNAMESPACE,
			// krmsGenericTypeDefinition);
			// createAgendaDefinition(AGENDA_MustHaveSuccessfullyCompletedAMinOfnCreditsFromCoursesInTheOrg,
			// contextRecommendedPreparation, TSUNAMI_EVENT, KSNAMESPACE,
			// krmsGenericTypeDefinition);
		}
	}
	...
	public void testNothing() {
		System.out.println("Tested setup");
	}
	...
	public void testSelectAgendaByAttributeAndName() {
		...
		contextQualifiers.put(NAME, KSContextRepeatableforCredit);
		...
		SelectionCriteria sc1 = SelectionCriteria.createCriteria(now,
				contextQualifiers, Collections.singletonMap(
						AgendaDefinition.Constants.EVENT, TSUNAMI_EVENT));
		...
		EngineResults eResults1 = KrmsApiServiceLocator.getEngine().execute(
				sc1, factsBuilder1.build(), xOptions1);
		perfLog.log("finished rule execution", true);
		List<ResultEvent> rEvents1 = eResults1.getAllResults();

		List<ResultEvent> ruleEvaluationResults1 = eResults1
				.getResultsOfType(ResultEvent.RULE_EVALUATED.toString());

		assertEquals("4 rules should have been evaluated", 4,
				ruleEvaluationResults1.size());

		assertTrue("rule 0 should have evaluated to true",
				ruleEvaluationResults1.get(0).getResult());
		assertFalse("rule 1 should have evaluated to false",
				ruleEvaluationResults1.get(1).getResult());
		assertTrue("rule 2 should have evaluated to true",
				ruleEvaluationResults1.get(2).getResult());

		// ONLY agenda 1 should have been selected
		assertTrue(TestActionTypeService
				.actionFired("TestAgenda1::Rule1::TestAction"));
		assertFalse(TestActionTypeService
				.actionFired("TestAgenda1::Rule2::TestAction"));
		assertTrue(TestActionTypeService
				.actionFired("TestAgenda1::Rule3::TestAction"));
	...
	public void testSelectAgendaByName() {
		...
		/*
		 * We'll specifically NOT select this attribute to make sure that
		 * matching only takes place against qualifiers in the selection
		 * criteria
		 */
		// agendaQualifiers.put(AgendaDefinition.Constants.EVENT,
		// EARTHQUAKE_EVENT);

		agendaQualifiers.put(NAME, AGENDA3);
		DateTime now = new DateTime();

		SelectionCriteria selectionCriteria = SelectionCriteria.createCriteria(
				now, contextQualifiers, agendaQualifiers);
		...
		EngineResults eResults1 = KrmsApiServiceLocator.getEngine().execute(
				selectionCriteria, factsBuilder2.build(), xOptions2);
		...
		selectionCriteria = SelectionCriteria.createCriteria(now,
				contextQualifiers, agendaQualifiers);
		...
		assertTrue(TestActionTypeService
				.actionFired("Agenda3::Rule1::TestAction"));
		assertFalse(TestActionTypeService
				.actionFired("Agenda3::Rule2::TestAction"));
		assertTrue(TestActionTypeService
				.actionFired("Agenda3::Rule3::TestAction"));
	...
	public void testSelectMultipleAgendasByAttribute() {
		...
		Map<String, String> agendaQualifiers = new HashMap<String, String>();
		agendaQualifiers
				.put(AgendaDefinition.Constants.EVENT, EARTHQUAKE_EVENT);
		...
		assertEquals("8 rules should have been evaluated", 8,
				ruleEvaluationResults1.size());
		...
		assertTrue(TestActionTypeService
				.actionFired("Agenda2::Rule1::TestAction"));
		assertFalse(TestActionTypeService
				.actionFired("Agenda2::Rule2::TestAction"));
		assertTrue(TestActionTypeService
				.actionFired("Agenda2::Rule3::TestAction"));
	...
	private void assertAgendaDidNotExecute(String agendaName) {
		assertFalse(TestActionTypeService.actionFired(agendaName
				+ "::Rule1::TestAction"));
		assertFalse(TestActionTypeService.actionFired(agendaName
				+ "::Rule2::TestAction"));
		assertFalse(TestActionTypeService.actionFired(agendaName
	...
	private void createAgendaDefinition(String agendaName,
			...
			String nameSpace, KrmsTypeDefinition krmsGenericTypeDefinition) {
		AgendaDefinition agendaDef = AgendaDefinition.Builder.create(null,
				agendaName, krmsGenericTypeDefinition.getId(),
				contextDefinition.getId()).build();
		agendaDef = agendaBoService.createAgenda(agendaDef);
	}

	private void createRulesForAgenda(String agendaName,
			ContextDefinition contextDefinition, String eventName,
			String nameSpace, AgendaDefinition agendaDef) {
		...
		AgendaItemDefinition.Builder agendaItemBuilder2 = AgendaItemDefinition.Builder
				.create(null, agendaDef.getId());
		agendaItemBuilder1.setAlways(agendaItemBuilder2);
		agendaItemBuilder2.setRuleId(createRuleDefinition2(contextDefinition,
				agendaName, nameSpace).getId());

		AgendaItemDefinition.Builder agendaItemBuilder3 = AgendaItemDefinition.Builder
				.create(null, agendaDef.getId());
		agendaItemBuilder2.setAlways(agendaItemBuilder3);
		agendaItemBuilder3.setRuleId(createRuleDefinition3(contextDefinition,
				agendaName, nameSpace).getId());

		AgendaItemDefinition.Builder agendaItemBuilder4 = AgendaItemDefinition.Builder
				.create(null, agendaDef.getId());
		agendaItemBuilder3.setAlways(agendaItemBuilder4);
		agendaItemBuilder4.setRuleId(createRuleDefinition4(contextDefinition,
				agendaName, nameSpace).getId());

		// String these puppies together. Kind of a PITA because you need the id
		// from the next item before you insert the previous one
		AgendaItemDefinition agendaItem4 = agendaBoService
				.createAgendaItem(agendaItemBuilder4.build());
		agendaItemBuilder3.setAlwaysId(agendaItem4.getId());
		AgendaItemDefinition agendaItem3 = agendaBoService
				.createAgendaItem(agendaItemBuilder3.build());
		agendaItemBuilder2.setAlwaysId(agendaItem3.getId());
		AgendaItemDefinition agendaItem2 = agendaBoService
				.createAgendaItem(agendaItemBuilder2.build());
	...
	private KrmsTypeDefinition createKrmsActionTypeDefinition(String nameSpace) {
		...
		KrmsTypeDefinition krmsActionTypeDefinition = krmsTypeRepository
				.getTypeByName(nameSpace, ACTION_TYPE_NAME);

		if (krmsActionTypeDefinition == null) {
			KrmsTypeDefinition.Builder krmsActionTypeDefnBuilder = KrmsTypeDefinition.Builder
					.create(ACTION_TYPE_NAME, nameSpace);
			krmsActionTypeDefnBuilder.setServiceName("testActionTypeService");
			krmsActionTypeDefinition = krmsTypeRepository
					.createKrmsType(krmsActionTypeDefnBuilder.build());
		}
	...
	private RuleDefinition createRuleDefinition(String nameSpace,
		...
		// Action
		ActionDefinition.Builder actionDefBuilder1 = ActionDefinition.Builder
				.create(null, ruleName + "::TestAction", nameSpace,
						createKrmsActionTypeDefinition(nameSpace).getId(),
						ruleDef1.getId(), 1);
		ActionDefinition actionDef1 = actionBoService
				.createAction(actionDefBuilder1.build());
	...
	private RuleDefinition createRuleDefinition1(
		...
		params1.add(
				createTermDefinition(CAMPUS_CODE_TERM_NAME, String.class,
		...
		return createRuleDefinition(nameSpace, agendaName + "::Rule1",
				contextDefinition, LogicalOperator.OR, params1, params2);
	}
	...
	private RuleDefinition createRuleDefinition2(
			ContextDefinition contextDefinition, String agendaName,
			String nameSpace) {

		PropositionParametersBuilder params1 = new PropositionParametersBuilder();
		params1.add(
				createTermDefinition2(contextDefinition, nameSpace).getId(),
				PropositionParameterType.TERM);
		...
		return createRuleDefinition(nameSpace, agendaName + "::Rule2",
				contextDefinition, LogicalOperator.AND, params1, params2);
	}

	private RuleDefinition createRuleDefinition3(
			ContextDefinition contextDefinition, String agendaName,
			String nameSpace) {

		FunctionDefinition gcdFunction = functionBoService
				.getFunctionByNameAndNamespace("gcd",
						contextDefinition.getNamespace());

		if (null == gcdFunction) {
			// better configure a custom fuction for this
			// KrmsType for custom function
			KrmsTypeDefinition.Builder krmsFunctionTypeDefnBuilder = KrmsTypeDefinition.Builder
					.create("KrmsTestFunctionType", nameSpace);
			krmsFunctionTypeDefnBuilder
					.setServiceName("testFunctionTypeService");
			KrmsTypeDefinition krmsFunctionTypeDefinition = krmsTypeRepository
					.createKrmsType(krmsFunctionTypeDefnBuilder.build());

			FunctionDefinition.Builder functionBuilder = FunctionDefinition.Builder
					.create(contextDefinition.getNamespace(), "gcd",
							Integer.class.getName(),
							krmsFunctionTypeDefinition.getId());

			functionBuilder.getParameters().add(
					FunctionParameterDefinition.Builder.create("arg0",
							Integer.class.getName(), 0));
			functionBuilder.getParameters().add(
					FunctionParameterDefinition.Builder.create("arg1",
							Integer.class.getName(), 1));
			functionBuilder.setReturnType(Integer.class.getName());

			gcdFunction = functionBoService.createFunction(functionBuilder
					.build());
		}
		...
		params.add(gcdFunction.getId(), PropositionParameterType.FUNCTION); // this
																			// should
																			// evaluate
																			// first:
																			// gcd(1024,
		...
		return createRuleDefinition(nameSpace, agendaName + "::Rule3",
				contextDefinition, null, params);
	}

	private RuleDefinition createRuleDefinition4(
			ContextDefinition contextDefinition, String agendaName,
			String nameSpace) {

		PropositionParametersBuilder params1 = new PropositionParametersBuilder();
		params1.add(
				createTermDefinition(BOOL1, Boolean.class, contextDefinition)
						.getId(), PropositionParameterType.TERM);
		params1.add(
				createTermDefinition(BOOL2, Boolean.class, contextDefinition)
						.getId(), PropositionParameterType.TERM);
		params1.add("=", PropositionParameterType.OPERATOR);
		...
		return createRuleDefinition(nameSpace, agendaName + "::Rule4",
				contextDefinition, LogicalOperator.AND, params1, params2);
	}

	private TermDefinition createTermDefinition2(
			ContextDefinition contextDefinition, String nameSpace) {

		Map<String, String> queryArgs = new HashMap<String, String>();
		queryArgs.put("specification.namespace",
				contextDefinition.getNamespace());
		queryArgs.put("specification.name", "outputTermSpec");
		TermBo result = getBoService()
				.findByPrimaryKey(TermBo.class, queryArgs);
		if (result != null)
			return TermBo.to(result);

		// output TermSpec
		TermSpecificationDefinition outputTermSpec = TermSpecificationDefinition.Builder
				.create(null, "outputTermSpec",
						contextDefinition.getNamespace(), "java.lang.String")
				.build();
		outputTermSpec = termBoService.createTermSpecification(outputTermSpec);

		// prereq TermSpec
		TermSpecificationDefinition prereqTermSpec = TermSpecificationDefinition.Builder
				.create(null, PREREQ_TERM_NAME,
						contextDefinition.getNamespace(), "java.lang.String")
				.build();
		prereqTermSpec = termBoService.createTermSpecification(prereqTermSpec);

		// Term Param
		TermParameterDefinition.Builder termParamBuilder2 = TermParameterDefinition.Builder
				.create(null, null, "testParamName", "testParamValue");
		...
		KrmsTypeDefinition.Builder krmsTermResolverTypeDefnBuilder = KrmsTypeDefinition.Builder
				.create("KrmsTestResolverType", nameSpace);
		krmsTermResolverTypeDefnBuilder
				.setServiceName("testTermResolverTypeService");

		KrmsTypeDefinition krmsTermResolverTypeDefinition = krmsTypeRepository
				.createKrmsType(krmsTermResolverTypeDefnBuilder.build());

		// TermResolver
		TermResolverDefinition termResolverDef = TermResolverDefinition.Builder
				.create(null,
						contextDefinition.getNamespace(),
						"testResolver1",
						krmsTermResolverTypeDefinition.getId(),
						TermSpecificationDefinition.Builder
								.create(outputTermSpec),
						Collections
								.singleton(TermSpecificationDefinition.Builder
										.create(prereqTermSpec)), null,
	...
	protected List<Lifecycle> getSuiteLifecycles() {
		...
		List<Lifecycle> steps = super.getSuiteLifecycles();
		int cnt = -1;
		for (int i = 0; i < steps.size(); i++) {
			if (steps.get(i) instanceof ClearDatabaseLifecycle) {
				cnt = i;
			}
		}
		if (cnt > 0) {
	...
	protected void createAllLumTermDefinitions(String nameSpace) {
		// Create all the terms...
		createNumberOfCreditsTermDefinition(nameSpace);
		createDeptOrgNumberTermDefinition(nameSpace);
	...
	private void createDeptOrgNumberTermDefinition(String nameSpace) {
		...
		Map<String, String> queryArgs = new HashMap<String, String>();
		queryArgs.put("namespace", nameSpace);
		queryArgs.put("name", TERM_SPEC_ORG_NUMBER);
		TermSpecificationBo termSpecBo = getBoService().findByPrimaryKey(
				TermSpecificationBo.class, queryArgs);
		//
		TermSpecificationDefinition termSpec = null;
		if (termSpecBo == null) {
			 termSpec = TermSpecificationDefinition.Builder
					.create(null, TERM_SPEC_ORG_NUMBER, nameSpace,
							String.class.getCanonicalName()).build();
	...
	private void createProposedCourseTermDefinition(String nameSpace) {

		Map<String, String> queryArgs = new HashMap<String, String>();
		queryArgs.put("namespace", nameSpace);
		queryArgs.put("name", TERM_SPEC_COURSE);
		TermSpecificationBo termSpecBo = getBoService().findByPrimaryKey(
				TermSpecificationBo.class, queryArgs);
		//
		TermSpecificationDefinition termSpec = null;
		if (termSpecBo == null) {
			 termSpec = TermSpecificationDefinition.Builder
					.create(null, TERM_SPEC_COURSE, nameSpace,
							String.class.getCanonicalName()).build();
	...
	private void createApprovedCourseTermDefinition(String nameSpace) {
