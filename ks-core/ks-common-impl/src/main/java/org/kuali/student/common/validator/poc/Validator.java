...
public class Validator {
	...
	public List<ValidationResultInfo> validateField(FieldDefinition field, ObjectStructureDefinition objStruct,
		...
		if (DataType.COMPLEX
			...
			if (value instanceof Collection) {
				for (Object o : (Collection<?>) value) {
					processConstraint(results, field, field, objStruct, o, dataProvider, elementStack);
				}
				...
				processConstraint(results, field, field, objStruct, value, dataProvider, elementStack);
			}
	...
	private boolean isNullable(FieldDefinition field) {
		if (field.getMinOccurs() != null && field.getMinOccurs() > 0) {
			return false;
		}
		return true;
	...
	private void processConstraint(List<ValidationResultInfo> valResults,
			Constraint constraint, FieldDefinition field, 
			ObjectStructureDefinition objStructure, Object value,
		...
		processBaseConstraints(valResults, field, value, elementStack);
		...
		if (null != constraint.getOccursConstraint()
			...
			for (MustOccurConstraint oc : constraint.getOccursConstraint()) {
				ValidationResultInfo val = processOccursConstraint(elementPath,
						oc, field, objStructure, dataProvider);
				if (null != val) {
					valResults.add(val);
				}
			}
		...
		if (null != constraint.getCaseConstraint()
			...
			for (CaseConstraint cc : constraint.getCaseConstraint()) {
				processCaseConstraint(valResults, cc, field, 
						objStructure, value, dataProvider, elementStack);
			}
		}
	}

	private ValidationResultInfo processRequireConstraint(String element,
			RequiredConstraint constraint, FieldDefinition field,
			ObjectStructureDefinition objStructure, ConstraintDataProvider dataProvider) {
	...
	private void processCaseConstraint(List<ValidationResultInfo> valResults,
			CaseConstraint constraint, FieldDefinition field, 
			ObjectStructureDefinition objStructure, Object value,
			ConstraintDataProvider dataProvider, Stack<String> elementStack) {

		String operator = (hasText(constraint.getOperator())) ? constraint
				.getOperator() : "EQUALS";
		FieldDefinition caseField = (hasText(constraint.getFieldPath())) ? ValidatorUtils
				.getField(constraint.getFieldPath(), objStructure) : null;

		// TODO: What happens when the field is not in the dataProvider?
		Object fieldValue = (null != caseField) ? dataProvider
				.getValue(caseField.getName()) : value;

		// Extract value for field Key
		for (WhenConstraint wc : constraint.getWhenConstraint()) {
			Object whenValue = wc.getValue();

			if (ValidatorUtils.compareValues(fieldValue, whenValue, caseField.getDataType(), operator, dateParser)) {
				processConstraint(valResults, wc.getConstraint(), field, objStructure, value, dataProvider,
						elementStack);
			}
		}
	}
	...
	private ValidationResultInfo processOccursConstraint(String element,
			MustOccurConstraint constraint, FieldDefinition field,
			ObjectStructureDefinition objStructure, ConstraintDataProvider dataProvider) {
		...
		for (MustOccurConstraint oc : constraint.getOccurs()) {
			trueCount += (processOccursConstraint("", oc, field,
	...
	private void processBaseConstraints(List<ValidationResultInfo> valResults,
		...
		if (DataType.STRING.equals(dataType)) {
			validateString(value, field, elementPath, valResults);
		} else if (DataType.INTEGER.equals(dataType)) {
			validateInteger(value, field, elementPath, valResults);
		} else if (DataType.LONG.equals(dataType)) {
			validateLong(value, field, elementPath, valResults);
		} else if (DataType.DOUBLE.equals(dataType)) {
			validateDouble(value, field, elementPath, valResults);
		} else if (DataType.FLOAT.equals(dataType)) {
			validateFloat(value, field, elementPath, valResults);
		} else if (DataType.BOOLEAN.equals(dataType)) {
			validateBoolean(value, field, elementPath, valResults);
		} else if (DataType.DATE.equals(dataType)) {
			validateDate(value, field, elementPath, valResults, dateParser);
		}
	}
	...
	private void validateString(Object value, Constraint constraint,
			String element, List<ValidationResultInfo> results) {
		...
		if (maxLength != null && constraint.getMinLength() > 0) {
			if (s.length() > maxLength || s.length() < constraint.getMinLength()) {
				val
						.setError(MessageUtils.interpolate(
								getMessage("validation.lengthOutOfRange"), toMap(constraint)));
			}
