...
public class TestMetadataServiceImpl
{
 ...
 public void testMetadataService ()
 {
  ...
  ConstraintMetadata nameConstraints = properties.get ("firstName").getConstraints ().get (
    0);
  ...
  ConstraintMetadata gpaConstraints = properties.get ("gpa").getConstraints ().get (
    0);
  ...
  gpaConstraints = metadata.getProperties ().get ("gpa").getConstraints ().get (
    0);
  ...
  ConstraintMetadata addrLineConstraint = metadata.getProperties ().get ("line1").getConstraints ().get (
    0);
  ...
  metadata = metadataService.getMetadata (ADDRESS_INFO, US_ADDRESS_TYPE,
                                          "SUBMITTED");
  addrLineConstraint = metadata.getProperties ().get ("line1").getConstraints ().get (
    0);
  assertEquals (2, addrLineConstraint.getMinLength ().intValue ());
  assertEquals (1, addrLineConstraint.getMinOccurs ().intValue ());
  assertEquals (30, addrLineConstraint.getMaxLength ().intValue ());
  ...
  metadata =
  metadataService.getMetadata (ADDRESS_INFO, US_ADDRESS_TYPE, "DRAFT");
  addrLineConstraint = metadata.getProperties ().get ("line1").getConstraints ().get (
    0);
  assertEquals (2, addrLineConstraint.getMinLength ().intValue ());
  assertEquals (1, addrLineConstraint.getMinOccurs ().intValue ());
  ...
  metadata = metadataService.getMetadata (ADDRESS_INFO, CANADIAN_ADDRESS_TYPE,
                                          "DRAFT");
  addrLineConstraint = metadata.getProperties ().get ("line1").getConstraints ().get (
    0);
  assertEquals (2, addrLineConstraint.getMinLength ().intValue ());
 ...
 public void testMetadataFormatter ()
 {
  ...
  Set<String> startingObjects = new LinkedHashSet ();
...
//   out.println ("getting meta data for " + clazz.getName ());
   ...
   for (String type : types.get (objectKey))
   {
    ...
    metadata = metadataService.getMetadata (objectKey, type,
                                            (String) null);
    assertNotNull (metadata);
    formatter = new MetadataFormatter (objectKey,
                                       metadata, type,
                                       null, new HashSet (),
                                       1);
    out.println (formatter.formatForWiki ());
