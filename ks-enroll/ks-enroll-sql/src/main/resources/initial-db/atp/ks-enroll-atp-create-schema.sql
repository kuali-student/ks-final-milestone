...
-----------------------------------------------------------------------------
-- KSEN_ATPMSOTNE_RELTN_ATTR
-----------------------------------------------------------------------------
DECLARE temp NUMBER;
BEGIN
  SELECT COUNT(*) INTO temp FROM user_tables WHERE table_name = 'KSEN_ATPMSOTNE_RELTN_ATTR';
	IF temp > 0 THEN EXECUTE IMMEDIATE 'DROP TABLE KSEN_ATPMSOTNE_RELTN_ATTR CASCADE CONSTRAINTS PURGE'; END IF;
END;
/
...
CREATE TABLE KSEN_LUI 
   (	ID VARCHAR2(255), 
			...
			STATE_ID VARCHAR2(255), 
			TYPE_ID VARCHAR2(255), 
			HAS_WTLST NUMBER(22,0), 
			IS_WTLSTCHK_REQ NUMBER(22,0), 
			STDY_TITLE VARCHAR2(255), 
			WTLST_MAX NUMBER(22,0)
   )
/
