...
public class ProcessPocProcessServiceDecorator extends ProcessServiceDecorator {
    ...
    private void initializeData() {
        ...
        _createCheck(ProcessServiceConstants.CHECK_KEY_IS_ALIVE, "kuali.process.check.type.rule.direct", "", "", "kuali.agenda.is.alive", "", "is alive", "Checks if student is actually alive", context);
        _createCheck(ProcessServiceConstants.CHECK_KEY_HAS_OVERDUE_LIBRARY_BOOK, "kuali.process.check.type.hold", "kuali.hold.issue.library.book.overdue", "", "", "", "has overdue library book", "Checks if student has an overdue library book", context);
        _createCheck(ProcessServiceConstants.CHECK_KEY_HAS_NOT_PAID_BILL_FROM_PRIOR_TERM, "kuali.process.check.type.hold", "kuali.hold.issue.financial.unpaid.tuition.prior.term", "", "", "", "has not paid bill from prior term", "Checks if student has an unpaid bill from a prior term", context);
        _createCheck(ProcessServiceConstants.CHECK_KEY_STUDENT_HAS_BASIC_ELIGIBILITY, "kuali.process.check.type.process", "", "", "", "kuali.process.registration.basic.eligibility", "student has basic eligibility", "Checks all the checks defined in the basic eligibility process", context);
        _createCheck(ProcessServiceConstants.CHECK_KEY_REGISTRATION_PERIOD_IS_OPEN, "kuali.process.check.type.milestone.startdate", "", "kuali.atp.milestone.RegistrationPeriod", "", "", "registration period is open", "Checks that the registration period is open", context);
        _createCheck(ProcessServiceConstants.CHECK_KEY_REGISTRATION_PERIOD_IS_NOT_CLOSED, "kuali.process.check.type.milestone.deadline", "", "kuali.atp.milestone.RegistrationPeriod", "", "", "registration period is not closed", "Checks that the registration period is not yet closed", context);
        _createCheck(ProcessServiceConstants.CHECK_KEY_REGISTRATION_HOLDS_CLEARED, "kuali.process.check.type.process", "", "", "", "kuali.process.registration.holds.cleared", "Registration Holds Cleared", "Checks that the checks in the registration holds process", context);
        _createCheck(ProcessServiceConstants.CHECK_KEY_IS_NOT_SUMMER_TERM, "kuali.process.check.type.rule.direct", "", "", "kuali.agenda.is.not.summer.term", "", "is not Summer Term", "Checks that this is not the summer term", context);
        _createCheck(ProcessServiceConstants.CHECK_KEY_ELIGIBILITY_FOR_TERM, "kuali.process.check.type.process", "", "", "", "kuali.process.registration.eligibility.for.term", "Eligibility for Term", "Checks all the checks that the student is eligible for the term", context);
        _createCheck(ProcessServiceConstants.CHECK_KEY_HAS_THE_NECESSARY_PREREQ, "kuali.process.check.type.rule.indirect", "", "", "", "", "Has the necessary pre-req", "Checks that the student has all the necessary pee-requisites to take the course", context);
        _createCheck(ProcessServiceConstants.CHECK_KEY_STUDENT_HAS_ELIGIBILITY_FOR_EACH_COURSE, "kuali.process.check.type.process", "", "", "", "kuali.process.registration.eligible.for.course", "student has eligibility for each course", "Checks all the checks that make sure the student is eligible for a particular course but does it for all the courses in the proposed set of courses", context);
        _createCheck(ProcessServiceConstants.CHECK_KEY_DOES_NOT_EXCEED_CREDIT_LIMIT, "kuali.process.check.type.value.max", "", "", "", "", "does not exceed credit limit", "Checks that the student has not exceeded her credit limit", context);
