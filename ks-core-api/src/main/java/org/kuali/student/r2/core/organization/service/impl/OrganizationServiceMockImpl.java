...
public class OrganizationServiceMockImpl implements MockService, OrganizationService
{
    ...
    public TypeInfo getOrgOrgRelationTypeForOrgType(String orgTypeKey, ContextInfo contextInfo)
            ...
            ,PermissionDeniedException
    {
        if (!this.typeMap.containsKey(orgTypeKey)) {
            throw new DoesNotExistException(orgTypeKey);
        }
        return this.typeMap.get (orgTypeKey);
    }
            ...
            ,PermissionDeniedException
    {
        ...
        for (OrgOrgRelationInfo info: orgOrgRelationMap.values ()) {
            if (orgId.equals(info.getOrgId())) {
                if (orgOrgRelationTypeKey.equals(info.getOrgOrgRelationTypeKey())) {
                    list.add (info);
                }
            }
        }
            ...
            ,PermissionDeniedException
    {
        List<OrgPersonRelationInfo> list = new ArrayList<OrgPersonRelationInfo> ();
        for (OrgPersonRelationInfo info: orgPersonRelationMap.values ()) {
            if (orgPersonRelationTypeKey.equals(info.getOrgPersonRelationTypeKey())) {
                if (orgId.equals(info.getOrgId())) {
                    list.add (info);
                }
            }
    ...
    public OrgPersonRelationInfo getOrgPersonRelationByTypeAndOrg(String orgPersonRelationTypeKey, String orgId, ContextInfo contextInfo)
            ...
            ,PermissionDeniedException
    {
        throw new OperationFailedException ("getOrgPersonRelationByTypeAndOrg has not been implemented");
    }
    ...
    {
        // create
        if (!orgPersonRelationTypeKey.equals (orgPositionRestrictionInfo.getTypeKey())) {
            throw new InvalidParameterException ("The type parameter does not match the type on the info object");
        }
        // TODO: check the rest of the readonly fields that are specified on the create to make sure they match the info object
