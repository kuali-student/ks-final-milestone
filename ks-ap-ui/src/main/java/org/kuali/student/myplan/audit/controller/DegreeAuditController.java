...
public class DegreeAuditController extends UifControllerBase {
    ...
    public OrganizationService getOrganizationService() {
        if (this.organizationService == null) {
            ...
            this.organizationService = KsapFrameworkServiceLocator.getOrganizationService();
        }
    ...
    public ModelAndView audit(@ModelAttribute("KualiForm") DegreeAuditForm form, BindingResult result,
        ...
        try {
            ...
            String systemKey = UserSessionHelper.getStudentName();
            if (StringUtils.hasText(systemKey)) {
                ...
                //TODO - Donald Soar
                //Setting this campusParam will set the default campus in the client-browser.  Therefore, I think that it should be applied (in the future) when the default campus can be readily retrieved from the database - given the Person's ID (AdviserID or StudentID).
                //form.setCampusParam(campusMap.get("0"));
                logger.info("audit systemkey " + systemKey);
                if (!isAuditServiceUp) {
                    AtpHelper.addServiceError("programParam");
                } else {
            ...
            e.printStackTrace();
            String[] params = {};
            GlobalVariables.getMessageMap().putWarning("programParam", DegreeAuditConstants.TECHNICAL_PROBLEM, params);
        }
    ...
    //            String systemKey = UserSessionHelper.getAuditSystemKey();
                ...
                if (StringUtils.hasText(regid)) {
                    ...
                    if (!programId.equalsIgnoreCase(DegreeAuditConstants.DEFAULT_KEY)) {
                        ...
                        String[] params = {};
                        GlobalVariables.getMessageMap().putError("programParam", DegreeAuditConstants.AUDIT_RUN_FAILED, params);
                ...
                GlobalVariables.getMessageMap().putError("programParam", DegreeAuditConstants.NO_SYSTEM_KEY, params);
...
//            }
            String strCampusParam = getCellValue(row, "org.resultColumn.orgShortName");
            String index = Character.toString(strCampusParam.charAt(strCampusParam.length()  - 1));
            orgCampusTypes.put(index, getCellValue(row, "org.resultColumn.orgId"));
            //orgCampusTypes.put(getCellValue("0", "org.resultColumn.orgShortName"), getCellValue(row, "org.resultColumn.orgId"));
