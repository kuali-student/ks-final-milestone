    ...
    public ModelAndView findClasses(
        ...
        SessionSearchInfo table = form.getQuery() == null ? null
                ...
                new Callable<SessionSearchInfo>() {
                    @Override
                    public SessionSearchInfo call() throws Exception {
                        return new SessionSearchInfo(request, form, true);
                    }
                }, request);
        ...
        for (CourseLevel cl : CourseLevel.class.getEnumConstants()) {
          ...
          //IU Specific move when separating into independent module.
			if(cl.name().compareTo("PROFESSIONAL")==0){
				levelFacet.put("condition", "(edu.iu.sis.acadorg.SisInstitution.IUBLA||edu.iu.sis.acadorg.SisInstitution.IUINA)");
			}
        }
        clCondition.append(")");
        ...
        specificTermFacet.put("value", !form.getFacet().contains(CourseSearchForm.SEARCH_TERM_ANY_ITEM));
        ...
        ArrayNode scheduledSpecificTermFacets = scheduledSpecificTermFacet.putArray("facets");
        ...
        try {
			...
			for (int i = 0; i < tto.length; i++){
				...
				seasonTerm.put("value", form.getFacet().contains(termTypeInfos.get(i).getKey()));
