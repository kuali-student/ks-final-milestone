...
public class CourseDetailsInquiryViewHelperServiceImpl extends KualiInquirableImpl {
    ...
    public CourseDetails retrieveDataObject(Map fieldValues) {
        ...
        return retrieveCourseDetails((String) fieldValues.get(PlanConstants.PARAM_COURSE_ID), studentId, context);
    }
...
//        }
        ...
        try {
            course = getCourseService().getCourse(courseId, context);
        ...
        if (str != null && str.contains("Prerequisite:")) {
            String req = (getCourseLinkBuilder().makeLinks(str.substring(str.indexOf("Prerequisite:"), str.length()), courseLinkTemplateStyle, context));
        ...
        if (str != null) {
            str = getCourseLinkBuilder().makeLinks(str, context);
        }
    ...
    public CourseDetails retrieveCourseDetails(String courseId, String studentId, ContextInfo context) {
        CourseDetails courseDetails = retrieveCourseSummary(courseId, studentId, context);
        ...
        // Campus Locations
        List<OrgInfo> orgInfoList = OrgHelper.getOrgInfo(CourseSearchConstants.CAMPUS_LOCATION, CourseSearchConstants.ORG_QUERY_SEARCH_BY_TYPE_REQUEST, CourseSearchConstants.ORG_TYPE_PARAM, context);
        ...
        for (AttributeInfo entry : attributes) {
            if ("Y".equals(entry.getValue()) && entry.getKey().startsWith(CourseSearchConstants.GEN_EDU_REQUIREMENTS_PREFIX)) {
                EnumeratedValueInfo e = EnumerationHelper.getGenEdReqEnumInfo(entry.getKey(), context);
        ...
        */
        ...
        if (courseCode != null) {
            ...
            subject = splitStr[0];
            number = splitStr[1];
            String temp = getTitle(subject, context);
     ...
     * @return
     */
    protected String getTitle(String display, ContextInfo context) {
    ...
    private List<String> getCampusLocationsOfferedIn(String courseId, ContextInfo context) {
        List<String> campusLocations = new ArrayList<String>();
        SearchRequestInfo searchRequest = new SearchRequestInfo("myplan.course.getCampusLocations");
...
//        searchRequest.addParam("currentTerm", AtpHelper.getCurrentAtpId());
	...
	// TODO: determine ATP type functionality expectations - this is in ks-1.3, not in CM-2.0.
	// Does it need to move from 1.3 to 2.0 within KS, or has this been explicitly removed?
    // /mwfyffe
    /**
