...
public class CourseDetailsInquiryViewHelperServiceImpl extends
	...
	public CourseDetails retrieveCourseSummary(String courseId, String studentId) {
		...
		try {
			...
			course = getCourseService().getCourse(verifiedCourseId,
		...
		YearTerm tn;
		if (et != null
				&& !((tn = KsapFrameworkServiceLocator.getAtpHelper()
						.getYearTerm(et)).getYear() == 9999))
	...
	public CourseDetails retrieveCourseDetails(String courseId, String studentId) {
		...
		// Campus Locations
		List<OrgInfo> orgInfoList = KsapFrameworkServiceLocator.getOrgHelper()
				.getOrgInfo(
		...
		for (AttributeInfo entry : abbrAttributes)
			if ("Y".equals(entry.getValue())
				...
				abbrGenEdReqs.add(KsapFrameworkServiceLocator
						.getEnumerationHelper().getEnumAbbrValForCode(
								entry.getKey()));
		...
		try {
			...
			for (String term : termList) {
				YearTerm yt = KsapFrameworkServiceLocator.getAtpHelper()
						.getYearTerm(term);
				ytList.add(yt);
			}
			...
			for (YearTerm yt : ytList) {
				...
				String atp = yt.toATP();
				List<ActivityOfferingItem> list = getActivityOfferingItems(
						courseId, atp, courseDetails.getCode());
				courseOfferingDetails.setActivityOfferingItemList(list);
				courseDetails.getCourseOfferingDetails().add(
			...
			for (StudentCourseRecordInfo studentInfo : studentCourseRecordInfos) {
				...
				if (courseDetails.getCourseId().equalsIgnoreCase(
					...
					YearTerm yearTerm = KsapFrameworkServiceLocator
							.getAtpHelper().getYearTerm(
									studentInfo.getTermName());
	...
	public CourseDetails getCourseSummaryWithSections(String courseId,
			String studentId, String termId) {
		CourseDetails courseDetails = retrieveCourseSummary(courseId, studentId);
		CourseOfferingDetails courseOfferingDetails = new CourseOfferingDetails();
		courseOfferingDetails
				.setActivityOfferingItemList(getActivityOfferingItems(courseId,
						termId, null));
		courseDetails.getCourseOfferingDetails().add(courseOfferingDetails);
		return courseDetails;
	}

	public List<ActivityOfferingItem> getActivityOfferingItems(String courseId,
			String termId, String courseCode) {
		// Get version verified course
		courseId = getVerifiedCourseId(courseId);

		if (courseCode == null) {
			try {
				CourseInfo course = getCourseService().getCourse(
						courseId,
						KsapFrameworkServiceLocator.getContext()
								.getContextInfo());
		...
		try {
			...
			List<CourseOfferingInfo> courseOfferingInfoList = cos
			...
			activityOfferingItemList = courseOfferingDetails
			...
			for (CourseOfferingInfo courseInfo : courseOfferingInfoList) {
				...
				List<ActivityOfferingDisplayInfo> aodiList = cos
				...
				if (courseInfo.getCreditOptionId() != null) {
					// TODO
				}
				...
				for (ActivityOfferingDisplayInfo aodi : aodiList) {
						...
						for (ScheduleComponentDisplay scdi : sdi
							...
							for (TimeSlot timeSlot : scdi.getTimeSlots()) {
								...
								if (startInfo != null && endInfo != null) {
									...
									// startInfo.getMilliSeconds();
									// String startTime =
									// TimeStringMillisConverter.millisToStandardTime(startTimeMillis);
									//
									// long endTimeMillis =
									// endInfo.getMilliSeconds();
									// String endTime =
									// TimeStringMillisConverter.millisToStandardTime(endTimeMillis);
									//
					...
					// planned/backup
					boolean planned = isPlanned(
							courseCode + " " + aodi.getActivityOfferingCode(),
							termId);
	...
	private List<String> getCampusLocationsOfferedIn(String courseId,
		...
		if (searchResult != null) {
			for (SearchResultRow row : searchResult.getRows()) {
				campusLocations.add(KsapFrameworkServiceLocator.getOrgHelper()
						.getCellValue(row, "lu.resultColumn.campusVal"));
			}
		}
