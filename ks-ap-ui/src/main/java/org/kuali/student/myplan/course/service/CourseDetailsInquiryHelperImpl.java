...
public class CourseDetailsInquiryHelperImpl extends
        KualiInquirableImpl {
    ...
    public CourseSummaryDetails retrieveCourseSummaryById(String courseId){
        ...
        try {
            CourseInfo course = getCourseService().getCourse(verifiedCourseId, KsapFrameworkServiceLocator.getContext().getContextInfo());
    ...
    protected CourseSummaryDetails retrieveCourseSummary(CourseInfo course) {
        ...
        List<org.kuali.student.r2.core.organization.dto.OrgInfo> orgInfoList = KsapFrameworkServiceLocator.getOrgHelper().getOrgInfo(
        ...
        try{
            ...
            for (CourseOffering co : cos.getCourseOfferingsByCourse(course.getId(),
                    KsapFrameworkServiceLocator.getContext().getContextInfo()))
            ...
            for (TermInfo ti : getAcademicCalendarService()
                    .searchForTerms(
                            QueryByCriteria.Builder.fromPredicates(equalIgnoreCase(
                                    "atpState", PlanConstants.PUBLISHED)), KsapFrameworkServiceLocator.getContext().getContextInfo()))
    ...
    public CourseDetails retrieveCourseDetails(String courseId, String studentId) {
        ...
        if (request.getParameter("section_term") != null) {
            String termId = KsapFrameworkServiceLocator.getAtpHelper().getYearTerm(request.getParameter("section_term")).toTermName();
        ...
        for (CourseOfferingInstitution institution : courseDetails.getCourseOfferingInstitutionList()) {
            for (CourseOfferingTerm term : institution.getCourseOfferingTermList()) {
                for (ActivityOfferingItem activity : term.getActivityOfferingItemList()) {
                    ...
                    String quarter = term.getYearTerm().getTermAsString();
                    ...
                    try {
                        String[] enrollmentFields = enrollmentStatusHelper.populateEnrollmentFields(year, quarter, curric, num, sectionID);
        ...
        try {
            ...
            for (StudentCourseRecordInfo studentInfo : studentCourseRecordInfos) {
                ...
                if (course.getId().equalsIgnoreCase(studentInfo.getId())) {
                    ...
                    YearTerm str = KsapFrameworkServiceLocator.getAtpHelper().getYearTerm(studentInfo.getTermName());
                    plannedCourseSummary.getAcademicTerms().add(str.toTermName());
                }
            }
        } catch (Exception e) {
        ...
        for (YearTerm yt : ytList) {
            ...
            for (ActivityOfferingItem activityOfferingItem : list) {
                ...
                if (courseOfferingTerm == null) {
                    ...
                    courseOfferingTerm.setTerm(yt.toTermName());
