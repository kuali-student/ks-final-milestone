...
import org.kuali.student.ap.plan.review.form.ConversationCreateForm;
...
@Controller
@RequestMapping(value = "/review/conversation")
public class ConversationCreateController extends ConversationControllerBase {
	
	private static final Logger LOG = Logger.getLogger(ConversationCreateController.class);
	...
	@Override
	protected ConversationCreateForm createInitialForm(HttpServletRequest request) {
	...
	public ModelAndView getCreate(
		...
		//((LearningPlanReviewForm)form).initialize();
		init(form);
		return changeStep(ConversationConstants.CREATE_CONV_WIZARD_STEP1, form, result, request, response);
	}
	...
	public ModelAndView getNextStep(
		...
		return changeStep(form.getWizardStep()+1, form, result, request, response);
	}
	...
	public ModelAndView gotoStep2(
			...
			BindingResult result, HttpServletRequest request,
			HttpServletResponse response) throws IOException {
		return changeStep(ConversationConstants.CREATE_CONV_WIZARD_STEP2, form, result, request, response);
	}
	...
	public ModelAndView gotoStep3(
			...
			BindingResult result, HttpServletRequest request,
			HttpServletResponse response) throws IOException {
		return changeStep(ConversationConstants.CREATE_CONV_WIZARD_STEP3, form, result, request, response);
	}
	...
	public ModelAndView submit(
		...
		if (!result.hasErrors()) {
			...
			ConversationAdvisor advisorInfo = form.getAdvisorMap().get(form.getAdvisorId());
			lprr.setAdvisor(advisorInfo);
			
			RichText richMessage = new RichTextInfo(form.getMessageText(), form.getMessageText());
			lprr.setFirstCommentToAdvisor(richMessage);
			
			lprr.setOriginalLearningPlanId(form.getLearningPlanId());
			lprr.setReviewTerms(getSelectedCoursesForRequest(form.getAvailableAcademicYears(), form.getTopicType(), form.isIncludeBackups()));
			lprr.setTopic(form.getMessageTitle());
			
			lprr.setUnassignedCourseIds(form.getSelectedBookmarks());
			
			String reviewId = getLearningPlanReviewStrategy().createLearningPlanReview(lprr);
			//return changeStep(ConversationConstants.CREATE_CONV_WIZARD_STEP3, form, result, request, response);
	 ...
	 * @param academicYears
	 * @param selectionType Flag indicating if the user selected all/terms/courses
	 * @param includeBackup Flag indicating that backup courses should be included
	 * @return
	 */
	private List<LearningPlanReviewTerm> getSelectedCoursesForRequest(List<AcademicYearInfo> academicYears, String selectionType, boolean includeBackup) {
		...
		for (AcademicYearInfo year : academicYears) {
			for (PlanTermInfo term : year.getTerms()) {
				...
				if (includeBackup || ConversationConstants.CONV_TOPIC_TYPE_COURSES.equals(selectionType)) {
				...
				for (CourseInfo course : courses) {
					boolean add = false;
					if (ConversationConstants.CONV_TOPIC_TYPE_ALL.equals(selectionType)) {
						add = true;
					}
					else if (ConversationConstants.CONV_TOPIC_TYPE_TERMS.equals(selectionType)) {
						add = term.isSelected();
					}
					else if (ConversationConstants.CONV_TOPIC_TYPE_COURSES.equals(selectionType)) {
	 ...
	 */
	private ModelAndView changeStep(int newStep,
			ConversationCreateForm form,
			BindingResult result, HttpServletRequest request,
			HttpServletResponse response) throws IOException {
	...
	/**
	 * Go through the form's academicYear property looking for courses/terms that have been selected
	 * @param form
	...
	private void determineSelectedCourses(ConversationCreateForm form) {
		...
		if (ConversationConstants.CONV_TOPIC_TYPE_ALL.equals(form.getTopicType())) {
		...
		if (howManyMore > 0) {
			String moreLink = "and [id='show_more_link' actionLabel='&nbsp;" + howManyMore + " more']";
	...
	/**
	 * Wrap a string with a rich span tag so that css formatting can be done on it on the front end
	 * @param content String content to be wrapped
	...
	private void init(ConversationCreateForm form) {
		...
		try {
			List<LearningPlanInfo> plans = getAcademicPlanService().getLearningPlansForStudentByType(
					getUserId(), PlanConstants.LEARNING_PLAN_TYPE_PLAN,
					KsapFrameworkServiceLocator.getContext().getContextInfo());

			String planId = plans.get(0).getId();
			form.setLearningPlanId(planId);
			List<PlanItemInfo> planItems = getAcademicPlanService().getPlanItemsInPlan(
					planId,
					KsapFrameworkServiceLocator.getContext().getContextInfo());
			...
			for (PlanItemInfo planItem : planItems) {
				...
				course.setCourseName(plannedCourseDO.getCourseDetails().getCourseTitle());
				course.setCourseDescription(plannedCourseDO.getCourseDetails().getCourseDescription());
				boolean backup = false;


				if (PlanConstants.LEARNING_PLAN_ITEM_TYPE_WISHLIST.equals(planItem.getTypeKey())) {
					...
					if (pTerm == null) {
						...
						pTerm = new PlanTermInfo(term.getTermId(), term.getTermName(), term.getYear());
						//term.
						terms.put(atp, pTerm);
					...
					CourseInfo courseInfo = new CourseInfo(course.getCourseId(), course.getCourseCode(), course.getCourseName(), course.getCourseDescription(), backup);
					...
					else if (PlanConstants.LEARNING_PLAN_ITEM_TYPE_BACKUP.equals(planItem.getTypeKey())) {
            ...
            LOG.error("Unable to get plan", e);
		...
		Map<String, AdvisorInfo> advisorMap = initAdvisorData();
		form.setAdvisorMap(advisorMap);

		// If there's only one available advisor, go ahead and select it on the form
		...
		if (atps.size() > 1) {
			fullPlanDisplay = terms.get(atps.first()).getFullName() + " - " + terms.get(atps.last()).getFullName();
		}
		else if (atps.size() == 1) {
	...
	/**
	 * Create some mock course data.  This is really only used for development purposes
	 * @param pTerm
	 */
	private void initMockCourseData(PlanTermInfo pTerm) {
		CourseInfo courseInfo = new CourseInfo("courseId1", "COURSE-A 100", "Test course 1", "Description for course 1", false);
		pTerm.addPlannedCourse(courseInfo);
		courseInfo = new CourseInfo("courseId2", "COURSE-B 200", "Test course 2", "Description for course 2", false);
		pTerm.addPlannedCourse(courseInfo);
		courseInfo = new CourseInfo("courseId3", "COURSE-C 300", "Test course 3", "Description for course 3", false);
		pTerm.addPlannedCourse(courseInfo);
		courseInfo = new CourseInfo("courseId4", "COURSE-D 400", "Test course 4", "Description for course 4", false);
		pTerm.addPlannedCourse(courseInfo);
		
		courseInfo = new CourseInfo("courseId5", "COURSE-E 500", "Test course 5", "Description for course 5", true);
		pTerm.addBackupCourse(courseInfo);
		courseInfo = new CourseInfo("courseId6", "COURSE-F 600", "Test course 6", "Description for course 6", true);
		pTerm.addBackupCourse(courseInfo);
		courseInfo = new CourseInfo("courseId7", "COURSE-G 700", "Test course 7", "Description for course 7", true);
		pTerm.addBackupCourse(courseInfo);
		courseInfo = new CourseInfo("courseId8", "COURSE-H 800", "Test course 8", "Description for course 8", true);
		pTerm.addBackupCourse(courseInfo);
	}
	
	/**
	 * Add a term to an academic calendar
	 * @param pTerm term to add
	 * @param aYears List of academic calendars.
	 * @return
	 */
	private boolean addTermToAcademicCalendar(PlanTermInfo pTerm, List<AcademicYearInfo> aYears) {
		List<AcademicCalendarInfo> acals = new ArrayList<AcademicCalendarInfo>();
		try {
			acals = getAcademicCalendarService()
					.getAcademicCalendarsForTerm(pTerm.getTermId(), KsapFrameworkServiceLocator.getContext().getContextInfo());
		} catch (DoesNotExistException e){
            LOG.error("Unable to get academic calendar", e);
            return false;
        } catch (InvalidParameterException e){
            LOG.error("Unable to get academic calendar", e);
            return false;
		...
		for (AcademicCalendarInfo acal : acals) {
			...
			String displayYear = startYear == endYear ? String.valueOf(startYear) : startYear + "-" + endYear;
	...
	/**
	 * Look through the list of AcademicYearInfo objects and return the one with the matching id.
	 * @param aYearId
	 * @param aYears
	 * @return
	 */
	private AcademicYearInfo findAcademicYear(String aYearId, List<AcademicYearInfo> aYears) {
	 ...
	 * @return
	 */
	private Map<String, AdvisorInfo> initAdvisorData() {
		Map<String, AdvisorInfo> advisorMap = new HashMap<String, AdvisorInfo>();
		
		for (ConversationAdvisor advisor : getLearningPlanReviewStrategy().getAdvisors()) {
			advisorMap.put(advisor.getUniqueId(), (AdvisorInfo) advisor);
		}
	...
	private synchronized CourseDetailsInquiryHelperImpl getCourseDetailsInquiryService() {
		//TODO does this need to be synchronized?  It was where I copied it from but not sure if it shoul dbe here or not.
	...
	private AcademicCalendarService getAcademicCalendarService() {
		if (academicCalendarService == null) {
			academicCalendarService = KsapFrameworkServiceLocator.getAcademicCalendarService();
		}
