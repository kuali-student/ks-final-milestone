...
import org.kuali.student.ap.plan.review.form.ConversationListForm;
...
public class ConversationListController  extends ConversationControllerBase {
	...
	@Override
	protected ConversationListForm createInitialForm(HttpServletRequest request) {
	...
	private void initialize(ConversationListForm form) {
		int unreadCount = 0;
		int openCount = 0;
		...
		for (Conversation conversation : conversations) {
			...
			//addAdvisorToMap(advisorMap, convo.getAdvisorId(), convo.getAdvisingRole(), convo.getTypeKey());
		}
		...
		for (Entry<String, List<ConversationInfo>> entry : yearConversationMap.entrySet()) {
			...
			List<Conversation> tempConvos = new ArrayList<Conversation>(convos);
			cyi.setConversations(tempConvos);
