...
public class Browser extends Composite implements HasFocusLostCallbacks,
		HasValueChangeHandlers<String>, HasDataValue, TranslatableValueWidget {

	private VerticalPanel layout = new VerticalPanel();
	private BasicWidget basicWidget;
	private WidgetConfigInfo config;
	private SearchRpcServiceAsync searchRpcServiceAsync = GWT
			.create(SearchRpcService.class);

	public Browser(WidgetConfigInfo config) {
		this.config = config;
		init(config.lookupMeta, config.additionalLookups);
	}

	public Browser(LookupMetadata inLookupMetadata,
	...
	private void setupBasicSearch(LookupMetadata inLookupMetadata) {
		...
		if (inLookupMetadata.getWidget() == LookupMetadata.Widget.SUGGEST_BOX) {
			...
			// FIXME should we search on values to populate drop down here or
			// later when user will access the screen?
			if (config.canEdit) {
				...
				searchRpcServiceAsync.search(sr,
						new AsyncCallback<SearchResult>() {
							...
							public void onSuccess(SearchResult results) {
								if (results != null) {
									((KSDropDown) basicWidget.get())
											.setListItems(new SearchResultListItems(
													results.getRows()));
								} else {
									((KSDropDown) basicWidget.get())
											.setListItems(new SearchResultListItems(
								...
								// ((KSDropDown)basicWidget.get()).processOutstandingCallback();
	...
	private List<LookupMetadata> getLookupMetadataBasedOnWidget(
			List<LookupMetadata> additionalLookupMetadata,
	...
	private class BasicWidget extends Widget implements HasDataValue,
			HasFocusLostCallbacks, TranslatableValueWidget {
		...
		public void setResults(List<SelectedResults> results) {
			if (basicWidget instanceof KSTextBox) {
				((KSTextBox) basicWidget).setText(results.get(0)
						.getDisplayKey()); // FIXME: what about the result id?
			} else if (basicWidget.getClass().getName().contains(
					"ContainerWidget")) {
				List<String> selections = new ArrayList<String>();
				for (SelectedResults result : results) {
					selections.add(result.getDisplayKey()); // FIXME: what about
															// the result ids?
				}
				((SelectionContainerWidget) basicWidget)
						.setSelections(selections);
			} else if (basicWidget instanceof KSSuggestBox) {
				IdableSuggestion theSuggestion = new IdableSuggestion();
				theSuggestion.setReplacementString(results.get(0)
						.getDisplayKey());
			...
			if (basicWidget instanceof KSTextBox) {
				...
				if (value != null) {
					if (!config.isRepeating) {
						((KSSuggestBox) basicWidget).setValue((String) value
								.get(), fireEvents);
					} else {
						...
						((KSSuggestBox) basicWidget).setValue((String) v,
								fireEvents);
					}
				} else {
				...
				// make sure the drop down has data loaded before we set the
				// selected value
				ListItems searchResults = ((KSDropDown) basicWidget)
						.getListItems();
				if (searchResults != null) {
					...
					((KSDropDown) basicWidget)
							.addWidgetReadyCallback(new Callback<Widget>() {

								@Override
								public void exec(Widget widget) {
									final ListItems searchResults = ((KSSelectItemWidgetAbstract) widget)
											.getListItems();
		...
		public Value getValue() {
			if (basicWidget instanceof KSTextBox) {
				StringValue value = new StringValue(((KSTextBox) basicWidget)
						.getText());
				return value;
			} else if (basicWidget instanceof KSSuggestBox) {
				// Do check here
				if (!config.isRepeating) {
					StringValue value = new StringValue(
							((KSSuggestBox) basicWidget).getValue());
					return value;
				} else {
					Data data = new Data();
					data.set(new Data.IntegerKey(0),
							((KSSuggestBox) basicWidget).getValue());
					DataValue value = new DataValue(data);
					return value;
				}
			} else if (basicWidget instanceof KSDropDown) {
				StringValue value = new StringValue(((KSDropDown) basicWidget)
						.getSelectedItem());
				return value;
			}
		...
		public void addFocusLostCallback(final Callback<Boolean> callback) {
			if (basicWidget instanceof KSTextBox) {
				...
				((KSSuggestBox) basicWidget).getTextBox().addBlurHandler(
						new BlurHandler() {
	...
	private SearchRequest initializeSearchRequest(LookupMetadata lookup) {
		...
		// initialize search parameters that are hidden from the UI because they
		// are set to default context specific values
		for (LookupParamMetadata metaParam : lookup.getParams()) {
			if (metaParam.getWriteAccess() == WriteAccess.NEVER) {
				if ((metaParam.getDefaultValueString() == null || metaParam
						.getDefaultValueString().isEmpty())
						&& (metaParam.getDefaultValueList() == null || metaParam
								.getDefaultValueList().isEmpty())) {
					// FIXME throw an exception?
					GWT
							.log(
									"Key = "
											+ metaParam.getKey()
											+ " has write access NEVER but has no default value!",
									null);
					continue;
				}
