...
import org.kuali.student.lum.lu.assembly.data.client.refactorme.base.GradeInfoHelper.Properties;


public class GradeInfoMetadata
{
	...
	{
		int recurseLevel = increment (recursions, "GradeInfoMetadata");
		...
		// metadata for scaleKey
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.SCALE_KEY.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.STRING);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
			...
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("single"));
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("grade.scales"));
		}
		
		// metadata for rank
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.RANK.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.STRING);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
