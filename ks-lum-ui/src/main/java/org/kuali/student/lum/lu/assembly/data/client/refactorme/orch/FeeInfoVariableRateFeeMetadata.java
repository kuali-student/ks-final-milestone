...
import org.kuali.student.lum.lu.assembly.data.client.refactorme.orch.FeeInfoVariableRateFeeHelper.Properties;


public class FeeInfoVariableRateFeeMetadata
{
	...
	public void loadChildMetadata (Metadata mainMeta, String type, String state,  RecursionCounter recursions)
	{
		...
		childMeta.setDataType (Data.DataType.STRING);
		childMeta.setWriteAccess (Metadata.WriteAccess.NEVER);
		childMeta.setDefaultValue (new Data.StringValue ("Variable Rate"));
		if (this.matches (type, state, "(default)", "(default)"))
		{
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("single"));
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("required"));
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("hard.coded.ratetype.variablerate"));
		...
		// metadata for MinAmount
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.MIN_AMOUNT.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.STRING);
		...
		// metadata for MaxAmount
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.MAX_AMOUNT.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.STRING);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
