...
public class CluInformationHelper {
    ...
    public CluInformation getCluInfoForCode(String code) {
        try {
            QueryByCriteria.Builder qbcBuilder = QueryByCriteria.Builder.create();
            qbcBuilder.setPredicates(PredicateFactory.equal("officialIdentifier.code", code),
                    PredicateFactory.in("state", getApprovedStateSearchParam().getValues().toArray()),
                    PredicateFactory.equal("luType.id", "kuali.lu.type.CreditCourse"));
