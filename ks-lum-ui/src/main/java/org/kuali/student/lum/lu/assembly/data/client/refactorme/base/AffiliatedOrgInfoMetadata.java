...
import org.kuali.student.lum.lu.assembly.data.client.refactorme.base.AffiliatedOrgInfoHelper.Properties;


public class AffiliatedOrgInfoMetadata
{
	...
	{
		int recurseLevel = increment (recursions, "AffiliatedOrgInfoMetadata");
		...
		if (this.matches (type, state, "(default)", "(default)"))
		{
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("required"));
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("single"));
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("date"));
		}
		...
		// metadata for percentage
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.PERCENTAGE.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.LONG);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("optional"));
