...
import org.kuali.student.common.ui.client.widgets.search.LinkPanel;
import org.kuali.student.common.ui.client.widgets.search.SelectedResults;
import org.kuali.student.common.ui.client.widgets.search.SwappablePanel;
...
public class BrowsePanel extends Composite
{
 ...
 private KSLabel enteredCriteriaHeading =
  new KSLabel (getMessage ("searchPanelCriteria"));
 ...
 public BrowsePanel (LookupMetadata meta)
 {
  lookups.add (meta);
  setupSearch ();
 ...
 public BrowsePanel (List<LookupMetadata> metas)
 {
  lookups = metas;
  setupSearch ();
  this.initWidget (layout);
 ...
 private Widget createSearchParamPanel (LookupMetadata meta)
 {
  ...
  LinkPanel panel =
   new LinkPanel (SearchStyle.ADVANCED, new AdvancedSearch (meta));
  ...
  {
   if ((metaParam.getUsage () == Usage.CUSTOM) || (metaParam.getUsage ()
    == Usage.ADVANCED_CUSTOM))
   {
    KSLinkButton button =
     panel.addLinkToPanel (SearchStyle.ADVANCED, getMessage ("searchPanelCustomizeSearch"), SearchStyle.CUSTOM);
    ...
    button =
     panel.addLinkToPanel (SearchStyle.CUSTOM, getMessage ("searchPanelReturnToAdvancedSearch"), SearchStyle.ADVANCED);
    button.addClickHandler (new ClickHandler ()
    {
 ...
 private class CustomizedSearch extends Composite
 {
  ...
  public CustomizedSearch (final LookupMetadata meta,
                           final ParamListItems listItems)
  {
   ...
   KSLinkButton addCriteria =
    new KSLinkButton (getMessage ("searchPanelAddCriteria"), ButtonStyle.SECONDARY);
   ...
   KSLinkButton searchButton =
    new KSLinkButton (getMessage ("search"), ButtonStyle.PRIMARY);
   ...
   searchButton.addClickHandler (new ClickHandler ()
   {
    ...
    public void onClick (ClickEvent event)
    {
     ...
     for (LookupParamMetadata metaParam : meta.getParams ())
     {
      ...
      else if (metaParam.getWriteAccess () == WriteAccess.WHEN_NULL)
      {
       if ((metaParam.getDefaultValueString () != null &&  ! metaParam.
        getDefaultValueString ().isEmpty ()) || (metaParam.getDefaultValueList ()
        != null &&  ! metaParam.getDefaultValueList ().isEmpty ()))
       {
        SearchParam param = new SearchParam ();
        param.setKey (metaParam.getKey ());
        if (metaParam.getDefaultValueList () == null)
     ...
     if ( ! resultsShown)
     {
      ...
      modifySearchPanel =
       new CollapsablePanel (getMessage ("searchPanelModifySearch"), searchSelectorPanel, false);
      ...
      BrowsePanel.this.layout.insert (modifySearchPanel, 0);
     }
     else
     {
 ...
 private class CustomLine extends Composite implements HasSearchParam
 {
  ...
  public CustomLine (LookupMetadata meta, final ParamListItems listItems)
  {
   ...
   paramSelector.addSelectionChangeHandler (new SelectionChangeHandler ()
   {
    ...
    {
     String id = ((KSSelectItemWidgetAbstract) event.getWidget ()).
      getSelectedItem ();
     widget = listItems.getWidget (id);
     widgetPanel.setWidget (widget);
     key = id;
  ...
  public SearchParam getSearchParam ()
  {
   return BrowsePanel.getSearchParam (widget, key);
  }
 ...
 private class AdvancedSearch extends Composite
 {
  ...
  public AdvancedSearch (final LookupMetadata meta)
  {
   ...
   for (LookupParamMetadata param : meta.getParams ())
   {
    ...
    if ((param.getUsage () != Usage.ADVANCED) && (param.getUsage ()
     != Usage.ADVANCED_CUSTOM))
    {
     continue;
    }

    if ((param.getWriteAccess () == WriteAccess.ALWAYS) || (param.getWriteAccess ()
    ...
    {
     if (param.getDefaultValueString () == null && param.getDefaultValueList ()
      == null)
     {
      SearchField paramField = new SearchField (param);
      searchFields.add (paramField);
   ...
   searchButton.addClickHandler (new ClickHandler ()
   {
    ...
    public void onClick (ClickEvent event)
    {
     ...
     for (SearchField field : searchFields)
     {
      ...
      if ((param.getValue () != null) && ((param.getValue ().toString ().trim ().
       isEmpty () == false) || (param.getKey ().toLowerCase ().indexOf ("optional")
       == -1)))
      {
       params.add (param);
       userCriteria.add (field);
     ...
     for (LookupParamMetadata metaParam : meta.getParams ())
     {
      if (metaParam.getWriteAccess () == WriteAccess.NEVER)
      {
        ...
        == null || metaParam.getDefaultValueList ().isEmpty ()))
       {
        ...
        GWT.log ("Key = " + metaParam.getKey ()
         + " has write access NEVER but has no default value!", null);
        continue;
       }
       SearchParam param = new SearchParam ();
       param.setKey (metaParam.getKey ());
 ...
 private class SearchField extends Composite implements HasSearchParam
 {
  ...
  public SearchParam getSearchParam ()
  {
   return BrowsePanel.getSearchParam (widget, meta.getKey ());
  }
 ...
 public List<SelectedResults> getSelectedValues ()
 {
  ...
  if (table != null)
  {
   ...
   {
    String displayKey = row.getValue (currentSearchLookupMetadata.
     getResultDisplayKey ());
    String returnKey = row.getValue (currentSearchLookupMetadata.
     getResultReturnKey ());
