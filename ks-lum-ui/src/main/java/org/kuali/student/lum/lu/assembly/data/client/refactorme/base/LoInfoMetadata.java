...
import org.kuali.student.lum.lu.assembly.data.client.refactorme.base.LoInfoHelper.Properties;


public class LoInfoMetadata
{
	...
	{
		int recurseLevel = increment (recursions, "LoInfoMetadata");
		...
		// metadata for loRepositoryKey
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.LO_REPOSITORY_KEY.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.STRING);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
