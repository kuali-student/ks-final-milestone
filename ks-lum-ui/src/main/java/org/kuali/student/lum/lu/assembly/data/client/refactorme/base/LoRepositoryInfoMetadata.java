...
import org.kuali.student.lum.lu.assembly.data.client.refactorme.base.LoRepositoryInfoHelper.Properties;


public class LoRepositoryInfoMetadata
{
	...
	{
		int recurseLevel = increment (recursions, "LoRepositoryInfoMetadata");
		...
		// metadata for rootLoId
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.ROOT_LO_ID.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.STRING);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
