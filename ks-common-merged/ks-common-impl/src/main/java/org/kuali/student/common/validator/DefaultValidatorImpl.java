...
import org.kuali.student.common.olddictionary.dto.MustOccurConstraint;
...
//                    beanPathStack.push(field.isDynamic()?"attributes("+field.getName()+")":field.getName());
                ...
                if (field.getMinOccurs() != null && field.getMinOccurs() > ((Collection<?>) value).size()) {
                    ...
                    valRes.setError(MessageUtils.interpolate(getMessage("validation.minOccurs", context), toMap(field)));
                    results.add(valRes);
                }
                ...
                if (maxOccurs != null && maxOccurs < ((Collection<?>) value).size()) {
                    ValidationResultInfo valRes = new ValidationResultInfo(xPathForCollection, value);
                    valRes.setError(MessageUtils.interpolate(getMessage("validation.maxOccurs", context), toMap(field)));
                    results.add(valRes);
                }
            } else {
                if (null != value) {
                    ...
                    if (field.getMinOccurs() != null && field.getMinOccurs() > 0) {
                        ...
                        val.setError(getMessage("validation.required", context));
                        results.add(val);
                    }
                }
            }
...
//                    beanPathStack.push(field.isDynamic()?"attributes("+field.getName()+")":field.getName());
    ...
    protected ValidationResultInfo processRequireConstraint(String element, RequiredConstraint constraint, FieldDefinition field, ObjectStructureDefinition objStructure, ConstraintDataProvider dataProvider, ContextInfo context) {
        ...
        if (!result) {
            ...
            val.setError(MessageUtils.interpolate(getMessage("validation.requiresField", context), rMap));
        }
        ...
        if ("regex".equalsIgnoreCase(processorType)) {
            if (fieldValue == null || !fieldValue.toString().matches(validChars)) {
                ...
                if(vcConstraint.getLabelKey()!=null){
                	val.setError(getMessage(vcConstraint.getLabelKey(), context));
                }else{
                	val.setError(getMessage("validation.validCharsFailed", context));
                }
            }
        }
    ...
    protected ValidationResultInfo processOccursConstraint(String element, MustOccurConstraint constraint, FieldDefinition field, ObjectStructureDefinition objStructure, ConstraintDataProvider dataProvider, ContextInfo context) {
        ...
        if (!result) {
         // TODO: figure out what data should go here instead of null
            ...
            val.setError(getMessage("validation.occurs", context));
        }
    ...
    protected void processLookupConstraint(List<ValidationResultInfo> valResults, LookupConstraint lookupConstraint, FieldDefinition field, Stack<String> elementStack, ConstraintDataProvider dataProvider, ContextInfo context) {
        ...
        if (searchResult == null || searchResult.getRows() == null || searchResult.getRows().isEmpty()) {
            ...
            val.setError(getMessage("validation.lookup", context));
            valResults.add(val);
        }
    }
    ...
    protected void validateBoolean(Object value, Constraint constraint, String element, List<ValidationResultInfo> results, ContextInfo context) {
        if (!(value instanceof Boolean)) {
            try {
                ...
                val.setError(getMessage("validation.mustBeBoolean", context));
                results.add(val);
            }
        }
    }
    ...
    protected void validateDouble(Object value, Constraint constraint, String element, List<ValidationResultInfo> results, ContextInfo context) {
        ...
        if (value instanceof Number) {
            ...
            try {
                ...
                val.setError(getMessage("validation.mustBeDouble", context));
            }
        }
        ...
        if (val.isOk()) {
            ...
            if (maxValue != null && minValue != null) {
                ...
                if (v > maxValue || v < minValue) {
                    val.setError(MessageUtils.interpolate(getMessage("validation.outOfRange", context), toMap(constraint)));
                }
            } else if (maxValue != null) {
                if (v > maxValue) {
                    val.setError(MessageUtils.interpolate(getMessage("validation.maxValueFailed", context), toMap(constraint)));
                }
            } else if (minValue != null) {
                if (v < minValue) {
                    val.setError(MessageUtils.interpolate(getMessage("validation.minValueFailed", context), toMap(constraint)));
                }
            }
        }
    ...
    protected void validateFloat(Object value, Constraint constraint, String element, List<ValidationResultInfo> results, ContextInfo context) {
        ...
        if (value instanceof Number) {
            ...
            try {
                ...
                val.setError(getMessage("validation.mustBeFloat", context));
            }
        }
    ...
    protected void validateLong(Object value, Constraint constraint, String element, List<ValidationResultInfo> results, ContextInfo context) {
        ...
        if (value instanceof Number) {
            ...
            try {
                ...
                val.setError(getMessage("validation.mustBeLong", context));
            }
        }
    ...
    protected void validateInteger(Object value, Constraint constraint, String element, List<ValidationResultInfo> results, ContextInfo context) {
        ...
        if (value instanceof Number) {
            ...
            try {
                ...
                val.setError(getMessage("validation.mustBeInteger", context));
            }
        }
    ...
    protected void validateDate(Object value, Constraint constraint, String element, List<ValidationResultInfo> results, DateParser dateParser, ContextInfo context) {
        ...
        if (value instanceof Date) {
            ...
            try {
                ...
                val.setError(getMessage("validation.mustBeDate", context));
            }
        }
    ...
    protected void validateString(Object value, Constraint constraint, String element, List<ValidationResultInfo> results, ContextInfo context) {
        ...
        if (maxLength != null && constraint.getMinLength() != null && constraint.getMinLength() > 0) {
            if (s.length() > maxLength || s.length() < constraint.getMinLength()) {
                val.setError(MessageUtils.interpolate(getMessage("validation.lengthOutOfRange", context), toMap(constraint)));
            }
        } else if (maxLength != null) {
            if (s.length() > Integer.parseInt(constraint.getMaxLength())) {
                val.setError(MessageUtils.interpolate(getMessage("validation.maxLengthFailed", context), toMap(constraint)));
            }
        } else if (constraint.getMinLength() != null && constraint.getMinLength() > 0) {
            if (s.length() < constraint.getMinLength()) {
                val.setError(MessageUtils.interpolate(getMessage("validation.minLengthFailed", context), toMap(constraint)));
            }
        }
    ...
    protected String getMessage(String messageId, ContextInfo context) {
