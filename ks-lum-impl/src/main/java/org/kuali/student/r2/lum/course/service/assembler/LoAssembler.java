...
public class LoAssembler implements BOAssembler<LoDisplayInfo, LoInfo> {
    ...
    public BaseDTOAssemblyNode<LoDisplayInfo, LoInfo> disassemble(
        ...
        //Process the LoCategoryRelation
        try {
            List<BaseDTOAssemblyNode<?, ?>> loCategoryRelationNodes = disassembleLoCategoryRelation(loDisplay, operation, contextInfo);
            result.getChildNodes().addAll(loCategoryRelationNodes);
        } catch (Exception e) {
            throw new AssemblyException("Error disassembling categories", e);
        }
        return result;
    ...
    private List<BaseDTOAssemblyNode<?, ?>> disassembleLoCategoryRelation(
    ...
    private List<BaseDTOAssemblyNode<?, ?>> disassembleCategories(
        ...
        //Category relations
        Set<LoCategoryInfo> currentCategories = new HashSet<LoCategoryInfo>();
        //Get current relations
        if (!NodeOperation.CREATE.equals(operation)) {
            try {
                List<LoCategoryInfo> categories = loService.getLoCategoriesByLo(loDisplay.getLoInfo().getId(), contextInfo);
        ...
        for (LoCategoryInfo category : loDisplay.getLoCategoryInfoList()) {
            ...
            // If this is a format create/new activity update then all activities will be created
            if (NodeOperation.CREATE == operation
                    || (NodeOperation.UPDATE == operation && !currentCategories.contains(category.getId()))) {

                category.setId(UUIDHelper.genStringUUID(category.getId()));
                category.setLoRepositoryKey(CourseAssemblerConstants.COURSE_LO_REPOSITORY_KEY);
                category.setStateKey("Active");

                // This should be removed once typeKey comes with value from UI. KSCM-1660
                if (category.getTypeKey() == null) {
                    category.setTypeKey("loCategoryType.accreditation");
                }
                BaseDTOAssemblyNode<LoDisplayInfo, LoCategoryInfo> categoryInfoNode = new BaseDTOAssemblyNode<LoDisplayInfo, LoCategoryInfo>(null);

                categoryInfoNode.setNodeData(category);
                categoryInfoNode.setOperation(NodeOperation.CREATE);
                results.add(categoryInfoNode);

                currentCategories.remove(category.getId());

            } else if (NodeOperation.UPDATE == operation
                    && currentCategories.contains(category.getId())) {
                currentCategories.remove(category.getId());
            }
        }
        //Delete leftovers
        for (LoCategoryInfo category : currentCategories) {

            BaseDTOAssemblyNode<LoDisplayInfo, LoCategoryInfo> categoryInfoDeleteNode = new BaseDTOAssemblyNode<LoDisplayInfo, LoCategoryInfo>(null);
            categoryInfoDeleteNode.setNodeData(category);
            categoryInfoDeleteNode.setOperation(NodeOperation.DELETE);
            results.add(categoryInfoDeleteNode);
        }
