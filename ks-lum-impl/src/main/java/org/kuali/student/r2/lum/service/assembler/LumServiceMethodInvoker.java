...
public class LumServiceMethodInvoker implements BusinessServiceMethodInvoker {
    ...
    protected void invokeServiceCallOnResult(BaseDTOAssemblyNode results, ContextInfo contextInfo)
        ...
        } else if (nodeData instanceof LoCategoryInfo) {
            LoCategoryInfo loCategoryInfo = (LoCategoryInfo) nodeData;
            switch (results.getOperation()) {
                case CREATE:
                    LoCategoryInfo createdLoCategory = loService.createLoCategory(loCategoryInfo.getTypeKey(), loCategoryInfo, contextInfo);
                    if (null != results.getBusinessDTORef()) {
                        results.getAssembler().assemble(createdLoCategory, results.getBusinessDTORef(), true, contextInfo);
                    }
                    break;
                case UPDATE:
                    LoCategoryInfo updatedLoCategory = loService.updateLoCategory(loCategoryInfo.getTypeKey(), loCategoryInfo, contextInfo);
                    if (null != results.getBusinessDTORef()) {
                        results.getAssembler().assemble(updatedLoCategory, results.getBusinessDTORef(), true, contextInfo);
                    }
                    break;
                case DELETE:
                    loService.deleteLoCategory(loCategoryInfo.getId(),contextInfo);
                    break;
            }
