...
public class LumServiceMethodInvoker implements BusinessServiceMethodInvoker {
	...
	@Override
	public void invokeServiceCalls_KRAD(BaseDTOAssemblyNode results,
			ContextInfo contextInfo) throws AlreadyExistsException,
			DataValidationErrorException, DoesNotExistException,
			InvalidParameterException, MissingParameterException,
			OperationFailedException, PermissionDeniedException,
			VersionMismatchException, DependentObjectsExistException,
			CircularRelationshipException, AssemblyException,
			UnsupportedActionException, UnsupportedOperationException,
			CircularReferenceException, ReadOnlyException {
		// For Delete operation process the tree from bottom up
	    if(NodeOperation.DELETE == results.getOperation()) {
            for(BaseDTOAssemblyNode childNode: (List<BaseDTOAssemblyNode>) results.getChildNodes()){
                invokeServiceCalls_KRAD(childNode,contextInfo);
            }
	    }

	    invokeServiceCallOnResult_KRAD(results, contextInfo);
	 ...
	 * @throws org.kuali.student.r2.common.exceptions.AlreadyExistsException
	 */
	protected void invokeServiceCallOnResult_KRAD(BaseDTOAssemblyNode results, ContextInfo contextInfo)
			throws AlreadyExistsException, DataValidationErrorException,
			DoesNotExistException, InvalidParameterException,
			MissingParameterException, OperationFailedException,
			PermissionDeniedException, AssemblyException,
		...
		if(nodeData instanceof CluInfo){
			...
			switch(results.getOperation()){
			case CREATE:
				CluInfo newClu = cluService.createClu_KRAD(clu.getTypeKey(), clu, contextInfo);
				if(results.getAssembler() != null) {
					results.getAssembler().assemble(newClu, results.getBusinessDTORef(), true, contextInfo);
				}
				break;
			case UPDATE:
				CluInfo updatedClu = cluService.updateClu_KRAD(clu.getId(), clu, contextInfo);
				if(results.getAssembler() != null) {
					results.getAssembler().assemble(updatedClu, results.getBusinessDTORef(), true, contextInfo);
				}
				break;
			...
			switch(results.getOperation()){
			case CREATE:
				CluCluRelationInfo newCluRel = cluService.createCluCluRelation_KRAD(relation.getCluId(), relation.getRelatedCluId(), relation.getTypeKey(), relation, contextInfo);
				// Update the businessDTO if one exists for the cluclurelation (for e.g. CourseJointInfo)
				if(null != results.getBusinessDTORef()) {
					results.getAssembler().assemble(newCluRel, results.getBusinessDTORef(), true, contextInfo);
				}
			...
			switch(results.getOperation()){
			case CREATE:
				cluService.createCluResult_KRAD(cluResult.getCluId(), cluResult.getTypeKey(), cluResult, contextInfo);
				break;
			case UPDATE:
				cluService.updateCluResult_KRAD(cluResult.getId(), cluResult, contextInfo);
				break;
			case DELETE:
				cluService.deleteCluResult(cluResult.getId(), contextInfo);
				break;
