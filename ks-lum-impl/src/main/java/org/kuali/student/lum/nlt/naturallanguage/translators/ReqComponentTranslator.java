...
public class ReqComponentTranslator {
    ...
    private ContextRegistry<Context<CustomReqComponentInfo>> contextRegistry;
     ...
     */
    public void setContextRegistry(final ContextRegistry<Context<CustomReqComponentInfo>> contextRegistry) {
     ...
     *             Translation fails
     */
    public String translate(final CustomReqComponentInfo reqComponent, final String nlUsageTypeKey) throws DoesNotExistException, OperationFailedException {
    	if(reqComponent == null) {
    		throw new DoesNotExistException("ReqComponent cannot be null");
    	}
    	
    	ReqComponentTypeInfo reqComponentType = reqComponent.getRequiredComponentType();

        Map<String, Object> contextMap = buildContextMap(reqComponent);

        ReqComponentTypeNLTemplateInfo template = getTemplate(reqComponentType, nlUsageTypeKey);
     ...
     * @throws DoesNotExistException
     */
    private Map<String, Object> buildContextMap(CustomReqComponentInfo reqComponent) throws DoesNotExistException, OperationFailedException {
    	String reqComponentTypeId = reqComponent.getRequiredComponentType().getId();
        Context<CustomReqComponentInfo> context = this.contextRegistry.get(reqComponentTypeId);
        if(context == null) {
     ...
     * @throws DoesNotExistException
     */
    private ReqComponentTypeNLTemplateInfo getTemplate(ReqComponentTypeInfo reqComponentType, String nlUsageTypeKey) throws DoesNotExistException {
        List<ReqComponentTypeNLTemplateInfo> templateList = reqComponentType.getNlUsageTemplates();
        for (ReqComponentTypeNLTemplateInfo template : templateList) {
