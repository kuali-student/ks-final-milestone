...
public class SamlTokenCxfOutInterceptor extends AbstractWSS4JInterceptor {
	...
	// KS : added to hold information used to create authenticated user, SAML
	// Assertion
	// KS : made this a ThreadLocal since we don't know if interceptors are
	// thread-safe.
	...
	final class SamlTokenCxfOutInterceptorInternal implements PhaseInterceptor<SoapMessage> {
		...
		public void handleMessage(SoapMessage mc) throws Fault {
			// KS : added so we can create the authenticated user SAML Assertion
			...
			// KS : this has been the problem and why we needed to code our own
			// WSS4JOutInterceptor.
			// WSS4JOutInterceptor needed to expose RequestData out of here so
			// you can add the signature parts
			// see code below, before doSenderAction(); Essentially anything
			// signature parts, gets singed by wss4j.
			...
			try {
				 ...
				 * into a Document. This forces CXF to serialize the SOAP
				 * request into FORM_STRING. This string is converted into a
				 * document. During the FORM_STRING serialization CXF performs
				 * multi-ref of complex data types (if requested), generates and
				 * inserts references for attachements and so on. The resulting
				 * Document MUST be the complete and final SOAP request as CXF
				 * would send it over the wire. Therefore this must shall be the
				 * last (or only) handler in a chain. Now we can perform our
				 * security operations on this request.
				 */
				...
				// KS : any element represented by an encryptionPart and added
				// to signatureParts will be signed.
				// The wsuId is the SAML Assertion we created above in
				// handleMessageUserSAML(mc).
		...
		// KS : All three methods below added to create the authenticated user
		// SAML Assertion
