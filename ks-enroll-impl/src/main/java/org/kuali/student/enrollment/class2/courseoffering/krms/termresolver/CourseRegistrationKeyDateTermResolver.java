...
public class CourseRegistrationKeyDateTermResolver implements TermResolver<Boolean> {
    ...
    public Boolean resolve(Map<String, Object> resolvedPrereqs, Map<String, String> parameters) throws TermResolutionException {
        ...
        try {
            CourseOfferingInfo coInfo = getCourseOfferingService().getCourseOffering(regGroupInfo.getCourseOfferingId(), context);
            List<ActivityOfferingInfo> aoInfos = getCourseOfferingService().getActivityOfferingsByIds(regGroupInfo.getActivityOfferingIds(), context);
            String coTermId = coInfo.getTermId();
            String termId = aoInfos.get(0).getTermId();
            for (ActivityOfferingInfo aoInfo : aoInfos) {
                if (!StringUtils.equals(termId, aoInfo.getTermId())) {
                    termId = coTermId;
                    break;
                }
            }
