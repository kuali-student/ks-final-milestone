...
public class TestCourseOfferingServiceImpl {
    ...
    @Test
    public void testChangeActivityOfferingStateToOffered() throws Exception{
        new MockLrcTestDataLoader(this.lrcService).loadData();
        dataLoader.createStateTestData();
        dataLoader.loadTerms();
        ...
        try {
            ...
            StatusInfo scStatus = coService.changeActivityOfferingState(created.getActivityId(),LuiServiceConstants.LUI_AO_STATE_OFFERED_KEY,callContext);

            assertTrue(scStatus.getIsSuccess());

            ActivityOfferingInfo retrieved = coService.getActivityOffering(
                    created.getId(), callContext);
            assertNotNull(retrieved);
            assertEquals(LuiServiceConstants.LUI_AO_STATE_OFFERED_KEY,
                    retrieved.getStateKey());
    ...
    @Test
    public void testChangeActivityOfferingStateToCancelled() throws Exception{
        new MockLrcTestDataLoader(this.lrcService).loadData();
        dataLoader.createStateTestData();
        dataLoader.loadTerms();
        ...
        try {
            ...
            StatusInfo scStatus = coService.changeActivityOfferingState(created.getActivityId(),LuiServiceConstants.LUI_AO_STATE_CANCELED_KEY,callContext);
            ...
            ActivityOfferingInfo retrieved = coService.getActivityOffering(
                    created.getId(), callContext);
            assertNotNull(retrieved);
            assertEquals(LuiServiceConstants.LUI_AO_STATE_CANCELED_KEY,
                    retrieved.getStateKey());
