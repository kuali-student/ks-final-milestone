...
@ContextConfiguration(locations = {"classpath:co-decorator-cache-test-context.xml"})
@TransactionConfiguration(transactionManager = "JtaTxManager", defaultRollback = true)
@Transactional
public class TestCourseOfferingServiceCacheDecorator{
    ...
    private CourseInfo buildCanonicalCourse(String id, String startTermId, String subjectArea, String code, String title, String description) {
        ...
        info.setEffectiveDate(calcEffectiveDateForTerm(startTermId));
        info.setId(id);
    ...
    private Date calcEffectiveDateForTerm(String termId) {
        ...
        return str2Date(year + "-" + mmdd + " 00:00:00.0");
    }
    ...
    private String testCreateCourseOffering() throws Exception {
        ...
        CourseOfferingInfo retrieved = courseOfferingService.getCourseOffering(
        ...
        List<CourseOfferingInfo> offerings = courseOfferingService.getCourseOfferingsByCourse("CLU-1", callContext);
    ...
    private CourseOfferingInfo createCourseOffering() throws Exception {
        ...
        CourseOfferingInfo created = courseOfferingService.createCourseOffering("CLU-1",
    ...
    private void testUpdateCourseOffering(String coId) throws Exception {
        CourseOfferingInfo coi = courseOfferingService.getCourseOffering(coId,
                callContext);
        assertNotNull(coi);
        ...
        CourseOfferingInfo updated = courseOfferingService.updateCourseOffering(coId,
                coi, callContext);
        assertNotNull(updated);

        CourseOfferingInfo retrieved = courseOfferingService.getCourseOffering(coId,
                callContext);
        assertNotNull(retrieved);
    ...
    private void testSearchForCourseOfferings() throws Exception {
        ...
        List<CourseOfferingInfo> coList = courseOfferingService
    ...
    private void testDeleteCourseOffering(String coId) throws Exception {
        ...
        // a success
        StatusInfo delResult = courseOfferingService.deleteCourseOffering(coId, callContext);
        assertTrue(delResult.getIsSuccess());

        try {
            courseOfferingService.getCourseOffering(coId, callContext);
            fail("DoesNotExistException should have been thrown");
        } catch (DoesNotExistException ex) {
            assertNotNull(ex.getMessage());
            assertEquals(coId, ex.getMessage());
        }
    ...
    public void testCRFormatOffering() throws Exception {
        ...
        FormatOfferingInfo retrieved = courseOfferingService.getFormatOffering(created.getId(), callContext);
    ...
    private FormatOfferingInfo createFormatOffering(String coId, String coTermId) throws Exception {
        ...
        return courseOfferingService.createFormatOffering(coId,
    ...
    private ActivityOfferingInfo createActivityOffering(CourseOfferingInfo courseOffering, String foId) throws Exception {
        ...
        return courseOfferingService.createActivityOffering(
                foId, activityId,
    ...
    public void testCRActivityOffering() throws Exception {
        ...
        ActivityOfferingInfo retrieved = courseOfferingService.getActivityOffering(
                created.getId(), callContext);
        assertNotNull(retrieved);
        ...
        List<ActivityOfferingInfo> activities = courseOfferingService
        ...
        ActivityOffering createdAo = courseOfferingService.getActivityOffering(retrieved.getId(), callContext);
    ...
    public void testDeleteCourseOfferingCascaded() throws Exception {
        ...
        List<FormatOfferingInfo> oformats = courseOfferingService.getFormatOfferingsByCourseOffering(courseOffering.getId(), callContext);
        assertEquals(1, oformats.size());

        List<ActivityOfferingInfo> oactivities = courseOfferingService.getActivityOfferingsByCourseOffering(courseOffering.getId(), callContext);
        assertEquals(1, oactivities.size());

        StatusInfo status = courseOfferingService.deleteCourseOfferingCascaded(courseOffering.getId(), callContext);
        assertTrue(status.getIsSuccess());

        List<FormatOfferingInfo> formats = courseOfferingService.getFormatOfferingsByCourseOffering(courseOffering.getId(), callContext);
        assertTrue(formats.isEmpty());

        List<ActivityOfferingInfo> activities = courseOfferingService.getActivityOfferingsByCourseOffering(courseOffering.getId(), callContext);
        assertTrue(activities.isEmpty());

        try {
            courseOfferingService.getCourseOffering(courseOffering.getId(), callContext);
            fail("DoesNotExistException should have been thrown");
        } catch (DoesNotExistException ex) {
            assertNotNull(ex.getMessage());
            assertEquals(courseOffering.getId(), ex.getMessage());
        }
    }
    ...
    public void testGetActivityOfferingsForSeatPoolDefinition() throws Exception {
        ...
        SeatPoolDefinitionInfo seatPoolCreated = courseOfferingService.createSeatPoolDefinition(seatPoolDefinitionInfo, callContext);
        ...
        // Add Seatpool to AO
        courseOfferingService.addSeatPoolDefinitionToActivityOffering(seatPoolCreated.getId(), activityOfferingCreated.getId(), callContext);

        //  Actual test
        List<ActivityOfferingInfo> activityOfferingInfos = courseOfferingService.getActivityOfferingsForSeatPoolDefinition(seatPoolCreated.getId(), callContext);
    ...
    public void testActivityOfferingClusters() throws Exception {
        ...
        ActivityOfferingInfo activities[] = new ActivityOfferingInfo[]{
                courseOfferingService.getActivityOffering("CO-1:LEC-AND-LAB:LEC-A",
                        callContext),
                courseOfferingService.getActivityOffering("CO-1:LEC-AND-LAB:LAB-A",
                        callContext),
                courseOfferingService.getActivityOffering("CO-1:LEC-AND-LAB:LAB-B",
                        callContext),
                courseOfferingService.getActivityOffering("CO-1:LEC-AND-LAB:LAB-C",
                        callContext),};
        ...
        ActivityOfferingClusterInfo actual = courseOfferingService.createActivityOfferingCluster("CO-1:LEC-AND-LAB", CourseOfferingServiceConstants.AOC_ROOT_TYPE_KEY, expected, callContext);
        ...
        List<RegistrationGroupInfo> rgList = courseOfferingService.getRegistrationGroupsByActivityOfferingCluster(actual.getId(), callContext);

        assertTrue(rgList.isEmpty());

        courseOfferingService.generateRegistrationGroupsForCluster(actual.getId(), callContext);

        rgList = courseOfferingService.getRegistrationGroupsByActivityOfferingCluster(actual.getId(), callContext);
