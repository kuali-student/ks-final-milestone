 ...
 * @author nwright
 */
public class EqualsValueCheckProposition extends AbstractValueCheckProposition {

    public EqualsValueCheckProposition(InstructionInfo instruction, CheckInfo check) {
        super(instruction, check);
    }

    @Override
    protected String getExemptionTypeToLookFor() {
        return ExemptionServiceConstants.CHECK_VALUE_EQUALS_EXEMPTION_TYPE_KEY;
    }

    @Override
    protected boolean matches(KualiDecimal leftComparisonValue, KualiDecimal rightComparisonValue) {
        if (leftComparisonValue.isGreaterThan(rightComparisonValue)) {
            return false;
        }
