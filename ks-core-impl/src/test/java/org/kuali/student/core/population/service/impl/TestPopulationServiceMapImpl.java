...
public class TestPopulationServiceMapImpl {
    ...
    public void before() {
        ...
        while (true) {
            ...
            if (service instanceof PopulationServiceDecorator) {
                PopulationServiceDecorator decorator = (PopulationServiceDecorator) service;
                service = decorator.getNextDecorator();
            }
        }
    }
    ...
    public void testGetPopulationRuleForPopulation() throws Exception {
        ...
        PopulationInfo refCreated = populationService.createPopulation(popList.get(0).getTypeKey(), popList.get(0),
                contextInfo);
        PopulationInfo threeCreated = populationService.createPopulation(popList.get(1).getTypeKey(), popList.get(1),
                contextInfo);
        PopulationInfo fourCreated = populationService.createPopulation(popList.get(2).getTypeKey(), popList.get(2),
                contextInfo);
        PopulationRuleInfo ruleInfo = _constructPopulationRuleInfoByType(
                PopulationServiceConstants.POPULATION_RULE_TYPE_UNION_KEY, 4);
        ...
        PopulationRuleInfo ruleInfoCreated = populationService.createPopulationRule(ruleInfo.getTypeKey(), ruleInfo,
                contextInfo);
    }
    ...
    public void testGetPopulationsForPopulationRule() throws Exception {
        List<PopulationInfo> popList = _constructPopulationList();
        // Create ref population and child population IDs
        PopulationInfo twoCreated = populationService.createPopulation(popList.get(0).getTypeKey(), popList.get(0),
                contextInfo);
        PopulationInfo threeCreated = populationService.createPopulation(popList.get(1).getTypeKey(), popList.get(1),
                contextInfo);
        PopulationInfo fourCreated = populationService.createPopulation(popList.get(2).getTypeKey(), popList.get(2),
        ...
        // Now the pop rule
        PopulationRuleInfo ruleInfo = _constructPopulationRuleInfoByType(
                PopulationServiceConstants.POPULATION_RULE_TYPE_UNION_KEY, 2);
    ...
    public void testGetPopulationRuleIdsByType() throws Exception {
        ...
        ruleInfos.add(two);
        PopulationRuleInfo three = _constructPopulationRuleInfoByType(
                PopulationServiceConstants.POPULATION_RULE_TYPE_INTERSECTION_KEY, 3);
        ruleInfos.add(three);
        ...
        List<String> popRuleIdsFetched =
                populationService.getPopulationRuleIdsByType(PopulationServiceConstants.POPULATION_RULE_TYPE_UNION_KEY,
                contextInfo);
        ...
        popRuleIdsFetched =
                populationService.getPopulationRuleIdsByType(PopulationServiceConstants.POPULATION_RULE_TYPE_INTERSECTION_KEY,
                contextInfo);
        assertEquals(1, popRuleIdsFetched.size());
        checkSameIds(intersectionIds, popRuleIdsFetched);
        // Check exclusion types
        popRuleIdsFetched =
                populationService.getPopulationRuleIdsByType(PopulationServiceConstants.POPULATION_RULE_TYPE_EXCLUSION_KEY,
                contextInfo);
...
//        PopulationRuleInfo ruleInfo = _constructPopulationRuleInfoByType(PopulationServiceConstants.POPULATION_RULE_TYPE_UNION_KEY,
//                4);
//
//
//        // Create ref population and child population IDs
