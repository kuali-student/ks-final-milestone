...
public class CluProposalRpcGwtServlet extends BaseRpcGwtServletAbstract<LuService> implements CluProposalRpcService {
	...
	/**
	 * TODO - this should all go away when a) we figure out how to do multiple
	 * LO's in the dictionary, so the configurable UI can handle this for us by
	 * using a key of "desc/plain", or b) the new orchestration framework makes
	 * all this moot
	 */
	...
	public static class LoDescMapper implements PropertyMapping {
		 ...
		 * (non-Javadoc)
		 * 
		 * @seeorg.kuali.student.common.ui.server.mvc.dto.PropertyMapping#
		 * fromModelDTOValue
		 * (org.kuali.student.common.ui.client.mvc.dto.ModelDTOValue,
		 * org.kuali.student.common.ui.server.mvc.dto.MapContext)
		 */
		@Override
		 ...
		 * 
		 * @seeorg.kuali.student.common.ui.server.mvc.dto.PropertyMapping#
		 * toModelDTOValue(java.lang.Object,
		 * org.kuali.student.common.ui.server.mvc.dto.MapContext)
		 */
		@Override
	...
	/*
	 * End of: TODO - this should all go away when ...
	 */
	...
	public String getActionsRequested(String cluProposalId) throws OperationFailedException {
		try {
			...
			// Build up a string of actions requested from the attribute set.
			// The actions can be S, F,A,C,K. examples are "A" "AF" "FCK" "SCA"
			...
			for (Map.Entry<String, String> entry : results.entrySet()) {
				...
				if (KEWConstants.ROUTE_HEADER_SAVED_CD.equals(documentStatus) || KEWConstants.ROUTE_HEADER_INITIATED_CD.equals(documentStatus)) {
					...
					// if not Complete or Approve code then show the standard
					// buttons
					else {
	...
	public Boolean disapproveDocument(String requestDocId) {
		...
		try {
			...
			// String docId, String principalId, String docTitle, String
			// docContent, String annotation
	...
	private List<LoInfo> getLoInfo(CluProposalModelDTO cluProposal) {
		try {
			MapContext ctx = new MapContext();// TODO should/can this be reused?
			ModelDTO loInfoModelDTO = ((ModelDTOType) cluProposal.get("cluInfo/loInfos" /*
																						 * LOInfoModelDTO.
																						 * DTO_KEY
																						 */)).get();
			LoInfo loInfo;
	...
	public CluProposalModelDTO createProposal(CluProposalModelDTO cluProposalDTO) throws OperationFailedException {
		...
		try {
			...
			// now update the clu with whatever changes were made in save...
			// methods
			...
			// TODO: Should effective/expiration date be copied from cluInfo or
			// new fields added to screen
			...
			/*
			 * ModelDTO loInfoModelDTO =
			 * ((ModelDTOType)cluProposalDTO.get(LOInfoModelDTO.DTO_KEY)).get();
			 * if(loInfoModelDTO != null && loInfoModelDTO.size() > 0) { LoInfo
			 * loInfo = (LoInfo)ctx.fromModelDTO(loInfoModelDTO);
			 * 
			 * //Create lo in lo service loInfo =
			 * learningObjectiveService.createLo(null, loInfo.getType(),
			 * loInfo); }
			 */
			...
			if (null != cluInfo.getAdminOrg()) {
				...
				// simpleDocService.save(docResponse.getDocId(), username,
				// title,getCluProposalDocContent(cluInfo,proposalInfo),
				// saveComment);
				// if(stdResp==null||StringUtils.isNotBlank(stdResp.getErrorMessage())){
				// throw new
				// OperationFailedException("Error found saving document: " +
	...
	public CluProposalModelDTO saveProposal(CluProposalModelDTO cluProposalDTO) throws OperationFailedException {
		...
		try {
			...
			/*
			 * //Convert loInfo model dto to LoInfo ModelDTO loInfoModelDTO =
			 * ((ModelDTOType)cluProposalDTO.get("cluInfo/loInfos"
			 * LOInfoModelDTO.DTO_KEY)).get(); if(loInfoModelDTO != null) {
			 * LoInfo loInfo = (LoInfo)ctx.fromModelDTO(loInfoModelDTO);
			 * 
			 * //Create lo in lo service loInfo =
			 * learningObjectiveService.updateLo(loInfo.getId(), loInfo);
			...
			if ((KEWConstants.ROUTE_HEADER_INITIATED_CD.equals(docDetail.getDocRouteStatus())) || (KEWConstants.ROUTE_HEADER_SAVED_CD.equals(docDetail.getDocRouteStatus()))) {
				// StandardResponse stdResp =
				// simpleDocService.save(docDetail.getRouteHeaderId().toString(),
				// username, title,
				// getCluProposalDocContent(cluInfo,proposalInfo), "");
				// if(stdResp==null||StringUtils.isNotBlank(stdResp.getErrorMessage())){
				// throw new
				// OperationFailedException("Error found saving document: " +
				// stdResp.getErrorMessage());
				...
				// StandardResponse stdResp =
				// simpleDocService.saveDocumentContent(docDetail.getRouteHeaderId().toString(),
				// username, title,
				// getCluProposalDocContent(cluInfo,proposalInfo));
				// if(stdResp==null||StringUtils.isNotBlank(stdResp.getErrorMessage())){
				// throw new
				// OperationFailedException("Error found updating document: " +
				// stdResp.getErrorMessage());
				// }
			}
	...
	private void saveCourseFormats(CluInfo parentCluInfo, ModelDTO cluInfoModelDTO) throws Exception {
		...
		if (courseFormatList != null) {
			...
			for (ModelDTOValue courseFormatValue : courseFormatList) {
				...
				courseFormatShell.setType("kuali.lu.type.CreditCourseFormatShell"); // TODO
				// -
				// a
				// constant
				// somewhere?
				courseFormatShell.setState("draft"); // TODO - a constant
				// somewhere?
				// TODO - what else should we set from the parentCluInfo?
				// courseFormatShell.setAcademicSubjectOrgs(new
				// ArrayList(parentCluInfo.getAcademicSubjectOrgs()));
				...
				if (activityList != null) {
					...
					for (ModelDTOValue activityValue : activityList) {
						...
						/*
						 * // populate activity's fields from overarching
						 * CluInfo // could probably just copy some of these
						 * List's, rather than creating copies
						 * activityCluInfo.setAcademicSubjectOrgs(new
						 * ArrayList(parentCluInfo.getAcademicSubjectOrgs()));
						 * //activityCluInfo.setAccountingInfo(parentCluInfo.
						 * getAccountingInfo()); // ??
						 * activityCluInfo.setAccreditationList
						 * (parentCluInfo.getAccreditationList()); // ??
						 * activityCluInfo
						 * .setPrimaryAdminOrg(parentCluInfo.getPrimaryAdminOrg
						 * ()); activityCluInfo.setAlternateAdminOrgs(new
						 * ArrayList
						 * <AdminOrgInfo>(parentCluInfo.getAlternateAdminOrgs
						 * ())); activityCluInfo.setAlternateIdentifiers(new
						 * ArrayList
						 * <CluIdentifierInfo>(parentCluInfo.getAlternateIdentifiers
						 * ())); // activityCluInfo.setAttributes(new
						 * HashMap<String,
						 * String>(parentCluInfo.getAttributes()); // ??
						 * activityCluInfo.setCampusLocationList(new
						 * ArrayList<String
						 * >(parentCluInfo.getCampusLocationList())); // don't
						 * do this; this is one of the values set by proposer
						 * when creating the activity //
						 * activityCluInfo.setDefaultEnrollmentEstimate(...
						 * activityCluInfo
						 * .setDefaultMaximumEnrollment(parentCluInfo
						 * .getDefaultMaximumEnrollment());
						 * activityCluInfo.setDesc(parentCluInfo.getDesc()); //
						 * ?? activityCluInfo.setEffectiveDate((Date)
						 * parentCluInfo.getEffectiveDate().clone());
						 * activityCluInfo.setExpirationDate((Date)
						 * parentCluInfo.getExpirationDate().clone());
						 * activityCluInfo
						 * .setFeeInfo(parentCluInfo.getFeeInfo()); // ??
						 * activityCluInfo
						 * .setHasEarlyDropDeadline(parentCluInfo.
						 * isHasEarlyDropDeadline());
						 * activityCluInfo.setHazardousForDisabledStudents
						 * (parentCluInfo.isHasEarlyDropDeadline());
						 * activityCluInfo.setInstructors(new
						 * ArrayList<CluInstructorInfo
						 * >(parentCluInfo.getInstructors());
						 * activityCluInfo.set
						 */
			...
			// TODO - delete activity or course-format and associated activities
			// when delete button is pressed in multiplicities
		}
	}
		...
		if (null != offeredJointlyListType) {
			...
			for (ModelDTOValue offeredJointlyValue : offeredJointlyList) {
				...
				try {
					...
					// Found it; create a LU_LU_RELATION_TYPE_JOINTLY_OFFERED
					// CluCluRelation
					CluCluRelationInfo ccrInfo = new CluCluRelationInfo();
					ccrInfo.setCluId(parentCluInfo.getId());
	...
	private void saveLearningObjectives(CluInfo cluInfo, CluProposalModelDTO cluProposalDTO) throws Exception {
		...
		if (null != learningObjectiveListType) {
			...
			// going to assume here that cluInfo is for a valid Clu that's
			// already been persisted
			// TODO - this needs to recursively navigate the LO tree(s) that the
			// user has specified in the UI
			for (ModelDTOValue learningObjectiveValue : learningObjectiveList) {
				...
				// for now, all we have is 'desc' in the ModelDTO, and don't
				// have an LoInfoModelDTO as such
				...
				if (null != desc && null != desc.getPlain() && desc.getPlain().length() > 0) {
					...
					LoInfo existingLo = (null != info.getId() ? learningObjectiveService.getLo(info.getId()) : null);
					if (null == existingLo) {
						// nope; create it
						// TODO - hardcoded Repo is just bad; somehow this needs
						// to come from
						...
						// TODO - this will be Clu-Lo or Lo-Lo, depending on
						// where we are in the tree
						service.addOutcomeLoToClu(createdLo.getId(), cluInfo.getId());
					} else {
						// update it in case the client updated description,
						// etc.
						...
						if (!service.getLoIdsByClu(cluInfo.getId()).contains(existingLo.getId())) {
							...
							// learningObjectiveService.createLoLoRelation()
							// when doing multi-level LO's
	...
	private void removeDeletedLOs(String cluId, List<ModelDTOValue> learningObjectiveList) throws DoesNotExistException, InvalidParameterException, MissingParameterException, OperationFailedException, org.kuali.student.core.exceptions.OperationFailedException, BeanMappingException, DependentObjectsExistException, PermissionDeniedException {
		...
		// now see if any of the existing related LO's weren't submitted by the
		// client
		for (String loId : loIds) {
			if (!loIdsFromUser.contains(loId)) {
				// With LO reuse, we need to check that the LO is not related to
				// any other LU's
				...
				if (relatedCluIds.size() == 1) {
					...
					// only Clu this LO's related to is the one this proposal's
					// for, so get rid of it
	...
	public CluProposalModelDTO getProposal(String proposalId) throws OperationFailedException {
		...
		try {
			...
			// FIXME: Better error handling if more than one reference or no
			// reference
	...
	public Boolean adhocRequest(String docId, String recipientPrincipalId, String requestType, String annotation) throws OperationFailedException {
		...
		try {
			 ...
			 * CluInfo cluInfo = cluProposal.getCluInfo(); String
			 * workflowDocTypeId = "CluCollaboratorDocument";// TODO make //
			 * sure this // name is // correct DocumentResponse docResponse =
			 * simpleDocService.create(username, docId, workflowDocTypeId,
			 * cluInfo.getOfficialIdentifier() .getLongName()); if
			 * (StringUtils.isNotBlank(docResponse.getErrorMessage())) { throw
			 * new OperationFailedException("Error found creating document: " +
			 * docResponse.getErrorMessage()); }
			 */
			// Do the adHoc
			// Error with the simpleDocService.requestAdHocXXXToPrincipal
			// method. the workflow document is getting routed to the wrong
			// user.
			// Please change the order once this has been fixed in the rice api.
			if (requestType.equals("FYI")) {
				// StandardResponse stdResp =
				// simpleDocService.requestAdHocFyiToPrincipal(docId, username,
				// recipientPrincipalId, fyiAnnotation);
			...
			if (requestType.equals("Approve")) {
				// StandardResponse stdResp =
				// simpleDocService.requestAdHocApproveToPrincipal(docId,
				// username, recipientPrincipalId, approveAnnotation);
			...
			if (requestType.equals("Acknowledge")) {
				// StandardResponse stdResp =
				// simpleDocService.requestAdHocAckToPrincipal(docId, username,
				// recipientPrincipalId, ackAnnotation);
	...
	/**
	 * @param learningObjectiveService
	 *            the learningObjectiveService to set
	 */
	...
	// /**
	// * This method should return a an empty model with associated model
	// definition for
	// * requested model
	// */
	...
	// def.define("proposal",
	// "org.kuali.student.lum.lu.assembly.data.client.proposal.ProposalInfoData");
	// def.define("course",
	// "org.kuali.student.lum.lu.assembly.data.client.creditcourse.CreditCourse");
	// def.define("course/formats/*",
	// "org.kuali.student.lum.lu.assembly.data.client.creditcourse.CreditCourseFormat");
	// def.define("course/formats/*/activities/*",
	// "org.kuali.student.lum.lu.assembly.data.client.creditcourse.CreditCourseActivity");
	//            
	// def.define("course/description",
	// "org.kuali.student.lum.lu.assembly.data.client.RichTextInfoData");
	// def.define("course/rationale",
	// "org.kuali.student.lum.lu.assembly.data.client.RichTextInfoData");
	// def.define("course/duration",
	// "org.kuali.student.lum.lu.assembly.data.client.atp.TimeAmountInfoData");
	// def.define("course/primaryInstructor",
	// "org.kuali.student.lum.lu.assembly.data.client.CluInstructorInfoData");
	// def.define("course/alternateIdentifiers",
	// "org.kuali.student.lum.lu.assembly.data.client.CluIdentifierInfoData");
	//            
	// def.define("course/formats/*/activities/*/intensity",
	// "org.kuali.student.lum.lu.assembly.data.client.atp.TimeAmountInfoData");
	//            
	// //
	// // def.define("cluInfo/officialIdentifier",
	// "org.kuali.student.lum.lu.dto.CluIdentifier");
	// // def.define("cluInfo/rationale", "org.kuali.student.RichText");
	// // def.define("cluInfo/formats/*/cluInfo",
	// "org.kuali.student.lum.lu.dto.CluInfo");
	// // def.define("cluInfo/formats/*/activities/*/cluInfo",
	// "org.kuali.student.lum.lu.dto.CluInfo");
	...
	private synchronized void initAssemblers() {
		if (creditCourseProposalAssembler == null) {
			...
			// TODO change how the state is set/passed in to the proposal
			// assembler, if at all
			// creditCourseProposalAssembler = new
			// CreditCourseProposalAssembler("draft");
