...
import org.kuali.student.lum.lu.assembly.data.client.refactorme.base.CurrencyAmountInfoHelper.Properties;


public class CurrencyAmountInfoMetadata
{
	...
	{
		int recurseLevel = increment (recursions, "CurrencyAmountInfoMetadata");
		
		Metadata childMeta;
		Metadata listMeta;
		
		// metadata for currencyTypeKey
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.CURRENCY_TYPE_KEY.getKey (), childMeta);
		...
		if (this.matches (type, state, "(default)", "(default)"))
		{
			...
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("currency.types"));
		}
		
		// metadata for currencyQuantity
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.CURRENCY_QUANTITY.getKey (), childMeta);
