...
import org.kuali.student.lum.lu.assembly.data.client.refactorme.base.ResultOptionInfoHelper.Properties;


public class ResultOptionInfoMetadata
{
	...
	{
		int recurseLevel = increment (recursions, "ResultOptionInfoMetadata");
		...
		// metadata for resultUsageTypeKey
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.RESULT_USAGE_TYPE_KEY.getKey (), childMeta);
		...
		if (this.matches (type, state, "(default)", "(default)"))
		{
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("required"));
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("single"));
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("resultusage.types"));
		}
		
		// metadata for resultComponentId
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.RESULT_COMPONENT_ID.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.STRING);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
