 ...
 * 
 * TODO: The following is a list of items that need to be fixed. 1) All
 * hardcoded drop downs need to be replaced with one populated via an
 * enumeration lookup 2) Any pickers (eg. org, course, needs to be replaced wtih
 * proper lookup based search pickers
 * 
 * @author Kuali Student Team
 * 
 */
public class CourseConfigurer implements CreditCourseProposalConstants, CreditCourseProposalInfoConstants, CreditCourseConstants, CreditCourseFormatConstants, CreditCourseActivityConstants, MetaInfoConstants, CreditCourseDurationConstants, FeeInfoConstants, LearningObjectiveConstants {
	...
	public void configureCourseProposal(ConfigurableLayout layout) {
		...
		// layout.addSection(new String[] {"Assembler Test"}, new
		// AssemblerTestSection(LuSections.ASSEMBLER_TEST, "Assembler Test"));
	...
	private VerticalSection generateSummaryBrief(SectionTitle title) {
		...
		// TODO: Norm: find out why was this prefixed with proposal there is no
		// state on proposal it is on the main object
	...
	/**
	 * Adds a section for adding or modifying rules associated with a given
	 * course (program).
	 * 
	 * @param layout
	 *            - a content pane to which this section is added to
	 * @return
	 */
	private SectionView generateCourseRequisitesSection() {
		...
		/*
		 * TODO: Once we have a section that allows for flow between rule
		 * screens VerticalSection enrollmentSection = new VerticalSection();
		 * enrollmentSection
		 * .setSectionTitle(SectionTitle.generateH2Title("Enrollment Restrictions"
		 * )); enrollmentSection.addField(new FieldDescriptor("rationalle",
		 * "Rationalle", Type.STRING)); enrollmentSection.addField(new
		 * FieldDescriptor("rules", "Rules", Type.STRING));
	...
	private SectionView generateFinancialsSection() {
		...
		// TODO ALL KEYS in this section are place holders until we know actual
		// keys
		...
		addField(feeAmount, FEES + "/" + TAXABLE, getLabel(LUConstants.TAXABLE_SYMBOL_LABEL_KEY));// TODO
																									// checkboxes
																									// go
																									// here
																									// instead
	...
	public SectionView generateGovernanceSection() {
		...
		// FIXME the new OrgAdvSearchPicker doesn't fire selection/value change
		// events correctly, won't work with binding
		// adminOrgs.addField(new FieldDescriptor("course/department", null,
		// Type.STRING, configureAdminOrgSearch()));
		...
		deptDescriptor.setWidgetBinding(new ModelWidgetBinding<OrgListPicker>() {
			// FIXME had to create custom binding because the OrgListPicker
			// always returns a list, even of 1
			@Override
			public void setModelValue(OrgListPicker widget, DataModel model, String path) {
				model.set(deptPath, widget.getSelectedItem());

			}

			@Override
			public void setWidgetValue(OrgListPicker widget, DataModel model, String path) {
				widget.setValue((String) model.get(path));
			}
		});
		adminOrgs.addField(deptDescriptor);

		// adminOrgs.addField(new
		// FieldDescriptor("cluInfo/primaryAdminOrg/orgId", null, Type.STRING,
		// new OrgPicker()));
		// adminOrgs.addField(new FieldDescriptor("cluInfo/alternateAdminOrgs",
		// null, Type.LIST, new AlternateAdminOrgList()));
	...
	public SectionView generateCourseInfoSection() {
		...
		// FIXME: Label should be key to messaging, field type should come from
		// dictionary?
		...
		// TODO - hide cross-listed, offered jointly and version codes
		// initially, with
	...
	// public Callback<Boolean> getSubjectValidationCallback(final
	// FieldDescriptor subjectFD, final String objectKey){
	...
	// Application.getApplicationContext().getDictionaryData(objectKey);
	// if(objStructure == null){
	// Window.alert("Cannot load dictionary(object structure)");
	// }
	// List<ValidationResultContainer> results =
	// val.validateTypeStateObject((ModelDTO) m.get(), objStructure);
	// e.setValidationResult(results);// filled by calling the real validate
	// code
	...
	public SectionView generateCourseLogisticsSection() {
		...
		// FIXME wilj: do we need to set the instructor's orgId? or can we
		// default it at the assembler level?
		addField(instructors, COURSE + "/" + PRIMARY_INSTRUCTOR, null);
		// instructors.addField(new FieldDescriptor("cluInfo/instructors", null,
		// Type.LIST, new AlternateInstructorList()));

		// CREDITS
		// TODO: These needs to be mapped to learning results
		// VerticalSection credits =
		// initSection(getH3Title(LUConstants.CREDITS_LABEL_KEY), WITH_DIVIDER);
		// credits.addField(new FieldDescriptor("cluInfo/creditType",
		// getLabel(LUConstants.CREDIT_LABEL_KEY), Type.STRING));
		// credits.addField(new FieldDescriptor("cluInfo/creditValue",
		// getLabel(LUConstants.CREDIT_VALUE_LABEL_KEY), Type.STRING));
		// credits.addField(new FieldDescriptor("cluInfo/maxCredits",
		// getLabel(LUConstants.MAX_CREDITS_LABEL_KEY), Type.STRING));
		// VerticalSection learningResults =
		// initSection(getH3Title(LUConstants.LEARNING_RESULTS_LABEL_KEY),
		// WITH_DIVIDER);
		// learningResults.addField(new FieldDescriptor("cluInfo/evalType",
		// getLabel(LUConstants.EVALUATION_TYPE_LABEL_KEY), Type.STRING));
		// //TODO EVAL TYPE ENUMERATION ????
		...
		addField(scheduling, COURSE + "/" + CreditCourseConstants.DURATION + "/" + QUANTITY, getLabel(LUConstants.DURATION_LITERAL_LABEL_KEY)); // TODO
																																				// DURATION
																																				// ENUMERATION
	...
	private SectionView generateLearningObjectivesSection() {
		...
		// addField(los, LEARNING_OBJECTIVES, null, new LOBuilder(type, state,
		// groupName));
	...
	public class CourseActivityList extends UpdatableMultiplicityComposite {
		...
		public Widget createItem() {
			...
			/*
			 * CreditInfo is deprecated, needs to be replaced with learning
			 * results
			 * activity.setCurrentFieldLabelType(FieldLabelType.LABEL_TOP);
			 * activity.addField(new FieldDescriptor("creditInfo",
			 * "Credit Value", Type.STRING)); activity.nextRow();
			 * activity.setCurrentFieldLabelType(FieldLabelType.LABEL_TOP);
			 * activity.addField(new FieldDescriptor("evalType",
			 * "Learning Result", Type.STRING)); activity.nextRow();
			 */

			activity.setCurrentFieldLabelType(FieldLabelType.LABEL_TOP);
			// FIXME need to get the term offered added to the activity
			// metadata?
			// activity.addField(new FieldDescriptor("term",
			// getLabel(LUConstants.TERM_LITERAL_LABEL_KEY), Type.STRING, new
			// AtpTypeList()));
	...
	//
	// Field f = LUDictionaryManager.getInstance().getField(objectKey, type,
	// state, fieldName);
	//
	// FieldDescriptor fd =
	...
	// FIXME: This is a temp widget impl for the Curriculum Oversight field.
	// Don't yet know if this
	// will be a multiple org select field, in which case we need a multiple org
	// select picker widget.
	// Otherwise if it's single org picker, need a way to bind a HasText widget
	// to ModelDTOList
	...
	// public class AlternateAdminOrgList extends
	// MultiplicityCompositeWithLabels {
	// {
	...
	// ns.addField(new FieldDescriptor("orgId", "Organization ID", Type.STRING,
	// new OrgPicker() ));
	// multi.addSection(ns);
	//            
	// return multi;
	...
	// }
	//    
	// public class AlternateInstructorList extends
	// MultiplicityCompositeWithLabels {
	// {
	...
	// CustomNestedSection ns = new CustomNestedSection();
	// ns.setCurrentFieldLabelType(FieldLabelType.LABEL_TOP);
	// ns.addField(new FieldDescriptor("personId", "Instructor ID", Type.STRING
	// /*, new InstructorPicker() */ ));
	// multi.addSection(ns);
	//            
	// return multi;
	// }
	...
	// FIXME: Create a configurable checkbox list which can obtain values via
	// RPC calls
	...
	private KSSearchComponent configureAdminOrgSearch() {
		...
		// TODO: following code should be in KSSearchComponent with config
		// parameters set within SearchComponentConfiguration class
		final SearchSuggestOracle orgSearchOracle = new SearchSuggestOracle(searchConfig.getSearchService(), "org.search.orgByShortNameAndType", "org.queryParam.orgShortName", // field
																																												// user
																																												// is
																																												// entering
																																												// and
																																												...
																																												// the
																																												// parameter
				"org.queryParam.orgId", // if one wants to search by ID rather
										// than by name
				"org.resultColumn.orgId", "org.resultColumn.orgShortName");
