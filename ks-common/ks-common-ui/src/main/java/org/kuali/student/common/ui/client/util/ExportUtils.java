...
public class ExportUtils {
	...
	public static ExportElement getExportItemDetails(ExportElement exportItem,
			Widget fieldWidget, boolean setFirstFieldValue, String viewName,
			String sectionName) {
		ArrayList<ExportElement> subExportElements = new ArrayList<ExportElement>();
//		System.out.println("");
//		System.out.println("getExportItemDetails..."
...
//				fieldValue = sectionTitle.getTitleText();
				...
				System.out
						.println("Section tile interface probleem - only in debugging.");
				e.printStackTrace();
			}
...
//					subExportElements, viewName, sectionName);
			...
			subExportElements = widget.getExportElementsWidget(viewName,
					sectionName);
	...
	/**
	 * This method gets the current controller based on the widget that was
	 * passed to it.
	 * 
	 * @param theWidget
	...
	public static void handleExportClickEvent(Controller currentController,
			String format, String reportTitle) {
	...
	public static void debugExportElementsArray(
	...
	private static void debutExportElementsArraySubList(
			List<ExportElement> exportElements) {
...
//			System.out.println("Sub list : ");
	...
	public static ArrayList<ExportElement> getDetailsForWidget(
			SummaryTableSection tableSection,
			ArrayList<ExportElement> exportElements) {
				...
				if (row.isShown()) {
					if (fdWidget != null) {
						...
						if (fdWidget instanceof KSListPanel) {
							ArrayList<ExportElement> subExportElements = new ArrayList<ExportElement>();
							subExportElements = ExportUtils
									.getDetailsForWidget(fdWidget,
											subExportElements, blockName,
											blockName);
							element.setSubset(subExportElements);
						} else {
							//
							element = ExportUtils.getExportItemDetails(element,
									fdWidget, true, blockName, blockName);
						}
					} else {
					...
					if (fdWidget2 != null) {
						element = ExportUtils.getExportItemDetails(element,
								fdWidget2, false, blockName, blockName);
	...
	public static ArrayList<ExportElement> getDetailsForWidget(
			Widget currentViewWidget, ArrayList<ExportElement> exportElements,
			String viewName, String sectionName) {
//		System.out.println("");
//		System.out.println("getDetauksFirWidget..."
//				+ currentViewWidget.getClass().getName());
//		System.out.println("");
		if (currentViewWidget instanceof Section) {
			...
			for (FieldDescriptor field : widgetFields) {
				...
				//
				ExportUtils.addElementToElementArray(exportElements, getExportItemDetails(exportItem,
...
//
				ExportUtils.addElementToElementArray(exportElements, getExportItemDetails(exportItem, child,
...
// Vrydag						&& !(child instanceof SpanPanel) && !(child instanceof WarnContainer)) {
					...
					ExportElement exportItemDetails = getExportItemDetails(
							exportItem, child, true, viewName, sectionName);
					
					exportElements = ExportUtils.addElementToElementArray(exportElements, exportItemDetails);
// Vrydag					if ((exportItemDetails.getFieldValue() != null && !exportItemDetails.getFieldValue().equals(""))
...
//					}
			...
			System.out.println("ExportUtils does not cater for this type..."
					+ currentViewWidget.getClass().getName());
	...
	public static ArrayList<ExportElement> getExportElementsFromView(
			Widget currentViewWidget, ArrayList<ExportElement> exportElements,
			String viewName, String sectionName) {
		if (exportElements == null) {
		...
		if (currentViewWidget instanceof VerticalSectionView) {
			...
			for (FieldDescriptor field : widgetFields) {
				...
				exportElements.add(getExportItemDetails(exportItem,
						fieldWidget, true, viewName, sectionName));
			}
			if ((currentViewWidget instanceof BaseSection)
					&& (widgetHasFields.getFields().size() == 0)) {
				BaseSection bSection = (BaseSection) currentViewWidget;
				ExportElement exportItem = new ExportElement();
				exportItem.setSectionName(sectionName + viewName);
				exportItem.setViewName(sectionName + viewName);
				exportItem.setFieldLabel("???00");
				exportItem = getExportItemDetails(exportItem, bSection
						.getLayout(), true, viewName, sectionName);
	...
	public static ArrayList<ExportElement> addElementToElementArray(
			ArrayList<ExportElement> elementArray, ExportElement element) {
		if (element.getFieldLabel() != null || element.getFieldValue() !=  null 
				|| element.getFieldValue2() != null 
				|| (element.getSubset() != null && element.getSubset().size() > 0)) { 
