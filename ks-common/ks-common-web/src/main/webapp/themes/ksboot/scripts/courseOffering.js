...
/* When a user attempts to delete a format then we show them a prompt of any AOs/RGs it might also affect.
 * This updates the prompt and sets a value to indicate whether or not the user should be shown the prompt.
 */
function prepareToDeleteFormat( numAos, numRgs ) {

    // don't prompt the user if no impact
    if(numAos < 1 && numRgs < 1 ) {
        jQuery('#CourseOfferingEdit-DeleteFormatConfirmation-HasAosRgs').text('false');
        return;
    }

    // update the AO-details
    if( numAos < 1 ) {
        jQuery('#CourseOfferingEdit-DeleteFormatConfirmation .activityOfferings').hide();
    }
    else {
        jQuery('#CourseOfferingEdit-DeleteFormatConfirmation .activityOfferings').show();
        jQuery('#CourseOfferingEdit-DeleteFormatConfirmation .activityOfferings .numberAffected .inlineBlock span').text( numAos );
    }

    // update the RG-details
    if( numRgs < 1 ) {
        jQuery('#CourseOfferingEdit-DeleteFormatConfirmation .registrationGroups').hide();
    }
    else {
        jQuery('#CourseOfferingEdit-DeleteFormatConfirmation .registrationGroups').show();
        jQuery('#CourseOfferingEdit-DeleteFormatConfirmation .registrationGroups .numberAffected .inlineBlock span').text( numRgs );
    }

    // prompt the user, since at least 1 AO/RG will be impacted
    jQuery('#CourseOfferingEdit-DeleteFormatConfirmation-HasAosRgs').text('true');

    writeCurrentPageToSession(this);
}

/* When a user attempts to delete a format then we show them a prompt of any AOs/RGs it might also affect.
 * This returns a value to indicate whether or not the user should be shown the prompt.
 */
function shouldPromptUserToDeleteFormat() {
    hasAosRgs = jQuery('#CourseOfferingEdit-DeleteFormatConfirmation-HasAosRgs').text();
    return (hasAosRgs === 'true');
}
