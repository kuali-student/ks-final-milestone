/*
 * Copyright 2007 Sandy McArthur, Jr. Licensed to the Apache Software Foundation (ASF) under one or more contributor license
 * agreements. See the NOTICE file distributed with this work for additional information regarding copyright ownership. The
 * ASF licenses this file to You under the Apache License, Version 2.0 (the "License"); you may not use this file except in
 * compliance with the License. You may obtain a copy of the License at http://www.apache.org/licenses/LICENSE-2.0 Unless
 ...
 * <p>
 * Note: The emulated version differs from the JDK version in that it is <em>not</em> Serializable.
 * </p>
 */
public class VetoableChangeSupport {
    ...
    public synchronized VetoableChangeListener[] getVetoableChangeListeners() {
        ...
        Iterator<String> keysIterator = selectedVetoableChangeListeners.keySet().iterator();
        while (keysIterator.hasNext()) {
            ...
            if (selectedListeners != null) {
                for (Iterator<VetoableChangeListener> it = selectedListeners.iterator(); it.hasNext();) {
                    VetoableChangeListener listener = it.next();
    ...
    private void doFirePropertyChange(PropertyChangeEvent event) throws PropertyVetoException {
        ...
        List<VetoableChangeListener> allListeners = new ArrayList<VetoableChangeListener>();
        // Listeners to all property change events
        allListeners.addAll(allVetoableChangeListeners);

        // Listens to a given property change
        List<VetoableChangeListener> listeners = selectedVetoableChangeListeners.get(event.getPropertyName());
        if (listeners != null) {
            allListeners.addAll(listeners);
        }

        try {
            Iterator<VetoableChangeListener> iter = allListeners.iterator();
            while (iter.hasNext()) {
                VetoableChangeListener listener = iter.next();
    ...
    private static VetoableChangeListener[] getAsVetoableChangeListenerArray(List<VetoableChangeListener> listeners) {
