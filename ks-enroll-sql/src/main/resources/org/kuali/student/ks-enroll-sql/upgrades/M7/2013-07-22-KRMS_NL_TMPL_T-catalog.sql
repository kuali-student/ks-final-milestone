...
/
Insert into KRMS_NL_TMPL_T (LANG_CD,NL_TMPL_ID,NL_USAGE_ID,TMPL,TYP_ID,VER_NBR) values ('en','KS-KRMS-NL-TMPL-1505','KS-KRMS-NL-USAGE-1005','must be in a major #if($org.getAttributes().get("umType") == "Academic Department")within $org.getLongName() department#{else}in $org.getLongName()#end','10065',0)
/
...
/
Insert into KRMS_NL_TMPL_T (LANG_CD,NL_TMPL_ID,NL_USAGE_ID,TMPL,TYP_ID,VER_NBR) values ('en','KS-KRMS-NL-TMPL-1513','KS-KRMS-NL-USAGE-1005','$intValue $NLHelper.getProperGrammar($intValue, "course") with a minimum grade of $grade from #if($courseCluSet.getQueryValueFromParam("lu.queryParam.luOptionalDivision") == "")$courseCluSet.getCluSetAsCode()#{else}#set($prefix=${courseCluSet.getQueryValueFromParam("lu.queryParam.luOptionalDivision").toUpperCase()})$prefix$courseCluSet.getQueryValueFromParam("lu.queryParam.luOptionalCrsNoRange") course range#end','10028',0)
/
