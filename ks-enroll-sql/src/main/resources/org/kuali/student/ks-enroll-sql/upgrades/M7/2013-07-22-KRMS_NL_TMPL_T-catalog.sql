...
/
Insert into KRMS_NL_TMPL_T (LANG_CD,NL_TMPL_ID,NL_USAGE_ID,TMPL,TYP_ID,VER_NBR) values ('en','KS-KRMS-NL-TMPL-1504','KS-KRMS-NL-USAGE-1005','a minimum of $intValue $NLHelper.getProperGrammer($intValue, "course") from $courseCluSet.getCluSetAsCode()#{else}#if($courseCluSet.getQueryValueFromParam("lu.queryParam.luOptionalDivision") == "")minimum of $intValue $NLHelper.getProperGrammar($intValue, "course") from #set($a=$courseCluSet.getCluSetAsCode().lastIndexOf(","))#set($b=1)#set($c=$a+$b)#if($courseCluSet.getCluList().size() > 2)#set($courses=$courseCluSet.getCluSetAsCode().substring(0,$a)+", or"+$courseCluSet.getCluSetAsCode().substring($c))#{else}#set($courses=$courseCluSet.getCluSetAsCode().substring(0,$a)+" or"+$courseCluSet.getCluSetAsCode().substring($c))#end$courses#{else}minimum of $intValue $NLHelper.getProperGrammar($intValue, "course") from #set($prefix=${courseCluSet.getQueryValueFromParam("lu.queryParam.luOptionalDivision").toUpperCase()})$prefix$courseCluSet.getQueryValueFromParam("lu.queryParam.luOptionalCrsNoRange") course range#end#end','10019',0)
/
...
/
Insert into KRMS_NL_TMPL_T (LANG_CD,NL_TMPL_ID,NL_USAGE_ID,TMPL,TYP_ID,VER_NBR) values ('en','KS-KRMS-NL-TMPL-1520','KS-KRMS-NL-USAGE-1005','a minimum of &intValue $NLHelper.getProperGrammer($intValue, "course") from $courseCluSet.getCluSetAsCode()#{else}#if($courseCluSet.getQueryValueFromParam("lu.queryParam.luOptionalDivision") == "")#if($intValue > 1)$intValue $NLHelper.getProperGrammar($intValue, "course") from $courseCluSet.getCluSetAsCode()#{else}#set($a=$courseCluSet.getCluSetAsCode().lastIndexOf(","))#set($b=1)#set($c=$a+$b)#set($d=$courseCluSet.getCluSetAsCode().lastIndexOf(")"))#if($courseCluSet.getCluList().size() > 2)#set($courses=$courseCluSet.getCluSetAsCode().substring(1,$a)+", or"+$courseCluSet.getCluSetAsCode().substring($c, $d))#{else}#set($courses=$courseCluSet.getCluSetAsCode().substring(1,$a)+" or"+$courseCluSet.getCluSetAsCode().substring($c, $d))#end$intValue $NLHelper.getProperGrammar($intValue, "course") from $courses#end#{else}$intValue $NLHelper.getProperGrammar($intValue, "course") from #set($prefix=${courseCluSet.getQueryValueFromParam("lu.queryParam.luOptionalDivision").toUpperCase()})$prefix$courseCluSet.getQueryValueFromParam("lu.queryParam.luOptionalCrsNoRange") course range#end#end','10024',0)
/
