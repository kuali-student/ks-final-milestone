...
import org.kuali.student.brms.devgui.client.DateRange;
import org.kuali.student.brms.devgui.client.GuiUtil;
import org.kuali.student.brms.devgui.client.IllegalRuleFormatException;
import org.kuali.student.brms.devgui.client.GuiUtil.YieldValueFunctionInfoType;
...
public class RulesComposite extends Composite {
    ...
    private List<FactTypeInfo> definedFactTypesList = null; // keep copy of current list of factTypeKeys based on business
                                                               // rule type
    ...
    private BusinessRuleInfo displayedRule = null; // keep copy of business rule so we can update all fields user
    private BusinessRuleInfo displayedRuleInitialCopy = null; // keep the rule initial copy, give user warning if they
                                                                 // made changes
    private RulesVersionInfo displayedRuleInfo = null; // keep copy of rule meta info
    private Map<Integer, RuleElementInfo> definedPropositions = new TreeMap<Integer, RuleElementInfo>();
        ...
        if (!loaded) {
            ...
            rulesVersionsTable.addSelectionListener(new ModelTableSelectionListener<RulesVersionInfo>() {
                ...
                public void onSelect(RulesVersionInfo modelObject) {
                    ...
                    DevelopersGuiService.Util.getInstance().fetchDetailedBusinessRuleInfo(modelObject.getBusinessRuleId(), new AsyncCallback<BusinessRuleInfo>() {
            ...
            submitRuleButton.addClickListener(new ClickListener() {
                public void onClick(final Widget sender) {
                    ...
                    DevelopersGuiService.Util.getInstance().createBusinessRule(displayedRule, new AsyncCallback<BusinessRuleInfo>() {
                        public void onFailure(Throwable caught) {
                            // just re-throw it and let the uncaught exception handler deal with it
                            Window.alert(caught.getMessage());
                            // throw new RuntimeException("Unable to load BusinessRuleInfo objects", caught);
                        ...
                        public void onSuccess(BusinessRuleInfo newBusinessRule) {
            ...
            updateRuleButton.addClickListener(new ClickListener() {
                public void onClick(final Widget sender) {
                    ...
                    DevelopersGuiService.Util.getInstance().updateBusinessRule(displayedRule.getId(), displayedRule, new AsyncCallback<BusinessRuleInfo>() {
                        public void onFailure(Throwable caught) {
                            Window.alert(caught.getMessage());
                        }

                        public void onSuccess(BusinessRuleInfo updatedBusRule) {
                            // update the model
            ...
            activateRuleButton.addClickListener(new ClickListener() {
                public void onClick(final Widget sender) {
                    ...
                    DevelopersGuiService.Util.getInstance().updateBusinessRuleState(displayedRule.getId(), BusinessRuleStatus.ACTIVE.toString(), new AsyncCallback<BusinessRuleInfo>() {
                        public void onFailure(Throwable caught) {
                            Window.alert(caught.getMessage());
                        }
            ...
            createNewVersionButton.addClickListener(new ClickListener() {
                public void onClick(final Widget sender) {
                    ...
                    BusinessRuleInfo newRuleVersion = createRuleCopy(displayedRule);
                    ...
                    DevelopersGuiService.Util.getInstance().createBusinessRule(newRuleVersion, new AsyncCallback<BusinessRuleInfo>() {
                        public void onFailure(Throwable caught) {
                            // just re-throw it and let the uncaught exception handler deal with it
                            Window.alert(caught.getMessage());
                            // throw new RuntimeException("Unable to load BusinessRuleInfo objects", caught);
                        ...
                        public void onSuccess(BusinessRuleInfo newRuleVersion) {
            ...
            retireRuleButton.addClickListener(new ClickListener() {
                public void onClick(final Widget sender) {
                    ...
                    DevelopersGuiService.Util.getInstance().updateBusinessRuleState(displayedRule.getId(), BusinessRuleStatus.RETIRED.toString(), new AsyncCallback<BusinessRuleInfo>() {
                        public void onFailure(Throwable caught) {
                            Window.alert(caught.getMessage());
                        }
            ...
            propositionsListBox.addChangeListener(new ChangeListener() {
                public void onChange(Widget sender) {
                    ...
                    // extra check
                    RulePropositionInfo selectedRuleElement = definedPropositions.get(new Integer(box.getValue(selectedIndex))).getBusinessRuleProposition();
    ...
    private void loadExistingRule(BusinessRuleInfo ruleInfo) {
        ...
        int propCount = 1;
        ruleComposition = new StringBuffer();
        definedPropositions = new HashMap<Integer, RuleElementInfo>();

        int ix = 1;
        for (RuleElementInfo elem : ruleInfo.getBusinessRuleElementList()) {
    ...
    private void initializeRuleCopy(BusinessRuleInfo rule, String newRuleName) {
        rule.setId("");
    ...
    private boolean isPropositionValid(RulePropositionInfo prop) {
        StringBuilder messages = new StringBuilder();
        ...
        YieldValueFunctionInfo yvf = prop.getLeftHandSideInfo().getYieldValueFunctionInfo();
        if (yvf == null) {
            GuiUtil.showUserDialog("ERROR: YVF cannot be empty.");
            return false;
        }

        String yvfType = yvf.getYieldValueFunctionInfoType();
        ...
        FactStructureInfo firstFact = factStructureList.get(0);
        ...
        if (yvfType.equals(YieldValueFunctionInfoType.INTERSECTION.name())) {
            // 1. Fact Type cannot be empty
            ...
            FactStructureInfo secondFact = factStructureList.get(1);
    ...
    private boolean validateRuleComposition() {
        ...
        int propIx = -1;
        for (Map.Entry<Integer, RuleElementInfo> entry : definedPropositions.entrySet()) {
            propIx++;
    ...
    private void populatePropositionFormFields(RulePropositionInfo prop) {
        clearPropositionFormFields();
        ...
        populateYVFFormFields(prop.getLeftHandSideInfo().getYieldValueFunctionInfo());
    }
    ...
    private void initializeYVFFactFormFields(String yvfType) {
        if ((yvfType == null) || (yvfType.length() == 0) || yvfType.equals(EMPTY_LIST_BOX_ITEM)) {
            ...
            yvf.setYieldValueFunctionInfoType(yvfType);
            List<FactStructureInfo> factStructureList = new ArrayList<FactStructureInfo>();
            yvf.setFactStructureList(factStructureList);
            int numberOfYVFFacts = GuiUtil.YieldValueFunctionInfoType.getNumberOfFactsFromName(yvfType);
            for (int i = 0; i < numberOfYVFFacts; i++) {
                FactStructureInfo fact = createEmptyFactSturectureDTO();
                factStructureList.add(fact);
                fact.setFactTypeKey("fact.static_key");
                fact.setStaticFact(true);
                fact.setStaticValue("");
                fact.setStaticValueDataType(GuiUtil.YieldValueFunctionInfoType.getValueDataTypeFromSymbol(GuiUtil.getYVFSymbol(yvfType)));
            }
    ...
    private void populateYVFFormFields(YieldValueFunctionInfo yvf) {

        String yvfType = (yvf == null ? "" : yvf.getYieldValueFunctionInfoType());
        ...
        int nofFactsEpected = GuiUtil.YieldValueFunctionInfoType.getNumberOfFactsFromName(yvf.getYieldValueFunctionInfoType());
        List<FactStructureInfo> factStructureList = yvf.getFactStructureList();
        if (nofFactsEpected != factStructureList.size()) {
            System.out.println("INVALID STATE: number of facts (" + factStructureList.size() + ") found in YVF entity " + yvf.getYieldValueFunctionInfoType() + " is not equal to number of expected facts (" + nofFactsEpected);
        ...
        for (int i = 1; i <= nofFactsEpected; i++) {
            FactStructureInfo fact = factStructureList.get(i - 1);
            ...
            factType.addChangeListener(new ChangeListener() {
                public void onChange(final Widget sender) {
                    ...
                    if (selectedFactType.equals(USER_DEFINED_FACT_TYPE_KEY)) {
                        ...
                        for (FactTypeInfo factTypeInfo : definedFactTypesList) {
                            ...
                            Map<String, FactParamInfo> definedParamValueMap = factTypeInfo.getFactCriteriaTypeInfo().getFactParamMap();
                            for (String key : definedParamValueMap.keySet()) {
                                FactParamInfo factParam = definedParamValueMap.get(key);
    ...
    private boolean isExecutionTimeKey(String factType, String unknownKey) {
        for (FactTypeInfo factTypeInfo : definedFactTypesList) {
            ...
            Map<String, FactParamInfo> definedParamValueMap = factTypeInfo.getFactCriteriaTypeInfo().getFactParamMap();
            FactParamInfo factParam = definedParamValueMap.get(unknownKey);
    ...
    private void updateSelectedPropositionDTO(int selectedPropListIx) {
        ...
        RulePropositionInfo newProp = retrievePropositionDTOFromFormFields();
        
        //preserve the rule element info
        RuleElementInfo origElem = definedPropositions.get(origPropKey);
    ...
    private RulePropositionInfo retrievePropositionDTOFromFormFields() {
        // now create a new rule proposition
        RulePropositionInfo prop = new RulePropositionInfo();
        prop.setName(propNameTextBox.getText());
        prop.setDescription(propDescTextBox.getText());
        LeftHandSideInfo leftSide = new LeftHandSideInfo();
        YieldValueFunctionInfo yvf = new YieldValueFunctionInfo();
        ...
        if (yvfSelected) {
            yvf.setYieldValueFunctionInfoType(yvfListBox.getValue(yvfListBox.getSelectedIndex()));
            if (yvf.getYieldValueFunctionInfoType().equals(YieldValueFunctionInfoType.INTERSECTION.name())) {
                prop.setComparisonDataTypeKey("java.math.BigDecimal");
                processingColumn = "resultColumn.cluId";
            } else { // if ((yvf.equals(YieldValueFunctionInfoType.SUM.name()) || (yvf.equals(YieldValueFunctionInfoType..name()) {
                prop.setComparisonDataTypeKey("java.math.BigDecimal");
            ...
            int numberOfYVFFacts = GuiUtil.YieldValueFunctionInfoType.getNumberOfFactsFromName(yvf.getYieldValueFunctionInfoType());
            ...
            for (int i = 1; i <= numberOfYVFFacts; i++) {
                ...
                if (selectedFactType.trim().equals(USER_DEFINED_FACT_TYPE_KEY)) {
                    ...
                    fact.setStaticValueDataType(GuiUtil.YieldValueFunctionInfoType.getValueDataTypeFromSymbol(yvfType));             
                } else {
                    for (FactTypeInfo factTypeInfo : definedFactTypesList) {
                        if (!factTypeInfo.getFactTypeKey().equals(GuiUtil.addFactTypeKeyPrefix(selectedFactType))) {
                            continue;
                        ...
                        resultColumnKeyMap.put(GuiUtil.YieldValueFunctionInfoType.getProcessingColumnFromSymbol(yvfType), processingColumn);
        ...
        leftSide.setYieldValueFunctionInfo(yvf);
        prop.setLeftHandSideInfo(leftSide);
        RightHandSideInfo rightSide = new RightHandSideInfo();
    ...
    private BusinessRuleInfo createEmptyBusinessRule() {

        BusinessRuleInfo newRule = new BusinessRuleInfo();
        newRule.setId("");
        ...
        /* 
        RuleElementInfo elem = new RuleElementInfo();
        elem.setName("");
        elem.setDescription("");
        elem.setBusinessRuleElemnetTypeKey("");
        elem.setOrdinalPosition(1);
        elem.setBusinessRuleProposition(createEmptyRulePropositionInfo());
        elemList.add(elem); */
    ...
    private RuleElementInfo createEmptyRuleElementTO() {
        LeftHandSideInfo leftSide = new LeftHandSideInfo();
        RightHandSideInfo rightSide = new RightHandSideInfo();
        RulePropositionInfo prop = new RulePropositionInfo();
        prop.setName("");
    ...
    private BusinessRuleInfo createRuleCopy(BusinessRuleInfo originalRule) {
        ...
        List<RuleElementInfo> newElementList = new ArrayList<RuleElementInfo>();
        newRule.setBusinessRuleElementList(newElementList);
        for (RuleElementInfo originalRuleElement : originalRule.getBusinessRuleElementList()) {
            RuleElementInfo newRuleElement = new RuleElementInfo();
            ...
            RulePropositionInfo originalProposition = originalRuleElement.getBusinessRuleProposition();
            if (originalProposition == null) {
                continue;
            }

            RulePropositionInfo newProposition = new RulePropositionInfo();
            ...
            LeftHandSideInfo leftSide = new LeftHandSideInfo();
            leftSide.setYieldValueFunctionInfo(originalProposition.getLeftHandSideInfo().getYieldValueFunctionInfo());
            newProposition.setLeftHandSideInfo(leftSide);
    ...
    private FactStructureInfo createEmptyFactSturectureDTO() {
        FactStructureInfo factStructure = new FactStructureInfo();
    ...
    private String areRulesTheSame(BusinessRuleInfo originalRule, BusinessRuleInfo currentRule) {
        ...
        List<RuleElementInfo> origElementList = originalRule.getBusinessRuleElementList();
        List<RuleElementInfo> curElementList = currentRule.getBusinessRuleElementList();

        RuleElementInfo lowestOrdinalPositionOrig = new RuleElementInfo();
        lowestOrdinalPositionOrig.setOrdinalPosition(-1);
        RuleElementInfo lowestOrdinalPositionCur = new RuleElementInfo();
        ...
        for (int i = 0; i < curElementList.size(); i++) {
            ...
            RuleElementInfo temporarilyLowestOrig = new RuleElementInfo();
            temporarilyLowestOrig.setOrdinalPosition(99);
            for (RuleElementInfo originalRuleElement : origElementList) {
            ...
            RuleElementInfo temporarilyLowestCur = new RuleElementInfo();
            temporarilyLowestCur.setOrdinalPosition(99);
            for (RuleElementInfo curRuleElement : curElementList) {
            ...
            RulePropositionInfo originalProposition = lowestOrdinalPositionOrig.getBusinessRuleProposition();
            RulePropositionInfo currentProposition = lowestOrdinalPositionCur.getBusinessRuleProposition();
            ...
            YieldValueFunctionInfo originalYVF = originalProposition.getLeftHandSideInfo().getYieldValueFunctionInfo();
            YieldValueFunctionInfo currentYVF = currentProposition.getLeftHandSideInfo().getYieldValueFunctionInfo();
            if (!originalYVF.getYieldValueFunctionInfoType().equals(currentYVF.getYieldValueFunctionInfoType())) {
                return "Proposition's YVF Type";
            }

            List<FactStructureInfo> factListOrig = originalYVF.getFactStructureList();
            List<FactStructureInfo> factListCur = currentYVF.getFactStructureList();
            if (factListOrig.size() != factListCur.size()) {
                return "Proposition's number of Facts";
            }

            int ix = -1;
            for (FactStructureInfo originalFact : factListOrig) {

                ix++;
                
                // find the matching current fact
                FactStructureInfo matchingFact = factListCur.get(ix);
                /* we don't keep fact IDs so we shouldn't use it to match...
                for (FactStructureInfo currentFact : factListCur) {
    ...
    private void retrieveFactTypes() {
        if (definedFactTypesList == null) {
            ...
            DevelopersGuiService.Util.getInstance().fetchBusinessRuleType(displayedRule.getType(), displayedRule.getAnchorTypeKey(), new AsyncCallback<BusinessRuleTypeInfo>() {
                public void onFailure(Throwable caught) {
                    // just re-throw it and let the uncaught exception handler deal with it
                    Window.alert(caught.getMessage());
                }

                public void onSuccess(BusinessRuleTypeInfo ruleTypeInfo) {
                    Logger.info("Loading fact type key list: " + ruleTypeInfo.getFactTypeKeyList());
                    loadingFactTypeKeyList = false;
                    definedFactTypesList = new ArrayList<FactTypeInfo>();

                    // get fact type info for each fact type from the fact service
                    DevelopersGuiService.Util.getInstance().fetchFactTypeList(ruleTypeInfo.getFactTypeKeyList(), new AsyncCallback<List<FactTypeInfo>>() {
                        public void onFailure(Throwable caught) {
                            Window.alert(caught.getMessage());
                        }

                        public void onSuccess(List<FactTypeInfo> factTypeInfo) {
...
//        ruleVersionVerticalPanel.add(rulesVersionsTable);
    ...
    private void displayTestPageRuleFacts() {
        ...
        YieldValueFunctionInfo yvf;
        int ix = 0;
        for (Map.Entry<Integer, RuleElementInfo> entry : definedPropositions.entrySet()) {

            ix++;
            prop = entry.getValue().getBusinessRuleProposition();
            yvf = prop.getLeftHandSideInfo().getYieldValueFunctionInfo();
            if (yvf == null) {
                continue;
            }
            ...
            leftAndRightSide.add(new Label(GuiUtil.getYVFSymbol(yvf.getYieldValueFunctionInfoType()) + "  "));
