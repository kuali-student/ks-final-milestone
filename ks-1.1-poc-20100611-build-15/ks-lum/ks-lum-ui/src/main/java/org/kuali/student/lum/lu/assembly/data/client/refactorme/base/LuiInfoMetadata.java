...
import org.kuali.student.lum.lu.assembly.data.client.refactorme.base.LuiInfoHelper.Properties;


public class LuiInfoMetadata
{
	...
	public void loadChildMetadata (Metadata mainMeta, String type, String state,  RecursionCounter recursions)
	{
		...
		// metadata for luiCode
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.LUI_CODE.getKey (), childMeta);
		...
		// metadata for cluId
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.CLU_ID.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.STRING);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
		...
		// metadata for atpKey
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.ATP_KEY.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.STRING);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("required"));
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("single"));
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("valid.atp.types"));
		}
		
		// metadata for maxSeats
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.MAX_SEATS.getKey (), childMeta);
