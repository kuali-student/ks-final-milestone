...
import org.kuali.student.lum.lu.assembly.data.client.refactorme.base.SearchTypeInfoHelper.Properties;


public class SearchTypeInfoMetadata
{
	...
	public void loadChildMetadata (Metadata mainMeta, String type, String state,  RecursionCounter recursions)
	{
		...
		// metadata for searchResultTypeInfo
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.SEARCH_RESULT_TYPE_INFO.getKey (), childMeta);
		...
		new SearchResultTypeInfoMetadata ().loadChildMetadata (childMeta, type, state, recursions);
		
		// metadata for searchCriteriaTypeInfo
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.SEARCH_CRITERIA_TYPE_INFO.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.DATA);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
		...
		new SearchCriteriaTypeInfoMetadata ().loadChildMetadata (childMeta, type, state, recursions);
