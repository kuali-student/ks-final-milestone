...
import org.kuali.student.lum.lu.assembly.data.client.refactorme.base.OrgOrgRelationTypeInfoHelper.Properties;


public class OrgOrgRelationTypeInfoMetadata
{
	...
	public void loadChildMetadata (Metadata mainMeta, String type, String state,  RecursionCounter recursions)
	{
		...
		// metadata for revName
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.REV_NAME.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.STRING);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
		...
		// metadata for revDesc
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.REV_DESC.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.STRING);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
		...
		// metadata for orgHierarchyKey
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.ORG_HIERARCHY_KEY.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.STRING);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
