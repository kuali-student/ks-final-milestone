...
import org.kuali.student.lum.lu.assembly.data.client.refactorme.base.OrgInfoHelper.Properties;


public class OrgInfoMetadata
{
	...
	public void loadChildMetadata (Metadata mainMeta, String type, String state,  RecursionCounter recursions)
	{
		...
		// metadata for sortName
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.SORT_NAME.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.STRING);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
		...
		// metadata for shortDesc
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.SHORT_DESC.getKey (), childMeta);
		...
		// metadata for longDesc
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.LONG_DESC.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.DATA);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
		...
		// metadata for orgCodes
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.ORG_CODES.getKey (), childMeta);
		...
		new OrgCodeInfoMetadata ().loadChildMetadata (listMeta, type, state, recursions);
