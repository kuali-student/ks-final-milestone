...
import org.kuali.student.lum.lu.assembly.data.client.refactorme.base.StatementInfoHelper.Properties;


public class StatementInfoMetadata
{
	...
	public void loadChildMetadata (Metadata mainMeta, String type, String state,  RecursionCounter recursions)
	{
		...
		if (this.matches (type, state, "(default)", "(default)"))
		{
			...
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("statement.operators"));
		}
		
		// metadata for statementIds
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.STATEMENT_IDS.getKey (), childMeta);
		...
		if (this.matches (type, state, "(default)", "(default)"))
		{
			...
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("related.statementd"));
		}
		...
		if (this.matches (type, state, "(default)", "(default)"))
		{
			...
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("statement.types"));
		}
		...
		if (this.matches (type, state, "(default)", "(default)"))
		{
			...
			childMeta.getConstraints ().add (ConstraintMetadataBank.BANK.get ("statement.states"));
		}
