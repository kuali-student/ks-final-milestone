...
/**
 * This class handles all the requests for Managing HolidayCalendar, such as,
 * view, edit, copy, delete, etc. This controller is mapped to the view defined
 * in <code>HolidayCalendarView.xml</code> and its subviews
 * <code>HolidayCalendarEditPage.xml</code>,
 * <code>HolidayCalendarCopyPage.xml</code>, and
 * <code>HolidayCalendarViewPage.xml</code>
 * 
...
public class HolidayCalendarController extends UifControllerBase {
	...
	/**
	 * This is the add action when the add button is pushed in the Holiday
	 * calendar edit/create page.
	 * 
	 * @param form
	 * @param result
	 * @param request
	...
	public ModelAndView addLine(@ModelAttribute("KualiForm") UifFormBase form,
		...
		if (newCollectionLines != null && !newCollectionLines.isEmpty()) {
			for (Map.Entry<String, Object> entry : newCollectionLines
					.entrySet()) {
				...
				if (checkHoliday(newHoliday)) {
					if (holidays != null && !holidays.isEmpty()) {
						for (HolidayWrapper holiday : holidays) {
							boolean duplicated = isDuplicateHoliday(newHoliday,
									holiday);
							if (duplicated) {
								GlobalVariables
										.getMessageMap()
										.putError(KRADConstants.GLOBAL_ERRORS,
												RiceKeyConstants.ERROR_CUSTOM,
												"ERROR: The holiday being added is already in the collection.");
	...
	private boolean checkHoliday(HolidayWrapper holiday) {
		...
		// RiceKeyConstants.ERROR_CUSTOM,
		// "ERROR: The adding holiday start date should not be later than the end date.");
		// return false;
		// }else if (timeDiff == 0 ) {
		// holidayInfo.setIsDateRange(false);
		// }else {
	...
	private boolean isDuplicateHoliday(HolidayWrapper newHoliday,
			HolidayWrapper sourceHoliday) {
	...
	public ModelAndView startNew(
			@ModelAttribute("KualiForm") HolidayCalendarForm form,
			BindingResult result, HttpServletRequest request,
			HttpServletResponse response) {
		HolidayCalendarInfo hcInfo = null;

		try {
			String calendarId = request
					.getParameter(CalendarConstants.CALENDAR_ID);
			if (calendarId == null || calendarId.trim().isEmpty()) {
				hcInfo = getHolidayCalendarFormHelper(form)
						.getNewestHolidayCalendar();
			} else {
				hcInfo = getHolidayCalendarFormHelper(form).getHolidayCalendar(
						calendarId);
			}
		} catch (Exception e) {
			GlobalVariables.getMessageMap().putError(
					...
					RiceKeyConstants.ERROR_CUSTOM,
					"Unexpected error; could not get holiday to copy: "
							+ e.getMessage());
		}
		...
		if (null != hcInfo) {
			...
			if (endYear == currentYear) {
				...
				/*
				 * force user to enter dates cal.add(Calendar.YEAR, 1);
				 * form.setNewCalendarEndDate(cal.getTime());
				 * cal.setTime(hcInfo.getStartDate()); cal.add(Calendar.YEAR,
				 * 1); form.setNewCalendarStartDate(cal.getTime());
				 */
			}
		}
	...
	/**
	 * This is called when the user clicked on Copy button in the Calendar
	 * Search page with Holiday calendar selected.
	 * 
	 * @param form
	 * @param result
	 * @param request
	...
	public ModelAndView copyForNew(
			@ModelAttribute("KualiForm") HolidayCalendarForm form,
			BindingResult result, HttpServletRequest request,
			HttpServletResponse response) {
		HolidayCalendarInfo hcInfo = null;
	...
	/**
	 * This is called when the user clicked on Copy Holiday Calendar in the
	 * Calendar copy page.
	 * 
	 * @param form
	 * @param result
	...
	public ModelAndView copy(
			...
			HttpServletResponse response) {
		restoreForm(form);
		if ((null == form.getHolidayCalendarInfo())
				|| (null == form.getHolidayCalendarInfo().getId())) {
		...
		try {
			newHolidays = getHolidayCalendarFormHelper(form)
		...
		for (HolidayWrapper holidayWrapper : newHolidays) {
			// decision #14, copy over all information (types, dates, times,
			// flags, etc) for Holidays from the source calendar
			holidayWrapper.getHolidayInfo().setId(null); // else the old rcd
															// will be updated
		}
		...
		form.getHolidayCalendarInfo().setStartDate(
				form.getNewCalendarStartDate());
		form.getHolidayCalendarInfo().setEndDate(form.getNewCalendarEndDate());
		form.getHolidayCalendarInfo().setStateKey(
				AtpServiceConstants.ATP_DRAFT_STATE_KEY);
		// after changing the state in the info back to the default, reset the
		// isOfficialCalendar flag on the form
		form.setOfficialCalendar(false);
		form.getHolidayCalendarInfo().setDescr(
				CommonUtils.buildDesc(form.getNewCalendarName()));
	...
	/**
	 * redirect to search Calendar page This is called when the user clicked on
	 * 'Choose a Different Calendar' action link in the create Holiday Calendar
	 * page.
	 * 
	 * @param form
	 * @param result
	...
	public ModelAndView search(
		...
		urlParameters.put(CalendarConstants.CALENDAR_SEARCH_TYPE,
				CalendarConstants.HOLIDAYCALENDER);
		return super.performRedirect(form, controllerPath, urlParameters);
	}

	/**
	 * This is called when the user clicked on Start a blank calendar instead?
	 * action link in the create Holiday Calendar page.
	 * 
	 * @param hcForm
	 * @param result
	 * @param request
	...
	public ModelAndView toCreate(
			@ModelAttribute("KualiForm") HolidayCalendarForm hcForm,
			BindingResult result, HttpServletRequest request,
			HttpServletResponse response) {
		hcForm.setHcId(null);
	...
	private void restoreForm(HolidayCalendarForm hcForm) {
		...
		if (StringUtils.isBlank(hcalInfo.getId())
				&& StringUtils.isNotBlank(hcForm.getOrgHcId())) {
			try {
	...
	/**
	 * Method used to save HC This is the add action when the Update Official
	 * button is pushed in the Holiday calendar edit/create page.
	 * 
	 * @param hcForm
	 * @param result
	 * @param request
	...
	public ModelAndView save(
			...
			BindingResult result, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		return updateHolidayCalendarForm(hcForm,
				CalendarConstants.MessageKeys.INFO_HOLIDAY_CALENDAR_SAVED,
				CalendarConstants.UPDATE_SAVE);
	}

	/**
	 * This is called when the user clicked on Delete Draft in the
	 * holidayCalendarCopyPage.
	 * 
	 * @param hcForm
	 * @param result
	 * @param request
	...
	public ModelAndView delete(
			...
			BindingResult result, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		getHolidayCalendarFormHelper(hcForm).deleteHolidayCalendar(
				hcForm.getHolidayCalendarInfo().getId());
		Properties urlParameters = new Properties();
		...
		return performRedirect(hcForm, request.getRequestURL().toString(),
				urlParameters);
	}

	/**
	 * Method used to set HC official This is called when the Make Official
	 * button is pushed in the Holiday calendar create page.
	 * 
	 * @param hcForm
	 * @param result
	 * @param request
	...
	public ModelAndView makeOfficial(
			@ModelAttribute("KualiForm") HolidayCalendarForm hcForm,
			BindingResult result, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		hcForm.getHolidayCalendarInfo().setStateKey(
				AtpServiceConstants.ATP_OFFICIAL_STATE_KEY);
		return updateHolidayCalendarForm(hcForm,
				CalendarConstants.MessageKeys.INFO_HOLIDAY_CALENDAR_OFFICIAL,
				CalendarConstants.UPDATE_MAKEOFFICIAL);
	}

	/**
	 * This is called when the user clicked on delete button in the Holiday
	 * calendar edit page.
	 * 
	 * @param hcForm
	 * @param result
	...
	public ModelAndView deleteHoliday(
		...
		String selectedCollectionPath = hcForm
		...
		if (selectedLineIndex == -1) {
			throw new RuntimeException(
					"unable to determine the selected line index");
		}

		try {
			getHolidayCalendarFormHelper(hcForm).deleteHoliday(
					selectedLineIndex, hcForm);
		} catch (Exception e) {
			GlobalVariables.getMessageMap().putError(
					KRADConstants.GLOBAL_ERRORS, RiceKeyConstants.ERROR_CUSTOM,
					"Error deleting holiday - " + e.getMessage());
			e.printStackTrace();
		}
	...
	private ModelAndView updateHolidayCalendarForm(HolidayCalendarForm hcForm,
			String updateMsg, String from) throws Exception {

		getHolidayCalendarFormHelper(hcForm).validateHolidayCalendar(hcForm);
		getHolidayCalendarFormHelper(hcForm).populateHolidayCalendarDefaults(
				hcForm);
		...
		hcForm.setStateName(getHolidayCalendarFormHelper(hcForm)
				.getHolidayCalendarState(hCalInfo.getStateKey()));
		hcForm.setNewCalendar(false);
		hcForm.setOfficialCalendar(hCalInfo
				.getStateKey()
				.equals(AcademicCalendarServiceConstants.ACADEMIC_CALENDAR_OFFICIAL_STATE_KEY));
		hcForm.setHcId(hCalInfo.getId());

		// sort holidays again, in case any were added
		Collections.sort(hcForm.getHolidays());

		GlobalVariables.getMessageMap().putInfo("holidayCalendarInfo.name",
				updateMsg, hCalInfo.getName());
	...
	private boolean isValidHolidayCalendar(HolidayCalendarInfo hc)
			throws Exception {
	...
	private void getHolidayCalendar(String hcId, HolidayCalendarForm hcForm)
			throws Exception {
		HolidayCalendarInfo hcInfo = getHolidayCalendarFormHelper(hcForm)
				.getHolidayCalendar(hcId);
		hcForm.setHolidayCalendarInfo(hcInfo);
		hcForm.setAdminOrgName(getAdminOrgNameById(hcInfo.getAdminOrgId()));
		hcForm.setStateName(getHolidayCalendarFormHelper(hcForm)
				.getHolidayCalendarState(hcInfo.getStateKey()));
		hcForm.setNewCalendar(false);
		hcForm.setOfficialCalendar(hcInfo
				.getStateKey()
				.equals(AcademicCalendarServiceConstants.ACADEMIC_CALENDAR_OFFICIAL_STATE_KEY));
		// List<HolidayWrapper> holidays =
		// getHolidayCalendarFormHelper(hcForm).getHolidaysForHolidayCalendar(hcForm);
		List<HolidayWrapper> holidays = getHolidayCalendarFormHelper(hcForm)
				.getHolidayWrappersForHolidayCalendar(hcInfo.getId());
	...
	private HolidayCalendarViewHelperService getHolidayCalendarFormHelper(
			HolidayCalendarForm hcForm) {
		if (hcForm.getView() != null
				&& hcForm.getView().getViewHelperServiceClass() != null) {
			return (HolidayCalendarViewHelperService) hcForm.getView()
					.getViewHelperService();
		} else {
			return (HolidayCalendarViewHelperService) hcForm.getPostedView()
					.getViewHelperService();
		}
	}
