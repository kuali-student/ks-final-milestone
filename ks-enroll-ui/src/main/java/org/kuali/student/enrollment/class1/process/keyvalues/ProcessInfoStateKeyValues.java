 ...
 */
public class ProcessInfoStateKeyValues extends UifKeyValuesFinderBase implements Serializable {
    ...
    public List<KeyValue> getKeyValues(ViewModel model) {
        ...
        try {
            List<StateInfo> states = getStateService().getStatesByLifecycle(ProcessServiceConstants.PROCESS_LIFECYCLE_KEY, context);
            for (StateInfo state : states) {
                if(state.getKey().startsWith("kuali.process.process.state")) { //TODO remove check after data is fixed
