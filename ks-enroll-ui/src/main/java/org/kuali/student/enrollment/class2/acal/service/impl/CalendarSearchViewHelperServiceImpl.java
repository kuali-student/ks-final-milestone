...
public class CalendarSearchViewHelperServiceImpl extends
		KSViewHelperServiceImpl implements CalendarSearchViewHelperService {
	private final static Logger LOG = Logger
			.getLogger(CalendarSearchViewHelperServiceImpl.class);
	 ...
	 * @throws Exception
	 */
	public List<TermInfo> searchForTerms(String name, String year,
			ContextInfo context) throws Exception {
		return CalendarSearchViewHelperUtil.searchForTerms(name, year, context,
				getAcademicCalendarService());
	}

	/**
	 * This method search AcademicCalendars based on the input name and the
	 * input year
	 * 
	 * @param name
	 * @param year
	...
	public List<AcademicCalendarInfo> searchForAcademicCalendars(String name,
			String year, ContextInfo context) throws Exception {
		return CalendarSearchViewHelperUtil.searchForAcademicCalendars(name,
				year, context, getAcademicCalendarService());
	}

	/**
	 * This method search HolidayCalendars based on the input name and the input
	 * year
	 * 
	 * @param name
	 * @param year
	...
	public List<HolidayCalendarInfo> searchForHolidayCalendars(String name,
			String year, ContextInfo context) throws Exception {
		return CalendarSearchViewHelperUtil.searchForHolidayCalendars(name,
				year, context, getAcademicCalendarService());
	}
	...
	public Properties buildTermURLParameters(TermInfo term,
			String methodToCall, boolean readOnlyView, ContextInfo context) {

		String acalId = null;
		try {
			List<AcademicCalendarInfo> atps = getAcademicCalendarService()
					.getAcademicCalendarsForTerm(term.getId(), context);
			if (!atps.isEmpty()) {
				acalId = atps.get(0).getId();
			}
		} catch (Exception e) {
			if (LOG.isDebugEnabled()) {
				LOG.debug("Error calling getAcademicCalendarsForTerm - "
						+ term.getId());
		...
		props.put(CalendarConstants.PAGE_ID,
	...
	public Properties buildACalURLParameters(AcademicCalendarInfo acal,
		...
		if (StringUtils.equals(methodToCall, CalendarConstants.AC_COPY_METHOD)) {
			props.put(CalendarConstants.PAGE_ID,
					CalendarConstants.ACADEMIC_CALENDAR_COPY_PAGE);
		} else {
			props.put(CalendarConstants.PAGE_ID,
					CalendarConstants.ACADEMIC_CALENDAR_EDIT_PAGE);
		}
	...
	public Properties buildHCalURLParameters(HolidayCalendarInfo hcInfo,
		...
		} else if (StringUtils.equals(methodToCall,
				CalendarConstants.HC_VIEW_METHOD) && readOnlyView) {
			props.put(CalendarConstants.PAGE_ID,
