 ...
 */
public class CourseOfferingEditMaintainableImpl extends
		CourseOfferingMaintainableImpl implements CourseOfferingMaintainable {
	private static final long serialVersionUID = 1L;
	private final static Logger LOG = Logger
			.getLogger(CourseOfferingEditMaintainableImpl.class);

	private transient OrganizationService organizationService;
	private transient LRCService lrcService;
	private transient AcademicCalendarService acalService;

	// TODO : implement the functionality for Personnel section and its been
	// delayed now since the backend implementation is not yet ready
	// (06/06/2012).

	@Override
	public void saveDataObject() {
		if (getMaintenanceAction()
				.equals(KRADConstants.MAINTENANCE_EDIT_ACTION)) {
	...
	private void updateCourseOffering(CourseOfferingEditWrapper coEditWrapper) {
		try {
			...
			for (OrganizationInfoWrapper orgWrapper : coEditWrapper
			...
			if (coEditWrapper
					...
					&& !coEditWrapper.getCreditOption().getFixedCredit()
							.isEmpty()) {
				ResultValuesGroupInfo rvgInfo = getLrcService()
						.getCreateFixedCreditResultValuesGroup(
								coEditWrapper.getCreditOption()
										.getFixedCredit(),
								LrcServiceConstants.RESULT_SCALE_KEY_CREDIT_DEGREE,
								contextInfo);
				coInfo.setCreditOptionId(rvgInfo.getKey());
			} else if (coEditWrapper
					.getCreditOption()
					.getTypeKey()
					.equals(LrcServiceConstants.RESULT_VALUES_GROUP_TYPE_KEY_RANGE)
					&& !coEditWrapper.getCreditOption().getMinCredits()
							.isEmpty()
				...
				ResultValuesGroupInfo rvgInfo = getLrcService()
						.getCreateMultipleCreditResultValuesGroup(
								coEditWrapper.getCreditOption().getCredits(),
								LrcServiceConstants.RESULT_SCALE_KEY_CREDIT_DEGREE,
								contextInfo);
				coInfo.setCreditOptionId(rvgInfo.getKey());
			}
			...
			// TODO REMOVE THIS AFTER KRAD CHECKLISTS ARE FIXED for student
			// registration options
			if (coEditWrapper.getAuditStudentRegOpts()
					&& !coInfo.getStudentRegistrationGradingOptions().contains(
							LrcServiceConstants.RESULT_GROUP_KEY_GRADE_AUDIT)) {
				coInfo.getStudentRegistrationGradingOptions().add(
						LrcServiceConstants.RESULT_GROUP_KEY_GRADE_AUDIT);
			} else {
				coInfo.getStudentRegistrationGradingOptions().remove(
						LrcServiceConstants.RESULT_GROUP_KEY_GRADE_AUDIT);
			}
			...
			getCourseOfferingService().updateCourseOffering(coInfo.getId(),
					coInfo, contextInfo);

			// check for changes to states in CO and related FOs (may happen in
			// the case of deleted FOs)
			ViewHelperUtil
	...
	private void updateFormatOfferings(CourseOfferingEditWrapper coEditWrapper)
		...
		List<FormatOfferingInfo> formatOfferingList = coEditWrapper
		...
		if (formatOfferingList != null && !formatOfferingList.isEmpty()) {
			for (FormatOfferingInfo formatOfferingInfo : formatOfferingList) {
				if (formatOfferingInfo.getId() != null
					...
					// update FO
					if (coInfo.getFinalExamType() != null
							&& !coInfo.getFinalExamType().equals("STANDARD")) {
						formatOfferingInfo.setFinalExamLevelTypeKey(null);
					}
					FormatOfferingInfo updatedFormatOffering = getCourseOfferingService()
							.updateFormatOffering(formatOfferingInfo.getId(),
									formatOfferingInfo, contextInfo);
					updatedFormatOfferingList.add(updatedFormatOffering);
					currentFOIds.remove(formatOfferingInfo.getId());
				} else {
					// create a new FO
					formatOfferingInfo
							.setStateKey(LuiServiceConstants.LUI_FO_STATE_PLANNED_KEY);
					formatOfferingInfo
							.setTypeKey(LuiServiceConstants.FORMAT_OFFERING_TYPE_KEY);
					...
					FormatOfferingInfo createdFormatOffering = getCourseOfferingService()
							.createFormatOffering(coInfo.getId(),
									formatOfferingInfo.getFormatId(),
									formatOfferingInfo.getTypeKey(),
									formatOfferingInfo, contextInfo);
		...
		if (currentFOIds != null && currentFOIds.size() > 0) {
			for (String formatOfferingId : currentFOIds) {
				...
				// Note by bonnie deleteAO invoked in
				// deleteFormatOfferingCascaded seems not completely correct.
				// I didn't see the code if removing FO-AO relations before
				// deleting AOs....
				getCourseOfferingService().deleteFormatOfferingCascaded(
						formatOfferingId, contextInfo);
			}
		}
	}

	private List<String> getExistingFormatOfferingIds(String courseOfferingId)
			throws Exception {
		List<FormatOfferingInfo> formatOfferingInfoList = getCourseOfferingService()
	...
	protected boolean performAddLineValidation(View view,
		...
		if (addLine instanceof OfferingInstructorInfo) {
			...
			CourseOfferingEditWrapper coEditWrapper = (CourseOfferingEditWrapper) form
					.getDocument().getNewMaintainableObject().getDataObject();
			List<OfferingInstructorWrapper> instructors = coEditWrapper
					.getInstructors();
			if (instructors != null && !instructors.isEmpty()) {
				for (OfferingInstructorWrapper thisInst : instructors) {
					if (instructorInfo.getPersonId().equals(
							thisInst.getOfferingInstructorInfo().getPersonId())) {
						GlobalVariables
								.getMessageMap()
								.putErrorForSectionId(
										"KS-CourseOfferingEdit-PersonnelSection",
										ActivityOfferingConstants.MSG_ERROR_INSTRUCTOR_DUPLICATE,
			...
			// validate ID
			List<Person> lstPerson = ViewHelperUtil
					.getInstructorByPersonId(instructorInfo.getPersonId());
			if (lstPerson == null || lstPerson.isEmpty()) {
				GlobalVariables
						.getMessageMap()
						.putErrorForSectionId(
								"KS-CourseOfferingEdit-PersonnelSection",
								ActivityOfferingConstants.MSG_ERROR_INSTRUCTOR_NOTFOUND,
								instructorInfo.getPersonId());
				return false;
			}
		}

		return super.performAddLineValidation(view, collectionGroup, model,
				addLine);
	}
	...
	protected void processAfterAddLine(View view,
			...
			boolean isValidLine) {
		if (addLine instanceof OfferingInstructorInfo) {
			// set the person name if it's null, in the case of user-input
			// personell id
			OfferingInstructorInfo instructorInfo = (OfferingInstructorInfo) addLine;
			if (instructorInfo.getPersonName() == null
					&& instructorInfo.getPersonId() != null) {
				List<Person> personList = ViewHelperUtil
						.getInstructorByPersonId(instructorInfo.getPersonId());
			...
			if (instructorInfo.getStateKey() == null) {
				instructorInfo
						.setStateKey(LprServiceConstants.TENTATIVE_STATE_KEY);
			}
		}
	}
	...
	public Object retrieveObjectForEditOrCopy(MaintenanceDocument document,
		...
		try {
			...
			if (getDataObject() instanceof CourseOfferingEditWrapper) {
				...
				CourseOfferingInfo coInfo = getCourseOfferingService()
						.getCourseOffering(dataObjectKeys.get("coInfo.id"),
								contextInfo);
				CourseInfo courseInfo = getCourseService().getCourse(
						coInfo.getCourseId(), contextInfo);

				// 1. set CourseOfferingInfo
				CourseOfferingEditWrapper formObject = new CourseOfferingEditWrapper(
						coInfo);
				...
				List<FormatOfferingInfo> formatOfferingList = getCourseOfferingService()
						.getFormatOfferingsByCourseOffering(coInfo.getId(),
								contextInfo);
				formObject.setFormatOfferingList(formatOfferingList);

				// 4. Checking if Grading Options should be disabled or not and
				// assign default (if no value)
				// 5. Checking if there are any student registration options
				// from CLU for screen display
				...
				if (coInfo.getCourseId() != null && courseInfo != null) {
					List<String> gradingOptions = courseInfo
							.getGradingOptions();
					Set<String> regOpts = new HashSet<String>(
					...
					// Audit is pulled out into a dynamic attribute on course so
					// map it back
					if ("true"
							.equals(courseInfo
									.getAttributeValue(CourseAssemblerConstants.COURSE_RESULT_COMP_ATTR_AUDIT))) {
						studentRegOptions
				...
				if (coInfo.getStudentRegistrationGradingOptions().contains(
						LrcServiceConstants.RESULT_GROUP_KEY_GRADE_AUDIT)) {
				...
				// 6. Defining Credit Option and if CLU is fixed (then it's
				// disabled)
				...
				List<ResultValuesGroupInfo> courseCreditOptions = courseInfo
						.getCreditOptions();

				// Lookup the related course's credit constraints and set them
				// on the creditOption
				if (coInfo.getCourseId() != null
						&& !courseCreditOptions.isEmpty()) {
					ResultValuesGroupInfo resultValuesGroupInfo = courseCreditOptions
							.get(0);
					// Check for fixed
					if (resultValuesGroupInfo
						...
						if (!resultValuesGroupInfo.getResultValueKeys()
								.isEmpty()) {
							creditOption.setCourseFixedCredits(getLrcService()
									.getResultValue(
						...
						// Default the value
						creditOption
								.setTypeKey(LrcServiceConstants.RESULT_VALUES_GROUP_TYPE_KEY_FIXED);
						creditOption.setFixedCredit(creditOption
								.getCourseFixedCredits());
					} else {
						// This is either range or multiple

						// Copy all the allowed credits and sort so that the
						// multiple checkboxes can be properly displayed
						List<ResultValueInfo> resultValueInfos = getLrcService()
								.getResultValuesForResultValuesGroup(
										resultValuesGroupInfo.getKey(),
										contextInfo);
						for (ResultValueInfo rVI : resultValueInfos) {
							creditOption.getAllowedCredits()
									.add(rVI.getValue());
						}
						...
						if (resultValuesGroupInfo
							...
							creditOption
									.setCourseMinCredits(resultValuesGroupInfo
											.getResultValueRange()
											.getMinValue());
							creditOption
									.setCourseMaxCredits(resultValuesGroupInfo
											.getResultValueRange()
											.getMaxValue());

							// Default the value
							creditOption
									.setTypeKey(LrcServiceConstants.RESULT_VALUES_GROUP_TYPE_KEY_RANGE);
							creditOption.setMinCredits(creditOption
									.getCourseMinCredits());
							creditOption.setMaxCredits(creditOption
									.getCourseMaxCredits());
							...
							// Default the value
							creditOption
									.setTypeKey(LrcServiceConstants.RESULT_VALUES_GROUP_TYPE_KEY_MULTIPLE);
							creditOption.getCredits().addAll(
									creditOption.getAllowedCredits());
						}
					}
				}

				// Lookup the selected credit option and set from persisted
				// values
				if (creditOptionId != null) {
					// Lookup the resultValueGroup Information
					ResultValuesGroupInfo resultValuesGroupInfo = getLrcService()
							.getResultValuesGroup(creditOptionId, contextInfo);
					String typeKey = resultValuesGroupInfo.getTypeKey();

					// Get the actual values
					List<ResultValueInfo> resultValueInfos = getLrcService()
							.getResultValuesByKeys(
									resultValuesGroupInfo.getResultValueKeys(),
									contextInfo);
					...
					if (typeKey
						...
						if (!resultValueInfos.isEmpty()) {
							creditOption.setFixedCredit(resultValueInfos.get(0)
									.getValue());
						}
					} else if (typeKey
							.equals(LrcServiceConstants.RESULT_VALUES_GROUP_TYPE_KEY_RANGE)) {
						creditOption
								.setTypeKey(LrcServiceConstants.RESULT_VALUES_GROUP_TYPE_KEY_RANGE);
						creditOption.setMinCredits(resultValuesGroupInfo
								.getResultValueRange().getMinValue());
						creditOption.setMaxCredits(resultValuesGroupInfo
								.getResultValueRange().getMaxValue());
					} else if (typeKey
							.equals(LrcServiceConstants.RESULT_VALUES_GROUP_TYPE_KEY_MULTIPLE)) {
						creditOption
								.setTypeKey(LrcServiceConstants.RESULT_VALUES_GROUP_TYPE_KEY_MULTIPLE);
				...
				formObject
						.setOrganizationNames(new ArrayList<OrganizationInfoWrapper>());
				...
				if (coInfo.getUnitsDeploymentOrgIds() != null) {
					for (String orgId : coInfo.getUnitsDeploymentOrgIds()) {
						OrgInfo orgInfo = getOrganizationService().getOrg(
								orgId, contextInfo);
				...
				TermInfo termInfo = getAcalService().getTerm(
						coInfo.getTermId(), contextInfo);
				StringBuilder termStartDate = new StringBuilder(
						DateFormatters.MONTH_DAY_YEAR_DATE_FORMATTER
								.format(termInfo.getStartDate()));
				StringBuilder termEndDate = new StringBuilder(
						DateFormatters.MONTH_DAY_YEAR_DATE_FORMATTER
								.format(termInfo.getEndDate()));
				String termStartEnd = termInfo.getName() + " (" + termStartDate
						+ " to " + termEndDate + ")";
				...
				document.getDocumentHeader().setDocumentDescription(
						"Edit CO - " + coInfo.getCourseOfferingCode());

				// StateInfo state =
				// getStateService().getState(formObject.getDto().getStateKey(),
				// contextInfo());
	...
	private FormatInfo getFormatInfo(
			CourseOfferingEditWrapper courseOfferingEditWrapper, String coFormId) {
		List<FormatInfo> formatInfoList = courseOfferingEditWrapper.getCourse()
				.getFormats();
	...
	protected LRCService getLrcService() {
		if (lrcService == null) {
			lrcService = (LRCService) GlobalResourceLoader
					.getService(new QName("http://student.kuali.org/wsdl/lrc",
							"LrcService"));
		}
	...
	protected AcademicCalendarService getAcalService() {
		if (acalService == null) {
			acalService = (AcademicCalendarService) GlobalResourceLoader
					.getService(new QName("http://student.kuali.org/wsdl/acal",
