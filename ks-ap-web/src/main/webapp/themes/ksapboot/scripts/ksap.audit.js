...
function getPendingAudit(id, type) {
    if (jQuery.cookie('ksap_audit_running')) {
        ...
        if (type == data.auditType) {
            ...
            if (data) {
                var item = jQuery("<div />").addClass("uif-collectionItem pending").html('<img src="../themes/ksapboot/images/ajaxLoader.gif" class="icon"/><span class="title">Auditing <span class="program">' + data.programName + '</span></span>');
