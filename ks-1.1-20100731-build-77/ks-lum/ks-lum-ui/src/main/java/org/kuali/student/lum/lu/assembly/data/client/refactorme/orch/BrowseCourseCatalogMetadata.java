...
import org.kuali.student.lum.lu.assembly.data.client.refactorme.orch.BrowseCourseCatalogHelper.Properties;


public class BrowseCourseCatalogMetadata
{
	...
	public void loadChildMetadata (Metadata mainMeta, String type, String state,  RecursionCounter recursions)
	{
		...
		// metadata for BySubjectArea
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.BY_SUBJECT_AREA.getKey (), childMeta);
		...
		new BrowseCourseCatalogBySubjectAreaMetadata ().loadChildMetadata (childMeta, type, state, recursions);
		
		// metadata for BySchoolOrCollege
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.BY_SCHOOL_OR_COLLEGE.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.DATA);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
		...
		new BrowseCourseCatalogBySchoolOrCollegeMetadata ().loadChildMetadata (childMeta, type, state, recursions);
