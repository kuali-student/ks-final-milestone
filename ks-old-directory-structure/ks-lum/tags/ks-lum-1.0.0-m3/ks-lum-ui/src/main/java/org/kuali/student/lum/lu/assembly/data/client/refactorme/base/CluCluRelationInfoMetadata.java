...
import org.kuali.student.lum.lu.assembly.data.client.refactorme.base.CluCluRelationInfoHelper.Properties;


public class CluCluRelationInfoMetadata
{
	...
	{
		int recurseLevel = increment (recursions, "CluCluRelationInfoMetadata");
		...
		// metadata for relatedCluId
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.RELATED_CLU_ID.getKey (), childMeta);
		childMeta.setDataType (Data.DataType.STRING);
		childMeta.setWriteAccess (Metadata.WriteAccess.ALWAYS);
		if (this.matches (type, state, "(default)", "(default)"))
		{
		...
		// metadata for isCluRelationRequired
		childMeta = new Metadata ();
		mainMeta.getProperties ().put (Properties.IS_CLU_RELATION_REQUIRED.getKey (), childMeta);
