...
public class UpdateOriginBucketMojo extends S3Mojo {
    ...
    protected void recurse(AmazonS3Client client, String prefix, String delimiter) throws IOException {
        ...
        // Create an index.html file unless the bucket already has one in this directory that was not created by this plugin
        if (isCreateOrUpdateIndex(client, objectListing, defaultFileKey)) {
            client.putObject(getPutIndexObjectRequest(html, defaultFileKey));
        }

        // Don't attempt this if we are in the root "directory" of the bucket
        if (prefix != null) {
            // Place the index.html we generate under the key without a trailing slash, eg "foo/bar"
            ...
            // That is why we return our internally generated index.html if someone types in the url without a trailing slash
            ...
            getLog().info("#############");
            getLog().info("#############");
            getLog().info("trimmedPrefix=" + trimmedPrefix);
            getLog().info("#############");
            getLog().info("#############");
            client.putObject(getPutIndexObjectRequest(html, trimmedPrefix));

            // Lastly, copy the contents of index.html (either what we generated or what was already there) to "foo/bar/"
            // This allows the url "http://www.mybucket.com/foo/bar/" to return the proper html
            client.copyObject(getCopyObjectRequest(getBucket(), defaultFileKey, prefix));
        }
    ...
    protected boolean isCreateOrUpdateIndex(AmazonS3Client client, ObjectListing objectListing, String indexKey) {
        if (!isExistingObject(objectListing, indexKey)) {
            // There is no index, we will create one
            return true;
        }
        // There is an index, but it might have been the one created by this plugin
        ...
        // If it is our index, we need to update it, otherwise, we'll leave it alone
        return isOurIndex;
    }
