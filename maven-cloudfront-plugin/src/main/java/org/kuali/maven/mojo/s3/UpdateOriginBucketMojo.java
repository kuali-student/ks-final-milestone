...
/**
 * <p>
 * This mojo updates a bucket serving as an origin for a Cloud Front
 * distribution. It generates an html directory listing for each "directory" in
 * the bucket and stores the html under a key in the bucket such that a regular
 * http request for a directory returns the html instead of the XML for
 * "object does not exist" Amazon would normally return. For example: The url
 * "http://www.mybucket.com/foo/bar" returns an html page containing a listing
 * of all the files and directories under "foo/bar" in the bucket.
 * </p>
 * <p>
 * If a directory contains an object with a key that is the same as the default
 * object, the plugin copies the object to a key representing the directory
 * structure. For example, the url "http://www.mybucket.com/foo/bar/index.html"
 * represents an object in an S3 bucket under the key "foo/bar/index.html". This
 * plugin will copy the object from the key "foo/bar/index.html" to the key
 * "foo/bar/". This causes the url "http://www.mybucket.com/foo/bar/" to return
 * the same content as the url "http://www.mybucket.com/foo/bar/index.html"
 * </p>
 * <p>
 * It also generates an html directory listing at the root of the bucket
 * hierarchy and places that html into the bucket as the default object, unless
 * a default object already exists.
 * </p>
 *
...
public class UpdateOriginBucketMojo extends S3Mojo {
    ...
    /**
     * This controls the caching behavior for CloudFront. By default CloudFront
     * edge locations cache content from an S3 bucket for 24 hours. That
     * interval is shortened to 1 hour by default for the directory listing html
     * generated by this plugin.
     *
     * @parameter expression="${cacheControl}"
     * default-value="max-age=3600, must-revalidate"
     */
    ...
    /**
     * If true, "foo/bar/index.html" will get copied to "foo/bar". This is
     * defaulted to false because the relative pathing in the html generated by
     * the maven-site-plugin does not render correctly from a url without the
     * trailing slash.
     *
     * @parameter expression="${copyDefaultObjectWithoutDelimiter}"
     * default-value="false"
     */
     ...
     * default-value="http://s3browse.ks.kuali.org/css/style.css"
     */
    private String css;
     ...
     *
     * @parameter expression="${fileImage}"
     ...
     *
     * @parameter expression="${directoryImage}"
     ...
     * default-value="EEE, dd MMM yyyy HH:mm:ss z"
     */
    private String dateFormat;

    /**
     * The key containing the default object for the Cloud Front distribution.
     * If this default object already exists, the plugin will not modify it. If
     * there is no object in the bucket under this key, this plugin will
     * generate an html directory listing and place the directory listing into
    ...
    protected void goUpTheChain(final S3BucketContext context, final String startingPrefix)
        ...
        String[] prefixes = StringUtils.splitByWholeSeparator(startingPrefix,
    ...
    /**
     * Create a PutObjectRequest for some html generated by this mojo. The
     * PutObjectRequest sets the content type to S3_INDEX_CONTENT_TYPE, sets the
    ...
    /**
     * Return a SimpleDateFormat object initialized with the date format and
     * timezone supplied to the mojo
     */
    ...
    protected String getAbout() {
        ...
        PluginDescriptor descriptor = (PluginDescriptor) this
                .getPluginContext().get("pluginDescriptor");
        if (descriptor == null) {
            // Maven 2.2.1 is returning a null descriptor
            return "Listing generated by the maven-cloudfront-plugin on "
                    + date;
        } else {
            String name = descriptor.getArtifactId();
            String version = descriptor.getVersion();
            return "Listing generated by the " + name + " v" + version + " on "
                    + date;
        }
    }

    /**
     * Create an object in the bucket under a key that lets a normal http
     * request function correctly with CloudFront / S3.<br>
    ...
    protected void updateDirectory(final S3PrefixContext context,
        ...
        if (containsDefaultObject && isCopyIfExists) {
            ...
            context.getBucketContext()
                    .getClient()
                    .copyObject(
                            getCopyObjectRequest(context.getBucketContext()
                                    .getBucket(),
                                    context.getDefaultObjectKey(), copyToKey));
        } else {
            // Upload our custom content
            context.getBucketContext()
                    .getClient()
                    .putObject(
                            getPutIndexObjectRequest(context.getHtml(),
                                    copyToKey));
        }
    }
    ...
    protected void updateDirectory(final S3PrefixContext context) throws IOException {
        ...
        updateDirectory(context, isCopyDefaultObjectWithDelimiter(),
                context.getPrefix());

        // Handle "http://www.mybucket.com/foo/bar"
        updateDirectory(context, isCopyDefaultObjectWithoutDelimiter(),
                trimmedPrefix);
    ...
    /**
     * If this is the root of the bucket and the default object either does not
     * exist or was created by this plugin, overwrite the default object with
    ...
    protected S3PrefixContext getS3PrefixContext(final S3BucketContext context,
            final String prefix) {
        ListObjectsRequest request = new ListObjectsRequest(
                context.getBucket(), prefix, null, context.getDelimiter(), 1000);
        ObjectListing objectListing = context.getClient().listObjects(request);
        List<String[]> data = converter.getData(objectListing, prefix,
                context.getDelimiter());
        String html = generator.getHtml(data, prefix, context.getDelimiter());
        String defaultObjectKey = StringUtils.isEmpty(prefix) ? getDefaultObject()
                : prefix + getDefaultObject();
        String browseHtmlKey = StringUtils.isEmpty(prefix) ? getBrowseHtml()
                : prefix + getBrowseHtml();
    ...
    /**
     * Recurse the hierarchy of a bucket starting at "prefix" and create entries
     * in the bucket corresponding to the directory structure of the hierarchy
     */
    protected void recurse(final S3BucketContext context, final String prefix)
            throws IOException {
        ...
        List<String> commonPrefixes = prefixContext.getObjectListing()
                .getCommonPrefixes();
    ...
    /**
     * Return true if there is no object in the ObjectListing under
     * defaultObjectKey.<br>
     * Return true if the object in the ObjectListing was created by this
     * plugin.<br>
    ...
    protected boolean isCreateOrUpdateDefaultObject(final S3PrefixContext context) {
        ...
        S3Object s3Object = s3Context.getClient().getObject(
                s3Context.getBucket(), context.getDefaultObjectKey());
    ...
    /**
     * Return true if this S3Object was created by this plugin. This is is done
     * by checking the metadata attached to this object for the presence of a
     * custom value.
     */
     ...
     */
    protected CopyObjectRequest getCopyObjectRequest(final String bucket,
            final String sourceKey, final String destKey) {
    ...
    public void setCopyDefaultObjectWithDelimiter(
            final boolean copyDefaultObjectWithDelimiter) {
    ...
    public void setCopyDefaultObjectWithoutDelimiter(
            final boolean copyDefaultObjectWithoutDelimiter) {
